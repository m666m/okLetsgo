...
在虚拟机里安装的操作系统上使用桌面应用程序，安全性最高，在这里的桌面冲浪吧。如果有隔离需求，那就再复制一个虚拟机即可。

简单方案，用 hyper-v 安装一个 Windows 10，在这里安装各种cn软件并主力使用。

偏执狂解决强迫症方案：用 u 盘安装安全 Linux 发行版，在里面安装虚拟机，然后在虚拟机里安装安全 Linux 发行版，在这个虚拟机上使用你的各种保密程序。日常使用时，开机用u盘启动进入 live 模式，尽情冲浪，关机后所有写操作都不会保存。参见章节 [虚拟机下运行面向安全的 Linux 发行版](init_a_server think)。
