...
但是，3D 游戏画面显示技术想再进一步，实现动态效果的照片级真实感，比如像过场动画一样的游戏效果，光栅化处理却做不到：比如光源无法变化，阳光射入的室内通过预烘培的光照能给你照片级真实感，但是太阳这个光源不能移动，场景也不能被破坏和移动，可交互性很差。自由运动的游戏人物和角色，在光线变化时很难跟场景中光线的色调协调，你会看到游戏角色跑进黑洞但仍然发白的现象。光栅化也无法实现汽车表面这种曲面的反射和镜面对照。另外，对规模越来越大的开放世界场景的 RPG 游戏来说，预烘培生成的光照信息，最多可以高达几十TB，这已经无法部署到玩家硬盘了。
...
光线追踪的原理是对玩家屏幕上的每个像素点都进行实时计算，精确实现光线与物体表面的互动：
    ...
    一个像素点投射n多个光线，打到各个物体上，反射后如果达到光源如灯、太阳，则屏幕上的这个像素点就发什么颜色的光。其计算混合了物体的材质、粗糙度，光源的颜色等多种因素，实际显示效果比传统的光栅化处理更加真实，未来甚至可以做到像照片那么真实。

光线追踪的先天缺陷一：达到 1080p 下可以接受的画面，光线追踪的计算量都超级大
...
光线追踪的先天缺陷二：因为每个像素点都要投射光线，噪点是光线追踪技术的先天问题
...
最终，我们得到了充满各种妥协的光线追踪游戏画面：
    ...
    因为优化光线追踪就得模糊处理，所以干脆对物体的材质在处理时就加大粗糙度，这是降清晰度的，即便是高清材质也要糊一下才发布。
...
Nvidia 在显卡计算能力不足时强推光线追踪，导致了到 2025 年光线追踪游戏的画面效果还不如 10 年前的游戏：
    ...
    比如，优化最好的，死黑基本看不到的，Doom:The Dark Ages。号称完全使用光线追踪（其实是部分烘焙再加强制硬件光追），计算量超级大：为了减少反射的计算量，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于部分水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。相比 DOOM2016/DOOM ETERNAL 各种减少视觉元素，居然只是画面整体的光影真实感更强一些，并无任何惊艳。功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。<https://doomwiki.org/wiki/Tiago_Sousa>

    赛博朋克2077，糊的情况稍微好点，暗部死黑太难受了，玩这游戏最好晚上出门。

    所以 2025年末发布的游戏《战地6》直接宣布当前阶段不使用光线追踪技术，优先保证玩家体验。
...
光线追踪和路径追踪的概念：
    ...
    N 卡 4080 时代宣传的路径追踪是增加二次反射和漫反射，逐渐的在抛弃光栅化渲染，更多的使用光追来形成画面，这极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 完全使用光线追踪渲染画面，跟上一代的 Doom:Eternal 相比只是光影感觉更真实，画质细节完全没有进步，远看不错贴近就是糊。
...
很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体暗部出现死黑，只看暗部又会慢慢变亮，让你的眼睛非常不舒服。这不是什么黑屋出门看强光的 HDR 过曝效果，这就是游戏对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：
    ...
    好的例子是 Doom:The dark ages，全局光照（RTGI）调校的好，高光给人冲击时背光面也有足够的亮度能看清细节，明暗过渡非常自然，面向明暗交错的物体转动视角时眼睛完全不累。在场景设计方面，低光的山洞或水下也会细心的放置光源，死黑很少见。
...
游戏内的设置，开启 “路径追踪”，可以稍微增强画面整体的亮度和明暗表现，真实感更强。这是利用的光线的多次反射，物体反光后也会投射光线到其他物体，反光物体即使是有粗糙度的材质也会使周围环境稍微变亮。另外，火焰、灯条等自发光材质也能稍微照亮周边，这些在增加了光的二次反弹后都可以增加周边物体的亮度。

路径追踪会增强间接光照射物体的明暗感觉，真实性更好。判断是否使用了路径追踪，可背向光源看反光面，是否反光到其他物体，那些物体是否改变了亮度和颜色，比如光线照射的桌面，桌子是否照亮了周边的墙面和物体。
