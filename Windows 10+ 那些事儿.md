...
1、游戏设置里把胶片颗粒、电影质感、动态模糊等选项关掉试试。

2、以前的游戏，一般是调整 LOD 对应画质细节。

光线追踪流行后，如果感觉你的游戏中画质有点糊，不够清晰锐利，不要怀疑是原图就这样，游戏设置里找找显示，把锐化相关选项拉高再看看，如果启用了 DLSS，可能是 DLSS 的锐化选项

    Doom:The Dark Ages 锐化拉到 2~3 清晰度就跟之前的 Doom：Eternal 差不多了，不知道为啥这游戏出厂默认的锐化值拉的低。

锐化数值太大会让你的图形噪点有点明显，酌情调整。

3、还有个驱动程序级别的覆盖设置方式，用 steam 截图是原图会不体现效果：
...
2、驱动程序级别的覆盖设置方式，用 steam 截图是原图会不体现效果：

开启 Nvidia RTX 动态亮丽，让你的游戏暗处不再那么黑

    如果游戏支持，直接在游戏内设置该选项即可。
...
还有个使用方式
    ...
    运行你的游戏，进入游戏后按热键 alt+z，弹出的信息浮窗选择 “游戏滤镜”，锐化和动态亮丽等功能都可以直接勾选启用。
...
目前支持 HDR 模式的应用：
    ...
    VLC 播放器等支持 HDR 显示器的应用，在播放 HDR 内容时会自动通知显示器切换状态。另外对于 Nvidia RTX 视频增强，vlc 有个特殊的优化版本 <https://download.videolan.org/testing/vlc-rtx-upscaler/>
