...
光线追踪流行后设计原则变了，如果你在游戏选项中把画质细节拉满了还是觉得画面“糊”，大致原因就是游戏引擎切换为未来的方向 --- 光线追踪：
    ...
    200x 年的游戏技术基本都是光栅化，可以实现光线的反射折射透射，并依靠游戏开发时预烘培实现全局光照，这样已经可以给玩家非常真实的感觉了，贴图细节调高就可以看到非常清晰的画面。

    想要进一步的增加真实感，不得不切换到使用光线追踪技术，其在反射、折射、透射上最明显。光线追踪技术在光影方面对每个像素点进行实时计算：屏幕上的每个像素点都投射n多个光线，打到各个物体上，反射后如果达到有颜色的发光点如灯、太阳，则屏幕上的这个像素点就发什么颜色。其计算混合了物体的材质，光源的强弱等多种因素，实际显示效果就比传统的光栅化处理更加真实。

缺陷一：达到 1080p 下可以接受的画面，光线追踪的计算量都超级大

这个技术从 2000 年代就开始探讨，随着人类电子制造技术的进步，直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 才成为了现实，体验见 赛博朋克 2077。如果计算二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降30%+。

缺陷二：因为每个像素点都要投射光线，噪点是光线追踪技术的先天问题。

电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在游戏里，只能发射更多光线或者用算法降噪。

    发射的光线越多则画面效果越好，但需要庞大的算力，4090 显卡在这面前也只能说是“小小的也很可爱”。

    发射的光线数量越少，则计算量就越小，但是画面会暗，噪点会变多

所以，在光线不足条件下的降噪是很现实的解决方案，高斯模糊就来了。模糊处理会导致抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。

最终，我们得到了充满了各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 前后帧动态模糊的方式解决物体边缘的锯齿现象，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，这就是 TAA 的特点。

    因为优化光线追踪就要模糊，所以干脆对普通物体材质处理时就加大粗糙度，这是降清晰度的，高清材质也得糊一下才发布。

    Nvidia DLSS 用 AI 生成画面来减少计算量。从低分辨率生成高分辨率画面，到直接插帧生成新画面，各种歪招全都有。

比如 Doom:The Dark Ages，抛弃光栅化渲染完全使用光线追踪，场景中绝大部分材质限制在粗糙度阈值之下，以大大减少反射的计算，动态光源的使用非常克制，很少放置玻璃和镜子，反射远景的物体很少，镜面效果只见于水面和地面的水洼。各种降低视觉效果，换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 5080 显卡开路径追踪都跑不到 60 帧/秒。
