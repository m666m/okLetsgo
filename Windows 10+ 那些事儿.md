...
DLDSR 能够以更高、更细腻的分辨率渲染游戏，然后把画面智能地缩放至显示器的原生分辨率，在任意屏幕上为玩家提供 4K (3840x2160) 画质的图形效果
    ...
    DSR（虚拟超级分辨率）功能是 Nvidia 在 2014 年推出的，现在升级了叫 DLDSR(深度学习动态超分辨率)，号称效果更好 DLDSR 2.25x 相当于 DSR 4x。根据网友实测，对部分游戏有奇效。

开启方法：

    先在 Nvidia 控制面板的 3D 设置中找到选项 DSR 因数，可以调整渲染时放大的倍数，1080P 显示器选择 4x 则分辨率变成 4K (3840x2160)。DSR 4.00x就是1颗显示器像素显示的内容是显卡用4颗像素模拟计算后得出的，开启它可以明显提升电脑清晰度。

    然后 Windows 桌面进分辨率设置，分辨率选项已经有 3840X2160 的选项了，说明 DSR 已经成功开启。（桌面用如果字体显示各种扭曲发糊考虑调整字体缩放）
