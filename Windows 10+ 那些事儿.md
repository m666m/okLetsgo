...
Nvidia 在显卡计算能力不足时强推光线追踪，导致了到 2025 年光线追踪游戏的画面效果还不如 10 年前的游戏：
    ...
    跟之前的光栅游戏相比，完全光线追踪技术的游戏画面真实感轻微提升，清晰度明显下降，光影反射效果明显提升，物体暗部死黑太多（减少使用预烘焙又不舍得多给实时光线），帧率巨幅下降，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 基本都是如此。注：UE5 引擎开发的《寂静岭f》画面效果倒退到了 2000 年代。

    比如，优化最好的，死黑基本看不到的，Doom:The Dark Ages。号称完全使用光线追踪（其实是部分烘焙再加强制硬件光追），计算量超级大：为了减少反射的计算量，除了主角，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于部分水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。相比 DOOM2016/DOOM ETERNAL 各种减少视觉元素，居然只是画面整体的光影真实感更强一些，并无任何惊艳。功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。<https://doomwiki.org/wiki/Tiago_Sousa>
...
如果制作时优先考虑观众视觉感受，CGI 背景可以给你大景深的清晰宏大视野，画面细节逼真元素丰富，运动镜头也能保证不是全糊适当看清，最有代表性的就是《阿凡达》系列。
...
dvd 时代 x264 等算法，简单粗暴的用模糊降画质，高压缩视频的视觉效果太差，画面模糊黑暗。
