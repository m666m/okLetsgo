...
宿主机里的应用添加到 “工作空间”，实际使用起来很方便而且是隔离的：
    ...
    在实体机环境安装的 app 默认不会被添加到 “工作空间”，如果单独选择某个 app “添加到工作空间运行”，app 是全新安装完毕的状态，实体机内使用的该 app 的配置文件和数据信息不会被克隆。

    不用担心 app 被复制两次占用系统存储，容器化技术可以保证只需要安装一次 app，多个工作空间的运行，其实是利用了已安装的 app 的可执行部分，在运行时的系统的进程是隔离的，进程的数据存储也是隔离的。

    区分两个应用就是观察图标上是否带有公文包徽章。
...
安全限制：
    ...
    在 “工作空间” 内部运行的 app，都受到 “工作空间” 权限控制的管理，哪怕这个 app 绕过安全权限的限制，读取到的也是虚拟机本体的裸机状态，不会读取到实机的应用和数据。
...
实际使用起来很方便，而且数据是隔离，也可以手工选择 “添加到安全文件夹” 或 “移出安全文件夹”。
