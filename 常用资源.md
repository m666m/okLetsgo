...
目标系统类型 UEFI/CSM
    ...
    例如，显卡不支持UEFI模式。早期的Windows 7无法很好地支持UEFI，因此需要CSM来检测UEFI功能是否已完全启用。也就是说，主板BIOS在CSM模式下，对UEFI进行支持（甚至提供选项使用非UEFI的最古老方式），但是这跟Windows 10的那种开机后直接UEFI引导操作系统快速进入桌面有区别。
...
参考
    ...
    https://www.163.com/dy/article/FTJ5LN090531NEQA.html
