...
### 安装双系统
    ...
    双启动仍无法保护 /boot 分区，在操作系统引导阶段对木马监控程序的防护差，建议操作系统安装时开启 Secure Boot 功能以增强抵抗力。

    为防止 Windows 上的木马病毒扫描 Linux 上的数据，建议 Linux 安装时选择加密所在分区选项，同理。
...
hyper-v 虚拟机，使用宿主机的无线网卡建立虚拟交换机的，都有这个情况，虚拟机里上网打开网页卡顿甚至超时、从虚拟机到宿主机拷贝文件速度特别慢等，只要用到无线网卡虚拟交换机的场景，都有这个问题，而微软至今(2025)仍未优化这个性能问题。关联问题是章节 [Windows 访问 samba 速度慢](init_a_server think)。
    ...
    我用无线路由器桥接的办法解决了，宿主机用有线连接到路由器，虚拟交换机使用有线网卡就再也不慢了。。。
...
某些游戏把光线追踪、反锯齿等高画质功能拉满后，特别是在 4K 分辨率的显示器下，显存占用会超过 8GB，会提示显存不足，严重情况下一卡一卡的
    ...
    如果没爆显存，游戏优化的好，画质稍微降低或切换到内存，你感觉不出来

或者凑合下，开启 DLSS4 功能，可以稍微降低对显存的需求

推荐换用 16GB+ 显存的显卡，比如 GigaByte GeForce RTX™ 5070 Ti EAGLE OC SFF 16G

    https://www.gigabyte.cn/Graphics-Card/GV-N507TEAGLE-OC-16GD

### Nvidia 控制面板的输出动态范围选择 “完全”
...
1、使用显示器对应的 ICC 颜色配置文件：通常可以在显示器品牌官网中的服务与支持页面下载获取对应的 ICC 颜色配置文件，通过 “设置 —> 显示 —> 高级显示 —> 显示器 1 的适配器属性 —> 颜色管理” 来调整，切换到设备选项卡，添加并选中使用该颜色配置文件。

使用Windows系统内置的“HDR显示器校准”工具（设置→系统→显示器→HDR），根据向导调整亮度平衡和色彩饱和度。

2、HDR 技术用于家庭影院很好，但不太适合做电脑显示器

    阳光照耀的自然光环境，亮度在几万 nit，HDR 技术的目标是还原这个自然光照的效果...

    因为人是在室内相对偏暗的环境下用电脑的，27 英寸 HDR1000 显示器的亮度大概相当于近距离面对 60w 的白炽灯，会感到很晃眼

一旦进入 HDR 模式，显示器的亮度控制会自动拉至最高，显示器里设置开启的睿动光感、环境光、低蓝光模式、SmartImage 等动态对比度、自动亮度调节等可能与HDR产生冲突的功能均被关闭，黑暗房间中的屏幕会像个大光球，为了你的眼睛健康，显示器后面最好有个背景灯打光，显示器距离人眼的距离最好拉远到 1 米之外。这样的用法，32 寸 4k hdr 显示器比较合适，27 寸的嫌小了。

3、不要在 Windows 桌面环境开启 HDR 模式，仅在相关应用中设置开启，原因见章节 [显示器开启 HDR 后 Win10 桌面变灰泛白的原因]。
    ...
    VLC 播放器等支持 HDR 显示器的应用，在播放 HDR 内容时会自动通知显示器切换状态。对 RTX 视频增强，vlc 有个特殊的优化版本 <https://download.videolan.org/testing/vlc-rtx-upscaler/>

    大多数游戏支持在设置里选择开启 HDR，在开始游戏后，会自动切换显示器模式以显示 HDR 内容

如果你的应用不支持自行管理 HDR 模式，而是跟随桌面环境的 HDR 设置，那就只能在打开应用之前在桌面环境手工开启 HDR

    部分显示器在自己的设置菜单预设了电影/游戏等 HDR 模式，可以尝试使用
...
如果是普通 sdr 显示器播放 hdr 视频太暗
    ...
    VLC Player 支持比较完善，自动在 SDR 环境下进行 HDR-SDR 转换，自动在 HDR 开启后以 HDR 显示

> 玩 Doom

显示器开启 hdr 模式后，doom 会自动检测到，自动跟随调整颜色。或者在 doom 的设置中选择开启 hdr，会自动设置显示器到 hdr 模式。
