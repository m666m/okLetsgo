...
光影对人类立体视觉的影响，以素描为例进行理解。在纸上画几条线段是二维图像，如果在背景上浓淡相宜的扫几笔，就制造出了光影，就有了真实的立体感。数码处理过程中，要再逼真就得加色彩，再逼真就得提升像素点的精细程度。这个技术路径就依赖于增加色彩和光影的细节，到数码相机级别的真实感需要使用大的CCD采样来实现，这时人类手绘的方式已经无法实现了。

2020 年代之前的游戏

    基本都是光栅化渲染画面的，可以实现光线的反射折射阴影，并依靠游戏开发时在场景中预烘培实现全局光照，这样已经可以给玩家非常真实的感觉了，贴图细节调高就可以看到非常清晰的画面，场景中光线充足各处都不会太黑。

    一般玩家只需要调整 LOD 即可对应画质细节，在游戏设置中 LOD 拉的越高画面就越清晰。

    真实物体如果只有简单的光线照射就会表现出卡通化，很多网游等对显卡要求很低的游戏，即使很清晰其画面风格都是卡通化的。

但是，画面显示技术想再进一步，实现照片级的真实感，光栅化处理做不到，特别是处理汽车表面这种曲面的反射和镜面对照这种情况。而且对开放世界场景的 RPG 游戏来说，光栅化预烘培生成的光照信息，最多可以高达几十TB，这实际无法部署到玩家硬盘了 --- 而光追技术可以解决这一切。

Nvidia 在2018年的 RTX 2080 开始推广光线追踪后，高端游戏的引擎技术发生了巨大的改变。

如果你在游戏选项中把画质细节拉满了还是觉得画面糊，亮度拉高了暗部还是黑，大致原因就是游戏引擎切换为使用光线追踪了。

光线追踪的原理是对玩家屏幕上的每个像素点都进行实时计算：

    一个像素点投射n多个光线，打到各个物体上，反射后如果达到光源如灯、太阳，则屏幕上的这个像素点就发什么颜色的光。其计算混合了物体的材质、粗糙度，光源的颜色等多种因素，实际显示效果比传统的光栅化处理更加真实，未来甚至可以像照片那么真实。
...
数字电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在电子游戏里，只能发射更多光线或者用算法降噪。
    ...
    发射的光线数量越少，则计算量就越小，但是画面噪点会变多，整体变暗
...
最终，我们得到了充满了各种妥协的光线追踪游戏画面：
    ...
    在光追游戏优化不到位的画面部位，你可以看到稍后生成的阴影。

    其实 Nvidia DLSS 也是 TAA，但新增了很多功能，比如从低分辨率生成高分辨率画面，用 AI 生成画面来减少计算量，甚至直接插帧生成新画面，各种歪招全都有。

总体来说，现在的光线追踪游戏，有点类似数码相机刚出来的时代，清晰度是不如传统相机的，但是多次迭代后清晰度达标了，几年功夫就实现了对传统相机的替代。Nvidia 在显卡计算能力不足时强推光线追踪，在 2025年 实现的游戏画面导致玩家无法理解：

    跟之前的光栅游戏相比，光线追踪技术的画面真实感轻微提升，清晰度明显下降（不如玩家贴2015年游戏的截图），反射效果明显提升，物体暗部死黑太多（预烘焙减少），帧率爆降，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 全都如此。

    比如 Doom:The Dark Ages，抛弃了光栅化渲染而完全使用光线追踪，为了减少反射的计算量，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。各种降低视觉效果，换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。
...
1、游戏设置里把动态模糊、胶片颗粒、电影质感、景深、色差、VRS 等选项关掉。
...
1、如果是游戏暗处太黑（含光线追踪），首先在游戏内设置调整“伽马值”，适当放大，达到暗处稍亮而亮处不发白即可。伽马值拉太高整个屏幕都会都发白，酌情调整。
