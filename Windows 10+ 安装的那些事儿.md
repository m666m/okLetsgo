...
Qubes
    ...
    Qubes 操作系统是一个基于 Fedora Linux 的发行版，它运行基于 Xen 的虚拟化环境，以便您可以将应用程序和连接的设备作为独立组件运行。
...
Qubes OS 和 Whonix 的无缝组合，可实现更高级的安全性和匿名性。
    ...
    优势 https://www.whonix.org/wiki/Qubes/Why_use_Qubes_over_other_Virtualizers

    安装后安全建议 https://www.whonix.org/wiki/Post_Install_Advice

具体实现是在物理机上安装 Qubes 作为虚拟机管理器（Xen 虚拟机管理程序），在 Qubes 内的虚拟机里安装运行 Whonix。
