...
4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器 4K 的优化技术、DLSS 等等歪招全来了。

如果感觉你的游戏中画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节拉到最高，抗锯齿选项也拉到最高，也没有效果，尤其是 TAA抗锯齿会糊的略重一点。把“锐化”相关选项拉高再看看，一般会解决问题。注意锐化数值太大会让亮处噪点明显，树皮的纹理变得非常不真实等，酌情调整。

如果启用了 DLSS，可能还有 DLSS 的锐化选项。另外，DLSS 的档位尽量选择“DLAA”或“质量”，比“性能”档更清晰。

锐化可以解决大部分画质糊的问题，比如 Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，总给人画面比较糊的感觉。画面设置中的“锐化”拉到 2~3 清晰感就跟之前的 Doom：Eternal 差不多了，跟过场动画的细节类似。
...
光线追踪的某些游戏，对光线直射和背光混杂的视角变化处理不好，亮处很亮而暗部画面很黑，除非转头只看暗处才能看清，在开启 HDR 时更明显，这个不是什么黑屋出门看强光的人眼效果，这个是光照曲线有问题，根源在于游戏的设计和调校。
    ...
    不好的例子是 赛博朋克2077 的白天场景各种物体的暗部动态死黑，在阳光部分直射的街上转头就有想打手电筒的感觉。
...
比如 Doom:Then Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，以大大减少反射的计算，场景中的动态光影和阴影特效也用的非常克制，场景内反射远景的物体很少，物体的暗部维持在可以看清。试试开启 “路径追踪”，画面整体的亮度增加，清晰感更强。
...
光线追踪和路径追踪
    ...
    N 卡 4080 时代宣传的路径追踪，就是完全抛弃光栅化渲染，完全使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:Then Dark Ages 不敢使用不锈钢材质，各种场景中绝大部分材质的粗糙度设置都是不反射，镜面效果只见于水面和地面的水洼，不敢放置玻璃和镜子，功耗近 400瓦 的 5080 显卡都跑不到 60 帧。
