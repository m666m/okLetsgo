...
1、游戏设置里把动态模糊、胶片颗粒、电影质感、景深、色差、分辨率缩放、VRS(可变着色率) 等选项关掉。
...
2、锐化
    ...
    如果启用了 DLSS，则还可以调整 DLSS 的锐化选项。DLSS 的档位尽量选择“DLAA”或“质量”，比“性能”档更清晰

    如果有“渲染倍率”选项，拉到100%，看看是否可以解决糊的问题
...
3、如果游戏使用的 TAA 反锯齿，提高帧率也可以使画面更清晰，这是这个技术的一个特点。

如果可能，尝试不使用 TAA，更换反锯齿算法为传统的 8xMSAA，或者切换到 DLSS4 + DLAA 并拉高锐化数值，DLSS 是 AI 帮你生成画面的技术，比原始 TAA 渲染清楚些。

4、使用 4K 分辨率，依靠使用高分辨率材质来提高清晰度（如果游戏在开发时没有对材质进行粗糙化模糊处理的话）。

把显示器从 1080P 换成 4K，画质也能有明显提升。有些游戏 2k 分辨率拉满设置里的抗锯齿选项，实际画面还不如 4K 显示器开 DLSS 性能档不开抗锯齿的效果。

4k 或更高的分辨率下不需要开启反锯齿了，在 nvidia 的驱动程序设置中，关闭 反锯齿 相关的各个选项，看游戏图像是否变清楚了。
...
5、类似的，提升视频网站和直播平台的清晰度，可以利用 Nvidia RTX 视频增强（Super Resolution）。
...
网络视频的特点，决定了高压缩视频不重视高质量还原，而是有损的，从而实现低码率（文件小、带宽低）。
...
dvd-1080p 时代 x264 等算法，简单粗暴的用模糊降画质，高压缩视频的视觉效果太差，画面模糊黑暗。

4k 时代 x265 等算法， 压缩算法变了，低码率视频的观感也变了：
