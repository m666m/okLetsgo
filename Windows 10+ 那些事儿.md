...
在开放式场景中的游戏常见背光面死黑、白天屋里很黑的情况，根源在于光线追踪迫于计算量优化不能放置太多光线，这就非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校：

    比如 黑神话·悟空，对比度拉的很高但是全局光照考虑的太少，大白天的阴影里愣是黑得伸手不见五指，尤其是第二章的山洞场景不是过曝亮瞎眼就是黑乎乎什么都看不到，沙漠场景中明暗同时存在的视角闪爆你的眼球。

    塞伯朋克2077 也是类似的问题，很多场景暗部死黑影响体验。

为了方便，很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体背光的暗部出现死黑，只看暗部又会受环境光照影响慢慢变亮，让你的眼睛非常不舒服。这不是什么黑屋看门口强光的 HDR 过曝效果，这就是游戏对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：

    比如 赛博朋克2077 的白天场景各种物体的背光面死黑，屋里也死黑，在阳光直射的大街上转动视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑就像犯了低血糖。不要说原本就是黑的，你只看暗部的时候它会慢慢变亮给你正确的结果。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站在大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。
...
相比之下，电影级的离线渲染对光线的计算都是不计工本的，每个像素点发出的光线就非常多，而且多次发射并计算反射、甚至反射点再发射光线进行计算，这在家用游戏机行业用实时渲染是做不到的。

缓解办法：

1、如果是游戏暗处太黑（含光线追踪），首先在游戏内设置调整“伽马值”，适当放大比如 1.1，达到暗处稍亮而亮处不发白即可。伽马值拉太高整个屏幕都会都发白，酌情调整。

如果是 HDR 显示器，游戏内一般会有个专门的 HDR 参数设置页面，可以调整亮度、色调等细节。

NOTE: 传统的调节显示器亮度对比度的方式，已经不适合现代游戏了：

    光线追踪的游戏，对比度亮度都是动态计算出来的，全局光照的曝光度也随着视角和环境变化，这都跟游戏的运行时有关，单纯调节显示器的亮度对比度不会起作用。

    最典型的就是游戏开启 HDR 模式，则显示器会自动切换到最高亮度模式，全靠游戏或视频的亮度数据来动态变化明暗。

2、增加光线

游戏内的设置，开启 “路径追踪”，可以稍微增强画面整体的亮度和明暗表现，而且真实感更强。开启了光线的多次反弹，让物体反光后也会投射光线到其他物体，这样有粗糙度材质物体反光也会使周围环境稍微变亮。增加了光的二次反弹后，火焰、灯条等自发光材质也能稍微照亮周边物体。

路径追踪会增强间接光照射物体的明暗感觉，真实性更好。判断是否使用了路径追踪，可背向光源看反光面，是否反光到其他物体，那些物体是否改变了亮度和颜色，比如光线照射的桌面，桌子是否照亮了周边的墙面和物体、阳光照射的大街，不被直射的墙面也会变亮，而且门框等位置会出现阴影。
