...
UEFI模式一开, 都是直接厂商logo转一圈就直接进系统的. 不会再用主板启动画面和win10标.

其实secure boot是uefi设置中的一个子规格，简单的来说就是一个参数设置选项，它的作用体现在主板上只能加载经过认证过的操作系统或者硬件驱动程序，从而防止恶意软件侵入。

1.开启UEFI功能

    在“Boot”界面中，设置“Boot mode select”为“UEFI”，设置“Network”、“Storage”为 “UEFI only”，设置“Other PCI devices”为“UEFI first”。

2.设置“Secure Boot”为“Enable”

    选择进入“Key Management”界面，设置“Provision Factory Default keys”为 “Enable”，打开出厂默认密钥开关。

    设置“Provision Factory Default keys”为“Enable”后出现以下对话框，选择“Yes”，确认安装出厂默认密钥。

    确认“Advanced”界面存在对应的PCIe设备，PCIe卡中要有带数字签名的UEFI驱动，否则不会被加载。

设置完成后，按“F10”。
弹出“Save configuration and exit?”对话框。
选择“Yes”，按“Enter”保存设置并重新启动。
