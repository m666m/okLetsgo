...
电影的画风是按照电影院黑乎乎的环境下观看调校的

    sdr 标准亮度 80nit，很多电影拍摄时纸白（大致相当于平均亮度）设置就是 80-100nit，电影院里关灯看，感觉不会太黑。

    hdr 亮度等级：迄今无。很多导演都喜欢用阴冷风，在影院里看不黑就行，但是 hdr 效果导致暗部会更黑，需要高亮度高对比度的银幕才能体现细节。这就要求播放设备要高亮导致很容易高温，搜一下新闻---电影院播放阿凡达着火了，就是因为有些负责任的电影院为了保证原片效果用了高亮的电影灯，结果导致设备高温出现火情。所以，现在大多数电影院的播放亮度不足是很常见的，就像很少见到开通风的电影院一样。。。
...
tldr：
    ...
    在显卡计算能力还达不到完美的情况下，使用光线追踪就避免不了它的天生弱点：糊、暗
...
这个技术从 2000 年代就开始探讨用显卡实现，随着人类电子制造技术的进步，直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才成为了现实，体验见 赛博朋克 2077。如果计算二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降30%+。

缺陷二：因为每个像素点都要投射光线，噪点是光线追踪技术的先天问题。

电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在游戏里，只能发射更多光线或者用算法降噪。

    发射的光线越多则画面效果越好，但需要庞大的算力，5090 显卡在这面前也只能说是“小小的也很可爱”。

    发射的光线数量越少，则计算量就越小，但是画面噪点会变多，整体变暗

所以，在光线不足条件下的降噪是很现实的解决方案，高斯模糊就来了。但是，模糊处理在抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。

最终，我们得到了充满了各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，这就是 TAA 的特点。而从 UE5 引擎的各种游戏，到 Doom:The Dark Ages 全都切换到使用 TAA 技术了。

    因为优化光线追踪就要模糊，所以干脆对普通物体材质处理时就加大粗糙度，这是降清晰度的，高清材质也要糊一下才发布。

    Nvidia DLSS 用 AI 生成画面来减少计算量。从低分辨率生成高分辨率画面，到直接插帧生成新画面，各种歪招全都有。

也就是说，跟之前的光栅游戏相比，清晰度下降，反射效果提升，帧率爆降，这是2025年光线追踪游戏的现实，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 全都如此。

比如 Doom:The Dark Ages，抛弃光栅化渲染完全使用光线追踪，场景中绝大部分材质限制在粗糙度阈值之下，以大大减少反射的计算，镜面效果只见于水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。各种降低视觉效果，换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 5080 显卡开路径追踪都跑不到 60 帧/秒。
...
玩家可用的缓解办法：
    ...
    想看惊艳的光影效果还得再等2代显卡，到 8k 显示器时代像素点小到肉眼不可识别，轻微的模糊也不会影响高分辨率图片的细节了。

光线追踪和路径追踪：

    计算机图形学算法中，路径追踪是光线追踪的一个实现方式，而现实的发展中受显卡能力影响，光线追踪和路径追踪及光栅渲染分别划代了。

    N 卡 2080 时代宣传的光线追踪，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。比如 Doom:Eternal 开启光线追踪后的 “邪教徒基地关卡”，各种不锈钢材质都在反射周边的灯光甚至 slayer 角色模型，也敢于大范围布设玻璃和镜子，光栅和光追组合非常节省显卡的硬件资源，中低档显卡都能畅玩。

    N 卡 4080 时代宣传的路径追踪是增加二次反射和漫反射，逐渐的在抛弃光栅化渲染，更多的使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 完全使用光线追踪渲染画面，跟上一代的 Doom:Eternal 比只是光影感觉更真实，画质细节完全没有进步，远看不错贴近就是糊。
...
另外游戏帧数不足 60 帧的时候，有的游戏引擎会用动态模糊来增加流畅感，详见 [游戏不跟手 --- 延迟]。

2、锐化

如果感觉你的游戏画面有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高还是糊，游戏设置一般都有个 “锐化” 选项，拉高锐化的数值可以改善大部分这种画质糊的问题：

    锐化数值太大会让亮处噪点明显，树皮的纹理变得不真实等，酌情调整
...
很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体暗部出现死黑，只看暗部又会慢慢变亮，让你的眼睛非常不舒服。这不是什么黑屋出门看强光的 HDR 过曝效果，这就是游戏对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：
    ...
    比如 赛博朋克2077 的白天场景各种物体的暗部动态死黑，在阳光部分直射的大街上来回转头切换下视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑就像犯了低血糖。。。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。

    好的例子是 Doom:The dark ages，全局光照（RTGI）调校的好，高光给人冲击时暗部也有足够的亮度能看清细节，明暗过渡非常自然，面向明暗交错的物体转动视角时眼睛完全不累。在场景设计方面，没光的山洞或水下也会细心的放置光源，死黑很少见。
...
NOTE: 传统的调节显示器亮度对比度的方式，已经不适合现代游戏了：
    ...
    光线追踪的游戏，对比度亮度都是动态计算出来的，全局光照的曝光度也随着视角和环境变化，这都跟游戏的软件设置有关，你单纯调节显示器的亮度对比度不起作用。

    最典型的就是游戏开启 HDR 模式，则显示器会自动切换到最高亮度模式，全靠游戏或视频的亮度数据来动态变化明暗。

2、游戏内设置，开启 “路径追踪”，可以稍微增加画面整体的亮度，清晰感更强。这样可以利用光线的多次反射，即使物体是有粗糙度的材质也会稍微变亮。另外，火焰、灯光等自发光材质也能稍微照亮周边，这些都可以增加一点场景中物体的暗部亮度。

如果是虚幻5引擎开发的游戏，用工具调高 lumen 的漫反射颜色增强（Diffuse Color Boost），尽量增大光照，可以缓解暗部死黑的问题

    这种暗部死黑到无法营造黑暗氛围感的游戏很常见，根源在于光线追踪非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校。比如 黑神话·悟空，对比度拉的很高但是全局光照考虑的太少，大白天的阴影里愣是黑得伸手不见五指，尤其是第二章的山洞场景不是过曝亮瞎眼就是黑乎乎什么都看不到，沙漠场景中明暗同时存在的视角闪爆你的眼球。
...
我玩射击游戏，关垂直同步才不头晕
    ...
    建议除非遇到画面撕裂或抖动的问题，再开垂直同步。或者游戏支持的话开启三重缓冲可缓解头晕，但是偶尔会卡一下。
...
但是
    ...
    开 DLSS 帧生成 + Nvidia Reflex 后，Doom:The Dark Ages 的延迟反而很神奇的下降了，而且不需要开垂直同步锁帧。
...
这个方案的手感延迟是可以接受的，帧数能覆盖显示器刷新率也不会感到画面卡顿，最神奇的是 DLSS 生成的画面比游戏引擎原生还更清晰。
