...
如果达不到这个标准，或运行时波动太大，就可能出现卡顿的情况：
    ...
    画面卡顿：帧间距指的是两帧之间的时间，帧间距过大会影响观感，即眼睛看着画面感觉卡顿

1、每秒生成的帧数不足以覆盖显示器刷新率，帧间距过大造成画面卡顿：

    30 帧/s 时在 60hz 显示器上会感觉到画面卡顿

    60 帧/s 时在 120hz 显示器上会感觉到画面卡顿

举个例子，你的显示器刷新率为 300hz 情况下锁 30 帧渲染延迟有 1ms，锁 300 帧渲染延迟有 4ms，在实际体验上，锁 30帧/s 虽然 1ms 的渲染延迟不影响手感但是帧间距大会感到画面卡顿，而锁 300 帧的帧间距小眼睛看着更流畅，而且其 4ms 的渲染延迟也不影响手感，就应该选择锁 300 帧。比如有些低端的电视机的响应时间很长，但有的人在这种电视上打游戏感觉不出来滞后感，但如果游戏的每秒帧数从 60 帧降到 20 帧他就会觉得“卡”。帧间距过大可以用动态模糊骗掉眼睛，从而看起来很流畅；但是延迟的画面却不可以。
...
鼠标对准屏幕中心的某个物体，保持对准该物体横向平移跑动，或纵向前后跑动，如果画面中央出现轻微的水波纹或撕裂感，就说明需要开垂直同步。垂直同步会把游戏的帧率锁定为显示器的刷新率，但会导致玩家头晕。

帧数不足时不想开垂直同步的一个缓解办法是使用支持可变刷新率 VRR 的显示器如支持 Nvidia G-sync 标准的显示器，在低帧率时候有用，帧数超过显示器刷新率就没什么作用了，还是得开垂直同步。
...
头晕的原因可能在于相对于手的快速动作跟大脑一致，而屏幕的显示相对帧率波动性延迟，而 fps 射击游戏频繁快速的转身和视角变换，导致大脑对手运动的感知和对视觉的感知不一致，进而导致头晕。
    ...
    另外要把环境晃动、摄像头晃动都关了，有些游戏战斗时镜头快速变化或制造视觉模糊来制造压迫感，随后的胜利再给你释放压力的快感。但是晃画面是让人必晕的，没啥意思，关掉晃动反而游戏体验更好。
