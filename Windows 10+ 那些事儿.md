...
tldr：
    ...
    在显卡计算能力还达不到完美的情况下，使用光线追踪技术就避免不了它的天生弱点：糊、暗

光影对人类立体视觉的影响，以素描为例进行理解。在纸上画几条线段是二维图像，然后在背景浓淡相宜的扫几笔，就出来光影了，就有立体感。要再逼真就得加色彩，再逼真就得加大色彩和光影的细节变化。

以前光栅游戏时代，一般是调整 LOD 对应画质细节，游戏设置中 LOD 拉的越高越清晰。

光线追踪流行后设计原则变了，如果你在游戏选项中把画质细节拉满了还是觉得画面“糊”，大致原因就是游戏引擎切换为未来的方向 --- 光线追踪：

    200x 年的游戏技术基本都是光栅化，可以实现光线的反射折射阴影，并依靠游戏开发时预烘培实现全局光照，这样已经可以给玩家非常真实的感觉了，贴图细节调高就可以看到非常清晰的画面。

    2020 年代，为了进一步的增加真实感，不得不切换到使用光线追踪技术，其在反射、折射上效果最明显。光线追踪技术在光影方面对每个像素点都进行实时计算：一个像素点投射n多个光线，打到各个物体上，反射后如果达到光源如灯、太阳，则屏幕上的这个像素点就发什么颜色的光。其计算混合了物体的材质、粗糙度，光源的颜色等多种因素，实际显示效果就比传统的光栅化处理更加真实。

缺陷一：达到 1080p 下可以接受的画面，光线追踪的计算量都超级大
...
缺陷二：因为每个像素点都要投射光线，噪点是光线追踪技术的先天问题。

数字电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在游戏里，只能发射更多光线或者用算法降噪。

    发射的光线越多则画面效果越好，但需要庞大的算力，5090 显卡在这面前也只能说是“小小的也很可爱”。

    发射的光线数量越少，则计算量就越小，但是画面噪点会变多，整体变暗

所以在光线不足条件下，降噪是很现实的解决方案，高斯模糊等各种模糊方案就是解决王道。但是，模糊处理在抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。
...
最终，我们得到了充满了各种妥协的光线追踪游戏画面：
    ...
    因为优化光线追踪就要模糊，所以干脆对物体材质处理时就加大粗糙度，这是降清晰度的，即便是高清材质也要糊一下才发布。

    Nvidia DLSS 用 AI 生成画面来减少计算量。从低分辨率生成高分辨率画面，到直接插帧生成新画面，各种歪招全都有。

也就是说，跟之前的光栅游戏相比，画面光影真实感提升，清晰度下降，反射效果提升，帧率爆降，这是2025年光线追踪游戏的现实，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 全都如此。

比如 Doom:The Dark Ages，抛弃了光栅化渲染而完全使用光线追踪，场景中绝大部分物体的材质都限制在粗糙度阈值之下，以大大减少反射的计算，镜面效果只见于水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。各种降低视觉效果，换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。

玩家可用的缓解办法：

    画质选项里调高锐化数值，使凹凸细节更明显，在观感上改善清晰度。

    尝试不使用 TAA，更换反锯齿算法为传统的 8xSSAA，或者切换到 DLSS4 拉高锐化数值。

    开启 Nvidia 驱动程序自带的滤镜，直接给画面加亮度、加锐化。

    使用 4K 分辨率，依靠使用高分辨率材质来提高清晰度（如果游戏在开发时没有对材质进行粗糙化处理的话）。

    想看惊艳的光影效果估计还得再等2代显卡，到 8k 显示器时代像素点小到肉眼不可识别，轻微的模糊也不会影响观看高分辨率材质的细节了。

光线追踪和路径追踪：

    计算机图形学算法中，路径追踪是光线追踪的一个实现方式，而现实的发展中受显卡能力影响，光线追踪和路径追踪及光栅渲染分别划代了。

    N 卡 2080 时代宣传的光线追踪，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。比如 Doom:Eternal 开启光线追踪后的“邪教徒基地”关卡，各种不锈钢材质都在反射周边的灯光甚至 slayer 角色模型，也敢于大范围布设玻璃和镜子，光栅和光追组合非常节省显卡的硬件资源，中低档显卡都能畅玩。

    N 卡 4080 时代宣传的路径追踪是增加二次反射和漫反射，逐渐的在抛弃光栅化渲染，更多的使用光追的路径追踪算法来形成画面，这极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 完全使用光线追踪渲染画面，跟上一代的 Doom:Eternal 相比只是光影感觉更真实，画质细节完全没有进步，远看不错贴近就是糊。
...
如果感觉你的游戏画面有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高还是糊：
    ...
    游戏设置一般都有个 “锐化” 选项，拉高锐化的数值可以改善大部分这种画质糊的问题。锐化数值太大会让亮处噪点明显，树皮的纹理变得不真实等，酌情调整
...
3、如果游戏使用的 TAA 反锯齿，提高帧率也可以使画面更清晰，这是这个技术的特点。

4、把显示器从 1080P 换成 4K，画质也能有明显提升

有些游戏 2k 分辨率拉满设置里的抗锯齿选项，实际画面还不如 4K 显示器开 DLSS 性能档不开抗锯齿的效果。
...
很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体暗部出现死黑，只看暗部又会慢慢变亮，让你的眼睛非常不舒服。这不是什么黑屋出门看强光的 HDR 过曝效果，这就是游戏对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：
    ...
    比如 赛博朋克2077 的白天场景各种物体的暗部动态死黑，在阳光直射的大街上来回转头切换下视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑就像犯了低血糖。。。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。
...
如果是虚幻5引擎开发的游戏，用工具调高 lumen 的漫反射颜色增强（Diffuse Color Boost），尽量增大光照，可以缓解暗部死黑的问题
    ...
    这种暗部死黑的游戏很常见，根源在于光线追踪非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校。比如 黑神话·悟空，对比度拉的很高但是全局光照考虑的太少，大白天的阴影里愣是黑得伸手不见五指，尤其是第二章的山洞场景不是过曝亮瞎眼就是黑乎乎什么都看不到，沙漠场景中明暗同时存在的视角闪爆你的眼球。塞伯朋克2077 也是类似的问题，很多场景暗部死黑影响体验。
