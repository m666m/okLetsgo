...
merge 默认的操作不一定选择那种合并策略，参见章节 [`git merge` 默认策略有二义性]。
...
两分支合并方法
    ...
    # 不带参数有二义性，参见章节 [`git merge` 默认策略有二义性]
    git merge

    # 只尝试快进合并，否则不做，参见章节 [merge --ff-only 只做快进合并的拉直]
    merge --ff-only

    # 强制做分叉，参见章节 [merge --no-ff 强行分叉]
    git merge -no-ff

    # 强制做拉直，参见章节 [`rebase <branch>` 变基合并强行拉直]
    git rebase
...
酌情考虑如下合并策略：
    ...
    主干分支合入 hotfix， 属于主干分支上的接续，做变基合并拉直。
...
所以能做变基合并的几种情况是：
    ...
    一个本地分支做修改，一个远程分支跟多人同步，因为本地分支的 hash 值变更不会影响他人，可以做变基合并，方法见章节 [变基合并有远程分支要先合并到自身]。这个场景一般是用 hotfix 分支修复主干分支的缺陷，用变基合并保持提交记录的拉直状态。因为 hotfix 分支在合并到主干分支后就废弃了，所以在合并主干分支内容时用变基合并，变更它的提交记录的 hash 值没有影响。
...
解决方案如下：
    ...
    # 先把主干分支变基合并到 feature1 分支，这样变更的是 feature1 分支提交记录的 hash 值
    git rebase master feature1

    # 然后再把 feature1 合并到 master 分支，这时变成快进合并了，对 master 分支无任何影响。
    git switch master
    git merge feature1
...
而我们的要求是主干分支不出现分叉，拉直合并其它分支，那么只能使用 rebase 变基合并。
...
注意
    ...
    所以，一般在开发周期结束不需要再依赖 feature1 分支的场合，才变基合并 master 分支。
...
### 【推荐策略】功能分支合并到主干分支的操作流程

适用于主干分支和开发分支并行的工作流，参见章节 [git工作流： 功能分支工作流]。

简单来说

        功能分支先合并主干分支，应该用分叉合并，如果功能分支的提交记录的 hash 值变更不会引起混乱（远程多分支变更导致其他人的混乱，缺陷管理对应不到提交记录），才可以使用变基合并做拉直。

        变基合并见章节 [变基合并有远程分支要先合并到自身]。

        然后主干分支合并功能分支，用分叉方式合并到主干分支。详见章节 [功能分支合并到主干分支的操作流程]章节
...
·主干分支修复bug，单独拉分支 hostfix，改完后使用变基合并的方式合入主干分支，主干分支的节点延续到了 e。参见章节 [变基合并有远程分支要先合并到自身]。
