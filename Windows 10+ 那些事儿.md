...
一般来说，显示器或者电视机例如会有“HDMI黑色水平”这样的设置选项，其实就是两种不同的明度色阶（16-235）和（0-255）。
    ...
    其中 低/Limited Range 对应显卡输出动态范围有限

    正常/Full Range 对应显卡输出动态范围完全
...
一旦进入 HDR 模式，显示器的亮度控制会自动拉至最高，显示器里设置开启的睿动光感、环境光、低蓝光模式、SmartImage 等功能均被关闭，黑暗房间中的屏幕会像个大光球，为了你的眼睛健康，显示器后面最好有个背景灯给个暗光。
