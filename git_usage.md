...
rebase 通过更新自己分支提交记录的 hash 值的方式，避开制造分叉点获取拉直效果，可能要解决不少合并冲突
...
详细探讨见下面章节 [变基合并主干分支这种 “多人共享分支” 要先合并到自身]。

##### 变基合并主干分支这种 “多人共享分支” 要先合并到自身

    NOTE: 变更主干分支这种 “多人共享分支” 的 hash 值是大忌，会给所有之前拉取过主干分支的人带来混乱。

对主干分支这种不能变更提交记录hash值的多人共享分支来说，首选分叉合并。

如果不想被 rebase 修改接续部分提交记录的 hash 值，也应该选分叉合并
    ...
    功能分支更新合并主干分支，如果用 rebase 合入主干分支的内容，功能分支的提交记录的 hash 值就全变了，总会带来一些混乱。所以很多情况下，项目组不喜欢用 rebase，往往选择用 merge 分叉合入主干的内容。

    先把目标分支 rebase 合并到自己分支，这样变更的是自己分支提交记录的 hash 值，然后再合并到目标分支，这样就变成快进合并了，对目标分支无任何影响。

对 hotfix 修复主干 bug 这种情况，需要拉直合并，问题在于如果期间有人已经推送过，则无法快进合并进去了，那么就需要 rebase 强行拉直，可是这时也无法直接做 rebase 合并了

    # 这样会变更主干分支的提交记录的 hash 值，推送会报错，强推 push -f 会更乱
    git rebase master feature1

所以，要避免变更主干分支现有提交记录的 hash 值，需要用两步实现，下面举例说明：
...
这样做了你推送远程时也会报错，强行推送会更乱，参见章节 [遇到提示 push -f 的时候多想想]。
