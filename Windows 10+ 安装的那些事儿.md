...
如果在游戏设置里开启 HDR，在开始游戏后，显示器会自动切换回标准模式以显示 HRD 内容，原开启的睿动光感、环境光、低蓝光模式、SmartImage 等功能均被关闭，屏幕会进入高亮度状态，打完游戏需要手工调整显示器，把各功能再打开。
...
### 安装双系统
    ...
    还是无法保护 /boot 分区，在操作系统引导阶段对木马监控程序的防护差，建议开启 Secure Boot 功能。

一个硬盘划分多个分区，可以安装多个操作系统：

    建议先安装 Windows 10，再安装 Linux，这样 grub 引导加载器就能直接扫描到 Windows 10 系统，并将其添加到启动界面中。这样开机时会出现 grub 菜单，用户可以手工选择启动到哪个分区的操作系统。

使用双硬盘，隔离性比一个硬盘用多个分区安装多个操作系统的隔离性要好，且不涉及 gurb 引导菜单

    pc 中每次只安装一个硬盘分别安装 Windows 和 Linux，然后两块硬盘都安装到 pc 中。

    启动时主板一般会默认找第一块硬盘上的操作系统直接启动。在开机时按 F12 调出主板的启动菜单，它会列出所有硬盘上找到的可启动的项目，手工指定其它硬盘上的操作系统即可。

    如果可以在冷机启动时先进入 BIOS 设置屏蔽一个硬盘，可以更好的实现使用隔离性。

必须禁用“快速引导”/“快速启动”FAST Boot功能
    ...
    如果前一次使用计算机时用的是 Windows，则关机后再次开机启动到 Linux，因为硬件的状态不是冷启动的重置状态，大概率文件系统会不稳定。
