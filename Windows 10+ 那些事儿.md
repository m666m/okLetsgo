...
另外游戏帧数不足 60 帧的时候有的游戏引擎会用动态模糊来增加流畅感，详见 [游戏不跟手 --- 延迟]。

2、锐化

如果感觉你的游戏画面有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高还是糊，游戏设置一般都有个 “锐化” 选项，拉高可以改善大部分这种画质糊的问题：
...
分析下根源，以前光栅游戏时代，一般是调整 LOD 对应画质细节，设置拉的越高越清晰。4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器分辨率 4K 的优化技术、DLSS 等等歪招全来了，如果你把细节设置拉满画面还是糊，大致原因有以下两种：
    ...
    4K 显示器流行以来发展的 TAA 反锯齿: TAA 以前后帧动态模糊的方式解决物体边缘的锯齿现象。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，TAA 的特点就是不够清晰锐利。解决方法是画质选项里调高锐化，或把显示器从 1080p 升级到 4K 分辨率甚至 fps 帧率更高都可以大大缓解这个现象。实在不行，可以尝试更换反锯齿算法为传统的 8xSSAA，或者切换到 DLSS4 拉高锐化数值。

    光线追踪游戏的材质糊：显示中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在游戏里，只能发射更多光线或者用算法降噪。前者需要庞大的算力，4090 显卡在这面前也只能说是“小小的也很可爱”。而后者，是在光线不足条件下的降噪，这样在抹除了噪点的同时，也抹除了画面细节，这是光线追踪游戏不得不面对的现实。目前折衷的办法是加强漫反射，让用户接受材质糊一点，但光影真实感强。另外调高锐化数值使凹凸细节更明显，也会在观感上改善清晰度。

另外屏幕空间反射 (SSR)和后处理滤镜也可能使画面糊，可以尝试把屏幕空间反射 (SSR)拉到最高或关闭，关闭动态模糊、胶片颗粒、电影质感、景深、色差、VRS 等选项。

3、驱动程序级别的覆盖设置方式（用 steam 截图会得到原图）：
...
另外，可以开启 Nvidia DLDSR(深度学习动态超分辨率) 让你的 1080P 显示器秒变 4K：
...
光线追踪的某些游戏，对光线直射和背光混杂的视角变化处理不好，亮处很亮而暗部画面很黑，转头只看暗处又会慢慢变亮，使用自动曝光的游戏引擎根据视野焦点把暗部慢慢给你调亮，其实非常的不合逻辑。这不是什么黑屋出门看强光的HDR过曝效果，这就是游戏的调校和设计问题：

    比如 赛博朋克2077 的白天场景各种物体的暗部动态死黑，在阳光部分直射的大街上来回转头切换下视角就有想打手电筒的感觉。这也是为什么 2077 玩家都喜欢秀夜景的原因，因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。

    好的例子是 Doom:The dark ages，光线追踪全局光照（RTGI）调校的好，高光给人冲击时暗部也有足够的亮度能看清细节，明暗过渡自然，转换视角时眼睛完全不累。另外没光的山洞里也会细心的放置光源，死黑很少见，不需要打手电。
...
调显示器亮度对比度的方式不适合现代游戏
    ...
    而且，如果开启 HDR 显示器会自动切换到最高亮度，全靠游戏或视频的亮度数据来动态变化明暗。
...
光线追踪和路径追踪
    ...
    N 卡 4080 时代宣传的路径追踪，逐渐的在抛弃光栅化渲染，更多的使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 抛弃光栅化渲染完全使用光线追踪，为了降低显卡负担，各种场景中绝大部分材质的粗糙度设置都是不反射，动态光源的使用非常克制，很少放置玻璃和镜子，镜面效果只见于水面和地面的水洼。各种降低视觉效果换来的只是画面整体的光影真实感更强一些，功耗近 400 瓦的 5080 显卡开路径追踪都跑不到 60 帧/秒。估计想看惊艳的光影效果还得再过2代。
