...
光线追踪的游戏为什么会糊：
    ...
    光线不足时，除非画面完全过曝，否则 “散粒噪声” 会一直存在。在相机上，尚且可以用更大的光圈和更好的CMOS来缓解这个问题，但是在游戏里，只能发射更多光线或者用算法降噪

        前者需要庞大的算力，4090在这面前也只能说是“小小的也很可爱”。而后者，因为光线不足，而且为了消除噪点，不得不做大力的降噪，抹除了噪点的同时，也抹除了大量画面细节。

        目前折衷的办法就是加强漫反射的效果了，让用户接受材质糊一点，但真实感强。

    TAA:以模糊的方式解决了物体边缘的锯齿。所有凹凸表面的边缘包括光线穿过的迷雾甚至不同颜色的相接边缘等都会被软化，整体画面观感就是糊一点。不过，调高画面设置的锐化选项，或把显示器从 1080p 升级到 4K 甚至 fps 帧率更高都可以大大缓解这个现象。实在不行可以尝试更换反锯齿算法为传统的 8xSSAA 等来获得更清晰明亮的画面。
