...
为什么要这样操作呢？ windows 10安装的时候踩了个坑：
    ...
    我在主板BIOS设置中启动模式选项（Windows 10 Features）选择“windows 10”，“CSM Support”选项选择disable后下面的三个选项自动隐藏了，我以为都是自动UEFI了，其实技嘉主板只是把选项隐藏了，硬盘模式默认不是UEFI……

    而windows安装程序是根据当前运行的引导方式决定自动安装成哪个分区形式，在BIOS里把存储设备设为UEFI引导才会默认为GPT，在BIOS里设为legacy就会默认用MBR。
