...
tldr：
    ...
    在显卡计算能力还达不到完美的情况下，使用光线追踪就避免不了它的弱点：糊、暗

以前光栅游戏时代，一般是调整 LOD 对应画质细节，设置拉的越高越清晰。

光线追踪流行后设计原则变了，如果你在游戏选项中把细节设置拉满了画面感受还是糊，大致原因就是游戏引擎切换为未来的方向 --- 光线追踪：

    200x 年的游戏技术基本都是光栅化，可以实现光线的反射折射透射，并依靠游戏开发时预烘培实现全局光照，这样已经可以给玩家非常真实的感觉了，贴图细节调高就可以看到非常清晰的画面。

    为了追求三维画面表现更加真实，尤其是反射、折射、透射等，2020年代逐渐改为使用光线追踪技术在光影方面进行实时计算：屏幕上的每个像素投射n多个光线，打到各个物体上，反射后如果达到有颜色的发光点如灯、太阳，则屏幕上的这个像素点就发什么颜色。其计算混合了物体的材质，光源的强弱等多种因素，实际显示效果可以更加真实。

达到 1080p 下可以接受的画面，光线追踪的计算量都超级大

    从 2000 年代就开始探讨，随着人类电子制造技术的进步，直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 才成为了现实，体验见 赛博朋克 2077。如果计算二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降30%+。

因为每个像素点都要投射光线来实现，噪点是光线追踪技术的先天问题：
    ...
    电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在游戏里，只能发射更多光线或者用算法降噪。
        ...
        发射的光线数量越少，则计算量就越小，但是画面会暗，噪点会变多，解决办法就是高斯模糊。

所以，在光线不足条件下的降噪是很现实的解决方案。但是这会导致抹除了噪点的同时，也会抹除画面细节，这是不可避免的现实。

最终，我们得到了充满了各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 前后帧动态模糊的方式解决物体边缘的锯齿现象，也适合处理光线追踪中的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，这是 TAA 的特点。解决方法是画质选项里调高锐化，或把显示器从 1080p 升级到 4K 分辨率甚至画面帧率更高都可以大大缓解这个现象。实在不行，可以尝试更换反锯齿算法为传统的 8xSSAA，或者切换到 DLSS4 拉高锐化数值。

    因为优化光线追踪就要模糊，所以干脆对普通物体材质处理时就加大粗糙度，这是降清晰度的。

    Nvidia DLSS 用 AI 生成画面来减少计算量。从低分辨率生成高分辨率画面，到直接插帧生成新画面，各种歪招全都有。

玩家可用的缓解办法：

    调高锐化数值使凹凸细节更明显，在观感上改善清晰度。

    开启 Nvidia 驱动程序自带的滤镜，直接给画面加亮度、加锐化。

    使用 4K 分辨率，依靠使用高分辨率材质来提高清晰度（如果游戏在开发时没有进行粗糙化处理的话）。
...
光线追踪和路径追踪：
    ...
    N 卡 4080 时代宣传的路径追踪，逐渐的在抛弃光栅化渲染，更多的使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 抛弃光栅化渲染完全使用光线追踪，为了降低显卡负担，各种场景中绝大部分材质的粗糙度设置都是不反射，动态光源的使用非常克制，很少放置玻璃和镜子，镜面效果只见于水面和地面的水洼。各种降低视觉效果，换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 5080 显卡开路径追踪都跑不到 60 帧/秒。
...
很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体暗部出现死黑，只看暗部又会慢慢变亮，这不符合现实世界体验。这不是什么黑屋出门看强光的 HDR 过曝效果，这就是对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：
...
注意，传统的调节显示器亮度对比度的方式，已经不适合现代游戏了：
...
比如 Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，以大大减少反射的计算，场景中的动态光影和阴影特效也用的非常克制，场景内反射远景的物体很少，物体的暗部维持在可以看清。开启 “路径追踪”，可以稍微增加画面整体的亮度，清晰感更强。
