...
实际操作中发现，显卡bios升级后，虽然可以用DP口安装windows 10，但是系统启动并未进入UEFI模式，windows安装程序格式化的硬盘居然不是GPT的。。。
估计nvidia升级了也不是完美兼容，所以windows自动走的兼容模式的UEFI，凑合着装上的系统。

老老实实的使用HDMI口安装windows吧
