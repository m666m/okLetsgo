...
简单来说，如果是开发者本人使用的独立分支需要同步远程库内容，可以简单点操作，使用变基合并即可，整合到一个命令：
...
但是，如果你在做两个功能分支的合并：合入其它分支功能之前从远程更新，或多人共享开发一个分支经常出现合并冲突，则不能直接 git pull，参见章节 [本地分支从远程仓库更新]。

如果你需要把功能分支合并主干分支，参见章节 [功能分支合并到主干分支的操作策略]。
...
如果需要对分支进行各种操作，参见章节 [日常编辑常用] 下面的各个章节，随用随查即可。
...
最后确认没有问题了，推送远程
    ...
    如果看到远程有新增提交，别急着推送了，先更新本地，见章节 [本地分支从远程仓库更新]
...
NOTE: 新建的分支没有对应到远程仓库，无法推送到远程，如何建立关联，参见章节 [本地非空目录，远程裸仓库无本地分支]。
...
merge 默认的操作不一定选择那种合并策略，参见章节 [`git merge` 执行合并的默认策略有二义性]。

详细的关于分支合并的策略选择，参见章节 [分支合并：merge菱形分叉还是rebase拉直]。

### 本地分支从远程仓库更新

本地分支的更新，就是 fetch 远程库到本地的远程库，然后合并到本地分支。这个合并时的过程与你合并本地的两个分支没什么区别。

    本地开发分支从远程更新，一般本地开发分支要保持不给远程分支添加分叉，则执行 `git pull --rebase`，强行给本地变基拉直合并远程。如果没有不许分叉的限制，则执行 `git pull`，让 merge 自动选择即可。

    本地主干分支从远程更新时，一般做分叉合并，参见章节 [功能分支合并到主干分支的操作策略]。

    合并策略的讨论详见章节 [分支合并：merge菱形分叉还是rebase拉直]。

但是，在以下场景就需要酌情选择更新远程时的合并策略了：

    你在做两个分支的合并，合入其它分支功能之前从远程更新

    多人共享开发一个分支经常出现合并冲突

这样就不能直接做 `git pull` 或 `git pull --rebase`，把拉取和合并分开做。先把远程拉取下来看看内容，然后才能明确选择做何种合并。
...
这里的合并做的是当前分支的本地库和本地的远程库进行合并，实际执行的是 `git merge master origin/master`。

酌情选择自己的合并策略，用分叉还是拉直，参见章节 [分支合并：merge菱形分叉还是rebase拉直]。
...
4、如果合并过程中提示有合并冲突需要处理，操作参见章节 [解决合并冲突conflicts] 的示例。
...
两分支合并方法
    ...
    # 不带参数有二义性，参见章节 [`git merge` 执行合并的默认策略有二义性]
    git merge
...
TODO:酌情考虑如下策略：
    ...
    主干分支 master 合入 hotfix， 属于主干分支上的接续，做拉直合并。

    功能分支合并主干分支，最好先精简下提交记录，然后用分叉方式合并到主干分支。

    功能分支更新合入主干分支的更新，应该用分叉合并，如果功能分支的提交记录的 hash 值变更不会引起混乱（远程多分支变更导致其他人的混乱，缺陷管理对应不到提交记录），才使用拉直合并。

    开发人员独立使用的分支本地拉取同步远程时做拉直合并。

    本地主干分支从远程更新时，一般做分叉合并，参见章节 [功能分支合并到主干分支的操作策略]。

    本地分支更新，参见章节 [本地分支从远程仓库更新]。
...
为强制分叉，可以明确指定不做快进合并，用于 `git merge` 默认做快进不合适的场合

    git switch master

    git merge --no-ff -m "分支feature1合入主干分支" feature1

主要用于大的功能分支合并到主干分支：不使用快进合并而用菱形分叉，这样在查看历史提交记录时，方便区分主干和功能分支，不让功能分支的众多提交记录混入主干分支。对功能分支合并主干分支的场景，更新本地主干分支也应该用分叉，参见章节 [功能分支合并到主干分支的操作策略]。
...
所以能做变基合并的前提是：
    ...
    本地开发分支更新，拉取自己的远程分支，需要拉直提交记录，可以做变基合并。这个场景最常用，参见章节 [本地分支从远程仓库更新]。

    一个本地分支做修改，一个远程分支跟多人同步，因为本地分支的 hash 值变更不会影响他人，可以做变基合并，方法见章节 [变基合并远程分支要先合并到自身]。这个场景一般是用 hotfix 分支修复主干分支的缺陷，用变基合并保持提交记录的拉直状态。因为 hotfix 分支在合并到主干分支后就废弃了，所以在合并主干分支内容时用变基合并，变更它的提交记录的 hash 值没有影响。
...
想清楚哪个分支的提交记录 hash 值不许变，哪个分支的可以变，然后才可以做拉直合并。

简单情况：

如果被变基合并的分支是你自己使用的本地分支，只要你的本地提交记录变更不涉及远程分支、缺陷管理等依赖 hash 值的场景，这样做就没有任何问题
...
如果被变基合并的分支是主干分支，在你的分支拉取主干后，有人又推送过新的提交。这时无法做快进合并拉直，但也不能直接做变基合并

    # 妄想直接给主干分支变基合并 feature1 的内容
    git rebase feature1 master

    这样做变基合并，变更主干分支的提交记录的 hash 值，推送远程会报错，强推 push -f 会更乱，缺陷管理系统也无法跟踪主干分支的提交记录了。

所以，要避免变更 master 分支现有提交记录的 hash 值，只能变更 feature1 分支的提交记录的 hash 值。

解决方案如下：

    # 先把 master 分支变基合并到自己分支，这样变更的是自己分支提交记录的 hash 值
    git rebase master feature1

    # 然后再合并到 master 分支，这时变成快进合并了，对 master 分支无任何影响。
    git switch master
    git merge feature1

当然如果 feature1 分支的提交记录 hash 值也不许变更，则无法做变基合并，老老实实做分叉合并。
...
假设分支 feature1 要合并到主干分支里，但是主干分支已经合入了分支 hotfix，提交点已经延续到了 e。这时两分支的 head 位置不一样，执行 `git merge` 会制造菱形分叉
...
而我们的要求是主干分支不出现分叉，拉直合并其它分支，那么只能使用 rebase 方法。

如果直接把 feature1 的内容变基合并到 master
...
1、先切换到分支 feature1，变基合并主干分支的内容，使得 feature1 分支在 master 分支的基础上延续拉直。这时 d、e 点被合并进来，feature1 分支的 f、g 点在后面接续，但 hash 值变更，成为 f'、g'。
...
注意
    ...
    做 rebase 之后，feature1 分支的提交记录 f、g 点的 hash 值全变了。如果推送 feature1 分支会报错提示用 'push -f'。如果无法接受 feature1 分支造成其它共享人员的混乱，或缺陷追踪系统无法对应提交记录，就不要使用变基合并了。所以一般在开发周期结束不需要再依赖 feature1 分支的场合，才变基合并 master 分支。

    如果你的分支管理依赖这个 hash 值，那么就不要用 rebase，这种情况下只能使用 merge，在合并后 feature 分支会出现菱形。

这样做的目的是：在分支 feature1 上做 rebase，变更的是自己的提交记录的 hash 值，不会变更合并进来的主干分支的 d、e 点的 hash 值。即便需要处理合并冲突，也都是更新分支 feature1 的提交记录，主干分支的提交记录 d、e 的 hash 值保持不变。
...
因为  f、g 已经接续在 d、e 的后面了，master 分支这时落后于 feature 分支了，可以做快进合并：因为 master 分支的 head 位于分叉点 c，实质二者在一条线上，所以 master 主干分支合入 feature 分支时，git 会做的是快进合并，执行 `git merge` 或 `git rebase` 效果相同，这种情况下不会修改任何提交记录的hash值。
...
如果开发分支本身就是用拉直策略延续的，并不需要使用合并专用分支，参见章节 [功能分支合并到主干分支的操作策略]。

开发分支在合并到主干分支前进行拉直，适用于开发分支使用完即废弃不再追踪的场景

    如果缺陷管理系统对应开发分支的提交记录，则拉直后bug清单会失去对应效果，这样就不能做拉直。

问题：两分支合并，因为开发分支的 commit 点的重整比较繁琐，而且容易搞乱，很多人都因此放弃整理，直接合入。如果不做精简，太多的 commit 点使得分支形态不够简洁，而且一般开发分支都是各种分叉的形态
    ...
    直接合入主干会使主干不够简洁，而我们需要使开发分支拉直后合入
...
·开发分支拉取自主干分支的节点 c，在此基础上延续，采取拉直的合并策略：各开发人员本地的开发分支从远程拉取，规定只能使用 `git pull --rebase` 的方式，这样变更的只是开发人员本地的提交记录的 hash 值，保持远程的提交记录 hash 值不变，以此来保持开发分支是拉直延续的。参见章节 [本地分支从远程仓库更新]。

    feature1分支 a---b---c
                          \
                            ---f---g  分支feature1 自己的提交记录

·主干分支修复bug，单独分支 hostfix，改完后使用变基合并的方式合入主干分支，主干分支的节点延续到了 e。参见章节 [变基合并远程分支要先合并到自身]。

                            ---d---e  分支 hotfix
                        /
    master主干  a---b---c

·开发分支在开发中不断更新合入主干分支的内容，使用 `git merge --no-ff` 强制分叉方式合并主干分支的内容，一般都是 hotfix。
...
·开发分支告一段落，使用 `git merge --ff-only` 合入到主干分支，这会是快进合并，不会变更两分支的任何提交记录

最终的形态，两分支都只是一个菱形的分叉，而且便于区分各分支
...
一般来说，只要不是同文件同行的修改，git 会根据你的合并策略选择分叉或拉直自动接续提交点。只要你运行 `git pull` 或 `git pull --rebase` 命令，会自动把最新的远程合并到你的本地，提示信息会告诉你合并的内容，然后你可以继续推送。

但是git无法完全解决合并冲突，有时候出现需要手工解决的情况：比如，文件有一行内容 ‘123456'，你修改为 '123'，别人修改为 '456'，谁先提交无所谓，后面的人在推送或拉取的时候就会被提示处理该合并冲突。

最差的情况是：你把已经推送远程库的代码，重新rebase了本地分支，再次推送就报错了，见章节 [拉shi往回缩：rebase本地分支以后报错了]。

所以，对多人共享开发经常出现合并冲突的场合，更新分支应该先 `git fetch`，详见章节 [本地分支从远程仓库更新]。

本章下面的两个示例，都是依据此操作流程处理过程中，发现合并冲突，然后选择合并策略后进行处理。

两个分支合并，也会出现合并冲突：

    本地分支 A 合入 本地分支 B，二者对同一文件的同一行的都进行了修改，则出现冲突。

    你在功能分支合并主干分支时，没有先更新本地的主干分支，而主干分支的远程已经有新的提交了，而你把功能分支合并到本地的这个旧的主干分支，在推送到远程的主干分支时会发现提示冲突。这种情况你只能先回退，更新主干分支之后再重新做合并。
...
用一系列的 git 操作来解决，可以保留提交记录。

如果这一系列的操作太繁琐反而搞乱了，为保留原有的提交记录，直接用文件操作解决内容问题反而更简洁。
