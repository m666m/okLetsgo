...
所以电竞选手一般选择高回报率鼠标和高刷显示器，搭配高配显卡保证帧率跟的上刷新旅。

例外：把 CS2 和 CSG0 都锁帧到 300帧/s，操作手感上 CS2 就是迟滞，这是游戏引擎的设计问题，它现在成了电竞游戏了，也不敢调整了，就这样了。

2、开启垂直同步选项会轻微增加渲染延迟

鼠标对准屏幕中心的某个物体，保持对准该物体横向平移跑动，或纵向前后跑动，如果画面中央出现轻微的水波纹或撕裂感，就说明需要开垂直同步，这样会把游戏的帧率锁定为显示器的刷新率，但会导致头晕。

帧数不足时不想开垂直同步的一个缓解办法是使用支持可变刷新率 VRR 的显示器如支持 Nvidia G-sync 标准的显示器，在低帧率时候有用，帧数超过显示器刷新率就没什么作用了，还是得开垂直同步。

我玩射击游戏，关垂直同步才不头晕

    建议除非遇到画面撕裂或抖动的问题，再开垂直同步，也可选择开启三重缓冲（依赖游戏支持）缓解头晕，但是偶尔会卡一下。
...
DLSS 等技术针对游戏原生输出的画面，显卡重新计算甚至用帧生成插入 AI 帧，都会增加渲染延迟，比如赛博朋克2077，开启后延迟会大大增加。

但是

    开 DLSS 帧生成 + Nvidia Reflex 后，Doom:The Dark Ages 的延迟反而很神奇的下降了，而且不需要开垂直同步锁帧。

所以对光线追踪+路径追踪的游戏，最优方案是组合起来用：
