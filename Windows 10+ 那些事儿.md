...
如果感觉你的游戏中画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节拉到最高也没有效果，尤其是TAA抗锯齿会糊的略重一点。在游戏设置里找显示画面的设置页，把“锐化”相关选项拉高再看看，如果锐化数值太大会让亮处噪点明显，树皮的纹理变得非常不真实等，酌情调整。

如果启用了 DLSS，可能还有 DLSS 的锐化选项。另外，DLSS 的档位尽量选择“质量”或“DLAA”，比“性能”档更清晰。还可以试试渲染倍率到100%，看看是否可以解决糊的问题。

比如 Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，总给人画面比较糊的感觉。画面设置中的“锐化”拉到 2~3 清晰感就跟之前的 Doom：Eternal 差不多了，跟过场动画的细节类似。
...
4、显示器从 1080P 换成 4K，画质也能有明显提升。
    ...
    注意有些游戏 2k 分辨率拉满设置里的抗锯齿选项，实际画面还不如 4K 显示器开 DLSS 性能档，没有了抗锯齿也不影响画面体验帧数还高。
...
光线追踪的某些游戏，对光线直射和背光混杂的视角变化处理不好，亮处很亮而暗部画面很黑，除非转头只看暗处才能看清，在开启 HDR 时更明显，这个不是什么黑屋出门看强光的人眼效果，这个是光照曲线有问题，根源在于游戏的设计和调校。

    不好的例子是2077的白天场景物体的暗部动态死黑，在阳光直射部分的街上或楼房转头就想打手电筒的感觉。

    好的例子是 Doom:The dark ages，光线追踪全局光照（RTGI）调校的好明暗过渡自然，高光给人冲击时暗部也有足够的亮度能看清细节，没光的山洞里也会细心的放置光源，死黑很少见，不需要打手电。
...
手调显示器亮度对比度的方式不适合现代游戏
    ...
    而且，如果开启 HDR 显示器会自动最高亮度，全靠游戏或视频每帧画面的亮度信息动态变化明暗。

2、游戏内设置，如果支持就把光线追踪的 “路径追踪” 选项打开。这样可以利用光线的多次反射，即使物体是有粗糙度的材质也会稍微变亮。另外，火焰、灯光等自发光材质也能稍微照亮周边，这些都可以增加一点场景中物体的暗部亮度。

比如 Doom:Then Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，这是因为粗糙材质可以大大减少反射的计算，场景中的动态光影和阴影特效也用的非常克制，场景内反射远景的物体很少，物体的暗部维持在可以看清。试试开启 “路径追踪”，画面整体的亮度增加，清晰感更强。

路径追踪

    计算机图形学算法中，路径追踪是光线追踪的一个实现方式，而现实游戏届的宣传受显卡厂商影响，光线追踪和路径追踪跟光栅渲染划代了。

    N 卡 2080 时代宣传的光线追踪，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。比如 Doom:Eternal 开启光线追踪后的 “邪教徒基地关卡”，各种不锈钢材质都在反射周边的灯光甚至 slayer 角色模型，城市场景里也敢于布设玻璃和镜子，二者组合的使用很节省显卡的硬件资源。

    N 卡 4080 时代宣传的路径追踪，就是完全抛弃光栅化渲染，完全使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:Then Dark Ages 不敢使用不锈钢材质，各种场景中绝大部分材质的粗糙度设置都是不反射，镜面效果只见于水面和地面的水洼，不敢放置玻璃和镜子。
