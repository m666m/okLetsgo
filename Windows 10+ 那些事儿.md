...
2、HDR 技术用于家庭影院很好，但不太适合做电脑显示器
    ...
    阳光照耀的自然光环境，亮度在几万 nit，HDR 技术的目标是还原这个自然光照的效果...

    因为人一般都是在室内相对偏暗的环境下用电脑的，27 英寸 HDR1000 显示器的亮度大概相当于近距离面对 60w 的白炽灯，会感到晃眼

一旦进入 HDR 模式，显示器的亮度控制会自动拉至最高，显示器里设置开启的睿动光感、环境光、低蓝光模式、SmartImage 等动态对比度、自动亮度调节等可能与HDR产生冲突的功能均被关闭，黑暗房间中的屏幕会像个大光球，为了你的眼睛健康，显示器后面最好有个背景灯打泛光，显示器距离人眼的距离最好拉远到 1 米之外。这样的用法下，32 寸 4k hdr 显示器比较合适，27 寸的嫌小了，电脑桌也不能太窄。
...
开启方式：Nvidia 控制面板，打开 “RTX 视频增强” 选项：包含 “RTX视频超分辨率技术(RTX Video Super Resolution)”和 HDR。这样浏览器在 全屏 播放网络视频时可以进入 HDR 状态并增强画质：
    ...
    模拟成 HDR 就是把不是 HDR 的视频展现成 HDR 的高光效果，在你的 HDR 显示器上会呈现 HDR 效果
...
显示器开启 hdr 模式后，进入游戏后会自动检测到，跟随调整颜色。或者在 doom 的设置中选择开启 hdr，则在进入游戏后会自动设置显示器开启 hdr 模式。

解决晕 3D 最好关闭垂直同步，这个技术锁帧为你的屏幕刷新率数值会导致画面延迟，开启三重缓冲能缓解但实际效果未必。原因可能在于手的动作和屏幕的显示有延迟且随帧率波动，fps 射击游戏频繁快速的转身和视角变换导致大脑对手运动的感知和对视觉的感知不一致，进而导致头晕
...
4K 分辨率流行后设计原则变了，渲染用 1K 材质然后放大到 4K 的优化技术、DLSS 等等歪招全来了。

如果感觉你的游戏中画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节拉到最高也没有效果。在游戏设置里找显示画面的设置页，把“锐化”相关选项拉高再看看，如果启用了 DLSS，可能还有 DLSS 的锐化选项。另外，DLSS 的档位尽量选择“质量”或“DLAA”，比“性能”档更清晰。

    Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，总给人画面比较糊的感觉。画面设置中的“锐化”拉到 2~3 清晰感就跟之前的 Doom：Eternal 差不多了，跟过场动画的细节类似。
...
另外，开启 Nvidia DLDSR(深度学习动态超分辨率) 让你的1080P显示器秒变4K
    ...
    这个功能是显卡驱动级别的功能，不需要游戏支持

    但游戏需要支持 4k 显示分辨率

DLDSR 能够以超出你的显示器的更高、更细腻的分辨率渲染游戏，然后把画面智能地缩放至显示器的原生分辨率，在任意屏幕上为玩家提供 4K (3840x2160) 画质的图形效果

    DSR（虚拟超级分辨率）功能是 Nvidia 在 2014 年推出的，现在升级了叫 DLDSR(深度学习动态超分辨率)，号称效果更好 DLDSR 2.25x 相当于 DSR 4x。
...
效果
    ...
    在有 DLSS 的游戏里，以 4K 分辨率开启 DLSS 在保证目标帧数超过60帧的情况下，能让 1080P 显示器下游戏的显示效果获得非常巨大的清晰度提升。

    如果开 DLDSR + DLSS 有噪点降低锐化也不管用，则换成 DLDSR + TAA 试试，大型 3A 游戏一般都可选择 TAA。
...
光线追踪的某些游戏，对光线直射和背光混杂的视角变化处理不好，亮处很亮而暗部画面很黑，除非转头只看暗处才能看清，在开启 HDR 时更明显，这个不是什么黑屋出门看强光的人眼效果，这个是光照曲线有问题。问题的根源在于游戏的设计和调校。不好的例子是2077的白天场景的暗部动态死黑，好的例子是 Doom:The dark ages，光线追踪全局光照（RTGI）调校的好明暗过渡自然，高光给人冲击时暗部也有足够的亮度能看清细节，没有 2077 那种白天在街上转头却想打手电筒的情况。这也是为什么 2077 玩家都喜欢秀夜景的原因，因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。

1、如果是游戏暗处太黑（含光线追踪），首先在游戏内设置调整 伽马值，适当放大，达到暗处稍亮而亮处不发白即可，伽马值太高整个屏幕都会都发白。

如果是 HDR 显示器，游戏内一般会有个专门的 HDR 参数设置页面，可以调整亮度、色调等细节。

光线追踪的游戏，对比度亮度都是动态的，全局光照的曝光度也随着视角和环境变化，这都跟软件的设置有关，单纯调节显示器的亮度没什么用。另外，如果开启 HDR 显示器会自动最高亮度，全靠游戏或视频的亮度信息动态变化明暗了。

2、游戏内设置，如果支持就把光线追踪的“路径追踪”选项打开。这样有粗糙度的材质会增加反射细节而使物体稍微变亮。另外，火焰、灯光等自发光材质也能稍微照亮周边，这些都可以增加一点场景中物体的暗部亮度。

比如 Doom:Then Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，因为粗糙材质可以大大减少反射的计算，场景中的动态光影和阴影特效也用的非常克制，场景内远景就能反光的物体很少，暗处乌图图的但是可以看清，死黑也很少见。开启“路径追踪”后，能感觉到整体的亮度稍微增加了，清晰感更强。
