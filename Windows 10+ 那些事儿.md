...
所以，在光线不足条件下的降噪是很现实的解决方案，高斯模糊就来了。模糊处理抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。

最终，我们得到了充满了各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，这就是 TAA 的特点。

    因为优化光线追踪就要模糊，所以干脆对普通物体材质处理时就加大粗糙度，这是降清晰度的，高清材质也要糊一下才发布。

    Nvidia DLSS 用 AI 生成画面来减少计算量。从低分辨率生成高分辨率画面，到直接插帧生成新画面，各种歪招全都有。

也就是说，跟之前的光栅游戏相比，清晰度下降，反射效果提升，帧率爆降，这是2025年使用光线追踪游戏的现实，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 全都如此。
