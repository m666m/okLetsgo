...
现代的图像数码处理，除了增加色彩和光影细节，还在提升像素点的精细程度。对数码相机来说，就意味着使用大的CCD采样，跟 1839 年的照片比起来，人类手绘的图画逼真程度已经无法超越现代的照片了。

视频游戏追求逼真的感觉，也是在光影和细致程度上下功夫。

如果只有简单的光线照射，预烘培和游戏中的光影计算都尽量简化，人物渲染效果就会表现出卡通化，比如 3D 网游等对显卡要求很低的游戏，即使画面很清晰但画面风格只能做卡通化的。
...
这里就引出了题目中的疑问：
    ...
    如果你在游戏选项中把画质细节拉满了还是觉得画面糊，亮度拉高到暗部蒙了层白纱感但还是黑，大致原因就是游戏引擎切换为使用光线追踪了，底层原因是 TAA 这个模糊技术。
...
从 2007 年代英特尔就尝试用多核心通用图形处理器（GPGPU）代号 Larrabee 来实现光追游戏 <https://www.gameres.com/853845.html>，甚至云游戏串流运行。但直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才真正成为了现实（光追游戏体验见 赛博朋克2077）。如果计算光线的二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降 30%+。

    至于为何 Nvidia 给显卡非常少的硬件光追单元 RT Core，就不足为外人道也了。比如 5090 显卡有 21760 个 CUDA core，仅 170 个 RT core，专为深度学习而设计的 Tensor Core 都有 680 个。
