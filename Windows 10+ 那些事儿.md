...
1、游戏设置里把动态模糊、胶片颗粒、电影质感、景深、VRS 等选项关掉。

2、锐化

如果感觉你的游戏画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高，抗锯齿选项也拉到最高，还是糊，调整下锐化级别可能会大大改善画质：
...
以前的光栅游戏，一般是调整 LOD 对应画质细节。4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器分辨率 4K 的优化技术、DLSS 等等歪招全来了。细节设置拉满还是会糊的主要原因有两个：

    4K 显示器流行以来发展的 TAA 反锯齿: TAA 以模糊的方式解决物体边缘的锯齿问题。所有凹凸表面的边缘包括光线穿过的迷雾甚至不同颜色的相接边缘等都会被软化，整体画面观感就是糊一点。解决方法就是调高锐化，或把显示器从 1080p 升级到 4K 甚至 fps 帧率更高都可以大大缓解这个现象。实在不行可以尝试更换反锯齿算法为传统的 8xSSAA 等来获得更清晰明亮的画面。

    光线追踪的游戏为什么会糊： “散粒噪声” 会一直存在。在相机上，尚且可以用更大的光圈和更好的CMOS来缓解这个问题，但是在游戏里，只能发射更多光线或者用算法降噪。前者需要庞大的算力，4090在这面前也只能说是“小小的也很可爱”。而后者，因为光线不足，而且为了消除噪点，不得不做大力的降噪，抹除了噪点的同时，也抹除了大量画面细节。目前折衷的办法就是加强漫反射，让用户接受材质糊一点，但真实感强。调高锐化数值也会改善清晰度。
