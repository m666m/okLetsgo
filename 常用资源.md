...
设置：打开Windows安全中心->设备安全性->内核隔离，手动开启核心隔离和内存完整性等选项 <https://support.microsoft.com/zh-cn/windows/afa11526-de57-b1c5-599f-3a4c6a61c5e2> <https://go.microsoft.com/fwlink/?linkid=866348>

### 默认键盘设置为英文

中文windows10默认键盘是中文，输入法是微软拼音。而炒股软件默认应该用英文输入，文本软件默认用中文输入，各有不同，来回切换太麻烦了，特别是有些游戏，一按shift就切出来中文输入法。这种情况下，需要添加英文键盘，使用热键是“Win+空格”切换不同的键盘，同时输入法也跟着变了，而且不同的窗口还需要记住不同的输入法。

搜索：语言，选择英语，添加确认，注意取消选择设置为默认的界面语言。

搜索：keyboard，打开“高级键盘设置”

    替换默认输入法，选择“英语”，而不是微软拼音输入法。

    勾选“允许我为每个应用窗口使用不同的输入法”

    注意这两种键盘的切换热键是“Win+空格”。

    原“Ctrl+空格”切换中英文被微软拼音输入法使用了，更简单的是按shift切换中英文。

    取消勾选“使用桌面语言栏”

    选择语言栏选项

        勾选“在任务栏中显示其他语言栏图标”，方便切换中英文键盘。
