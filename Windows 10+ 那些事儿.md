...
4、开启 Nvidia DSR（虚拟超级分辨率） 让你的1080P显示器秒变4K

在 Nvidia 控制面板的 3D 设置中找到选项 DSR 因数，可以调整渲染时放大的倍数，1080P显示器选择4x则分辨率变成 4K (3840x2160)。DSR 4.00x就是1颗显示器像素显示的内容是显卡用4颗像素模拟计算后得出的，开启它可以明显提升电脑清晰度。

因此 DSR 能够以更高、更细腻的分辨率渲染游戏，然后把画面智能地缩放至显示器的原生分辨率，在任意屏幕上为玩家提供 4K (3840x2160) 画质的图形效果。

验证：

    Windows 桌面进分辨率设置，分辨率选项已经有 3840X2160 的选项了，说明 DSR 已经成功开启，同样的游戏中也有这个选项了。

    在有DLSS的游戏里，以4K分辨率开启DLSS在保证目标帧数超过60帧的情况下，获得非常巨大的清晰度提升。

5、同理，还可以利用 Nvidia RTX视频增强（Super Resolution），获得视频网站和直播平台的清晰度提升，只是目前支持该技术的应用软件太少：

    edge 浏览器里，设置 --> “系统和性能”，勾选 “在可用时使用图形加速” 和 “增强 Microsoft Edge 中的视频”。Nvidia 控制面板，打开 “RTX 视频增强” 选项：包含 “RTX视频超分辨率技术(RTX Video Super Resolution)”和 HDR。这样在浏览器播放网络视频时可以进入 HDR 状态并增强画质。
