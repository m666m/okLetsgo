...
视频游戏追求逼真的感觉，也是在光影和细致程度上下功夫。
    ...
    光线不足就会降低真实感。如果预烘培和游戏中的光影计算都尽量简化，那么游戏角色的渲染效果就会表现出卡通化，比如手机游戏、3D 网游等对显卡要求很低的游戏，即使画面非常清晰，但画面风格只能做卡通化的。
...
但是，如果 3D 游戏画面显示技术想再进一步，实现动态效果的照片级真实感，比如像过场动画一样的游戏效果，依赖预烘培的光栅化处理技术就有很大的限制：
...
这里就引出了题目中的疑问：
    ...
    如果你在游戏选项中把画质细节拉满了还是觉得画面糊，亮度拉高到暗部蒙了层白纱感但还是黑，大致原因就是游戏引擎切换为使用光线追踪了，底层原因是都使用了 TAA 这个模糊技术。
...
如果是计算光线路径的多次反弹即所谓“路径追踪”，间接光下的漫反射制造的光影真实感更强，路径追踪的二次反射是在第一次光线路径的反弹点上递归，再计算反弹出来的光线路径的多次反弹，计算量更是爆炸，帧数一般暴降 +30%。
    ...
    至于为何 Nvidia 给显卡非常少的硬件光追单元 RT Core，就不足为外人道也了。比如 5090 显卡有 21760 个 CUDA core，仅 170 个 RT core，专为深度学习设计的 Tensor Core 在这张游戏卡上都有 680 个。
...
所以在光线不足条件下降噪是很现实的解决方案，高斯模糊等各种模糊方案就是解决王道。但是，模糊处理在抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。

最终，我们得到了充满各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。而且角色的毛发会有锯齿，跑动中看到的草坪会从清晰变模糊，这不是你的错觉，这是 TAA 的特点。无奈的是，从 UE5 引擎的各种游戏，到 Doom:The Dark Ages 全都切换到使用 TAA 技术了。Nvidia 推出的 DLSS 属于对 TAA 的改良，清晰度经过 AI 修正反而能降低模糊程度。
...
总体来说，现在的光线追踪游戏，有点类似数码相机替代胶片相机的过程。在刚开始发展的时代，数码相机清晰度不如传统的胶片相机，但是在多次迭代后清晰度达标了，几年功夫就实现了对传统相机的替代。
...
Nvidia 在显卡计算能力不足时强推光线追踪，导致了到 2025 年光线追踪游戏的画面效果还不如 10 年前的游戏：
    ...
    比如，优化最好的，基本看不到死黑的 Doom:The Dark Ages，号称完全使用光线追踪（其实是部分烘焙再加强制硬件光追），计算量超级大：为了减少反射的计算量，除了主角，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于部分水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。相比之前的 DOOM2016/DOOM ETERNAL， 各种减少视觉元素，居然只是画面整体的光影真实感更强一些，并无任何惊艳。功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。<https://doomwiki.org/wiki/Tiago_Sousa>
...
彻底解决办法：
    ...
    想看清晰逼真的画面还得再等几代显卡，光线追踪的渲染从 DLSS 发展到神经渲染（AI 技术神经网络机器学习），最终实现照片级效果，这次，全场景的光都是实时动态的。
