...
光影对人类立体视觉的影响，以素描为例进行理解。在纸上画几条线段是二维图像，如果在背景上浓淡相宜的扫几笔，就制造出了光影，就有了真实的立体感，继续添加色彩和光影细节就会让人感觉画面更逼真。

16世纪文艺复兴时期的绘画技术从尊重几何的角度重现光影，为了提高绘画的准确性和细节表现，甚至有了成像暗箱这种利用小孔成像来捕捉自然光线和阴影变化的辅助工具。

对传统胶片相机来说，底片要经过长时间的曝光，最终相片的颗粒感才会变少。现代的图像数码处理，除了增加色彩和光影细节，还在提升像素点的精细程度。对数码相机来说，就意味着使用大的CCD采样。跟 1839 年的照片比起来，人类手绘的图画逼真程度已经无法超越现代的照片了。

视频游戏追求逼真的感觉，也是在光影和细致程度上下功夫。

光线不足的会降低真实感，比如预烘培和游戏中的光影计算都尽量简化，那么人物渲染效果就会表现出卡通化，比如手机游戏、3D 网游等对显卡要求很低的游戏，即使画面非常清晰，但画面风格只能做卡通化的。

2020 年代之前的游戏

    都是光栅化技术来渲染画面，可以实现光线的反射折射阴影。依靠游戏开发时在场景中预烘培实现全局光照，美工上多下功夫就可以给玩家照片级的真实感了，贴图细节调高就可以看到非常清晰的画面，场景中预烘培的光线充足各处都不会太黑。比如 2007年的 《孤岛危机》、2016年的《神秘海域四》、2020年的《美国末日2》、2018年的《荒野大镖客2》，场景美轮美奂。

    在游戏设置中贴图细节选择越高、LOD 拉的越高，画面就越清晰。

但是，3D 游戏画面显示技术想再进一步，实现动态效果的照片级真实感，比如像过场动画一样的游戏效果，光栅化处理却做不到：

    比如光源无法变化，阳光射入的室内通过预烘培的光照能给你照片级真实感，但是太阳这个光源不能移动，场景也不能被破坏和移动，可交互性很差。自由运动的游戏人物和角色，在光线变化时很难跟场景中光线的色调协调，你会看到游戏角色跑进黑洞但仍然发白的现象。光栅化也无法实现汽车表面这种曲面的反射和镜面对照。另外，对规模越来越大的开放世界场景的 RPG 游戏来说，预烘培生成的光照信息，最多可以高达几十TB，这已经无法部署到玩家硬盘了。
...
总体来说，现在的光线追踪游戏，有点类似数码相机替代胶片相机。数码相机在刚开始发展的时代，清晰度不如传统相机，但是在多次迭代后清晰度达标了，几年功夫就实现了对传统相机的替代。

Nvidia 在显卡计算能力不足时强推光线追踪，导致了到 2025 年光线追踪游戏的画面效果还不如 10 年前的游戏：

    跟之前的光栅游戏相比，完全光线追踪技术的游戏画面真实感轻微提升，清晰度明显下降，反射效果明显提升，物体暗部死黑太多（预烘焙减少又不舍得多给实时光线），帧率巨幅下降，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 基本都是如此。注：UE5 引擎开发的《寂静岭f》画面效果倒退到了 2000 年代。
...
在开放式场景中的游戏常见背光面死黑、白天屋里很黑的情况，根源在于光线追踪迫于计算量优化不能放置太多光线，这就非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校。为了方便（削减成本），很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体背光的暗部出现死黑，只看暗部又会受环境光照影响慢慢变亮，让你的眼睛非常不舒服。这不是什么黑屋看门口强光的 HDR 过曝效果，这就是游戏对光线直射和非直射光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：

    黑神话·悟空，第二章的山洞场景不是过曝亮瞎眼就是黑乎乎什么都看不到，沙漠场景中明暗同时存在的视角闪爆你的眼球，对比度拉的很高但是全局光照考虑的太少，大白天的阴影里愣是黑得伸手不见五指。
...
相比之下，电影级的离线渲染对光线的计算都是不计工本的，每个像素点发出的光线就非常多，而且是多次发射并计算反弹、甚至反弹点再发射光线进行计算，在 3D 游戏行业的实时渲染是做不到的。
...
游戏内的设置，开启 “路径追踪”，可以增加漫反射，稍微增强画面整体的亮度和明暗表现，而且真实感更强。开启了光线的二次反弹后，非镜面的反光物(有粗糙度的材质)也会投射光线到其他物体，使周围环境稍微变亮或增加阴影。灯条等自发光材质也能稍微照亮周边物体。

    路径追踪会增强间接光照射物体的明暗感觉，真实性更好。判断是否使用了路径追踪，可背向光源看反光面，是否反光到其他物体，那些物体是否改变了亮度和颜色，比如光线直射的桌面，是否照亮了周边的墙面和物体、灯光直射的地面，不被直射的周边墙面也会跟随变亮，而且门框等凹凸物体也会出现阴影轮廓。
...
如果达不到这个标准，或运行时波动太大，就可能出现卡顿的情况：
    ...
    手感卡顿：渲染延迟指的是画面生成到显示器呈现的时间，渲染延迟过大会影响手感，晃动鼠标会感觉到画面里的人物动作跟不上，即不跟手
...
举个例子，你的显示器刷新率为 300hz，锁 30 帧渲染延迟有 1ms，锁 300 帧渲染延迟有 4ms，这两个渲染延迟完全不会影响人的感觉，但是实际体验中受“帧间距”影响很大：

    锁 30帧/s 虽然是 1ms 的渲染延迟，但是“帧间距” 300/30=10ms 太大，会感到画面卡顿

    锁 300 帧的帧间距 300/300=1ms 眼睛看着更流畅，而且其 4ms 的渲染延迟也不影响手感

这种情况下就应该选择锁 300 帧。

有些低端的电视机的响应时间很长，但有的人在这种电视上打游戏感觉不出来滞后感，但如果游戏的每秒帧数从 60 帧降到 20 帧，他就会觉得“卡”，就是因为帧间距的影响。帧间距过大可以用动态模糊骗掉眼睛，从而使画面看起来很流畅；但是延迟的画面却不可以。

帧数过高也可能影响画面观感，因为显卡送到显示器的数据太快也需要缓冲，反而排队了。
...
帧生成的最好效果是原始帧率在60帧/s以上，在帧率不足时开启，效果由玩家自行斟酌。

但是

    开 DLSS 帧生成 + Nvidia Reflex 后，Doom:The Dark Ages 的延迟反而很神奇的下降了，开启三重缓冲也不卡。

所以对光线追踪+路径追踪的游戏，最优方案是全都用：

    开启 DLSS 选择 DLAA 档或质量档

    开启 帧生成

    开启 Nvidia Reflex

这个方案的手感延迟是可以接受的，帧数能覆盖显示器刷新率也不会感到画面卡顿，最神奇的是 DLSS 生成的画面比游戏原生更清晰。

#### 晕 3D 怎么办

1、游戏设置问题

把环境晃动、摄像头晃动等游戏选项都关了，有些游戏战斗时镜头快速变化或视觉模糊来制造压迫感，以让你胜利后有释放压力的快感。但是晃画面是让人必晕的，没啥意思，关掉晃动反而游戏体验更好。
...
帧数不足，但是不想开垂直同步的一个缓解办法是，使用支持可变刷新率 VRR 的显示器如支持 Nvidia G-sync 标准的显示器，在低帧率时候有用，帧数超过显示器刷新率就没什么作用了，遇到画面撕裂还是得开垂直同步。

头晕的原因可能在于相对于手的快速动作跟大脑一致，而屏幕的显示相对帧率波动性延迟，而 fps 射击游戏频繁快速的转身和视角变换，导致大脑对手运动的感知和对视觉的感知不一致，进而导致头晕。
