...
1、文件和目录的组织原则
    ...
    git 是以文本文件的文字差异为基准进行差异比对的，所以目录结构和文件名不要使用操作系统提供的 rm、mv 等命令进行操作，使用 git 自带命令 `git mv`，否则对应的提交记录全丢
...
在实际使用中，会经常出现推送和提交冲突，一般都是本地库跟远程库的提交记录 hash 值对不上导致的。
...
3、能用文件操作解决内容问题，就别用一系列的 git 操作来解决，哪个办法简洁用哪个
    ...
    你的项目关注的是文件内容，而不是源代码管理系统使用的提交点，**不要**拘泥于用提交点来区别文件内容的修改。
...
简略操作才用 pull，让 merge 自动选择，参见章节 [`git merge` 执行合并的默认策略有二义性]。
...
### 常用的主干分支和开发分支并行的工作流的分支合并策略

主干分支和开发分支并行的工作流，参见章节 [git工作流： 功能分支工作流]。
...
·开发分支拉取自主干分支的节点 c，在此基础上延续，采取拉直的策略：各开发人员本地的开发分支从远程拉取，规定只能使用 `git pull --rebase` 的方式，这样变更的只是开发人员本地的提交记录的 hash 值，保持远端的提交记录不变，以此来保持开发分支是拉直延续的。
...
·开发分支在开发中不断更新主干分支，使用 `git merge` 分叉方式合并主干分支的内容，一般都是 hotfix。
...
·开发分支告一段落，使用 `git merge` 合入到主干分支，这会是快进合并，不会变更两分支的任何提交记录
...
TODO:酌情考虑如下策略：
    ...
    本地主干分支从远程更新，应该用做拉直合并

    本地分支更新从远程拉取合并的场景，如果是多人合作的功能分支、主干分支，都要把拉取和合并分开做，参见章节 [本地分支从远程仓库更新]。
...
所以能做变基合并的前提是：
    ...
    本地分支更新，拉取自己的远程分支，需要拉直提交记录，可以做变基合并。这个场景最常用，参见章节 [本地分支从远程仓库更新]。
...
养成好习惯，并不需要使用合并专用分支，参见章节 [常用的主干分支和开发分支并行的工作流的分支合并策略]。

合并前进行拉直，适用于开发分支使用完即废弃不再追踪的场景

    如果缺陷管理系统对应开发分支的提交记录，则拉直后bug清单会失去对应效果，这样就不能使用拉直。

问题：两分支合并，因为开发分支的 commit 点的重整比较繁琐，而且容易搞乱，很多人都因此放弃整理，直接合入。

    如果不整理，太多的commit点使得分支形态不够简洁，一般开发分支都是各种分叉的形态
...
合并冲突是指：在共同节点之后，出现了两种独立的提交(每种可能有多个提交)。两个分支合并，或本地合并远程时，都有可能出现合并冲突，参见章节 [`git merge` 执行合并的默认策略有二义性]。
