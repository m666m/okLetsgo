...
如果感觉你的游戏画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高还是糊。

游戏设置画面有个 “锐化” 选项，拉高可以解决大部分画质糊的问题：
...
比如 Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，总给人画面比较糊的感觉，而把 “锐化” 拉到 3 左右，清晰感就类似过场动画了，跟 Doom：Eternal 的细节水平差不多。当然 doom 系列一贯的风格是大多数材质贴近看是糊的，远景能清楚就行了。

分析下根源，以前光栅游戏时代，一般是调整 LOD 对应画质细节，设置拉的越高越清晰。4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器分辨率 4K 的优化技术、DLSS 等等歪招全来了，出现了细节设置拉满画面还是糊的奇怪现象：

    4K 显示器流行以来发展的 TAA 反锯齿: TAA 以模糊的方式解决物体边缘的锯齿问题。所有凹凸表面的边缘包括光线穿过的迷雾甚至不同颜色的相接边缘等都会被软化，整体画面观感就是糊一点。解决方法就是调高锐化，或把显示器从 1080p 升级到 4K 分辨率甚至 fps 帧率更高都可以大大缓解这个现象。实在不行可以尝试更换反锯齿算法为传统的 8xSSAA，或者切换到 DLSS4 拉高锐化数值。

    光线追踪游戏的材质糊： “散粒噪声” 会一直存在。在相机上，尚且可以用更大的光圈和更好的CMOS来缓解这个问题，但是在游戏里，只能发射更多光线或者用算法降噪。前者需要庞大的算力，4090在这面前也只能说是“小小的也很可爱”。而后者，因为光线不足，而且为了消除噪点，不得不做大力的降噪，抹除了噪点的同时，也抹除了大量画面细节。目前折衷的办法是加强漫反射，让用户接受材质糊一点，但真实感强。另外调高锐化数值也会改善清晰度。

另外屏幕空间反射 (SSR)和后处理滤镜也可能使画面糊，可以尝试把屏幕空间反射 (SSR)拉到最高或关闭，关闭动态模糊、胶片颗粒、电影质感、景深、色差、VRS 等。
