...
通常射击游戏在设计时会针对最常见的使用场景调整设计目标，比如帧率 60帧/s。其在 60hz 显示器上的帧间距是 1ms，一般原生画面的渲染延迟大概在 10 ms 左右，不会影响手感，视觉上也不卡顿
    ...
    手感卡顿：渲染延迟指的是画面生成到显示器呈现的时间，影响手感，鼠标动作了画面能否跟上，即跟不跟手

    画面卡顿：帧间距指的是两帧之间的时间，影响观感，即眼睛看着画面感觉卡不卡顿

举个例子，你的显示器刷新率为 300hz 情况下锁 30 帧渲染延迟有 1ms，锁 300 帧渲染延迟有 4ms，在实际体验上，锁 30帧/s 虽然 1ms 的渲染延迟不影响手感但是帧间距大会感到画面卡顿，而锁 300 帧的帧间距小眼睛看着更流畅，而且 4ms 的渲染延迟是不影响手感的，就应该选择锁 300 帧。有些低端的电视机的响应时间就很长，但有的人在这种电视上打游戏感觉不出来滞后感（也就是“卡”），但如果游戏的每秒帧数从 60 帧降到 20 帧他就会觉得“卡”。帧间距过长可以用动态模糊骗掉眼睛，从而看起来很流畅；但是延迟的画面却不可以。
...
2、开启垂直同步选项会轻微增加渲染延迟
    ...
    鼠标保持对准屏幕中心的某个物体，横向平移跑动，或纵向前后跑动，如果画面中央出现轻微的水波纹或撕裂感，就说明需要开垂直同步

我玩射击游戏，关垂直同步才不头晕

    除非遇到画面撕裂或抖动的问题，再开垂直同步。这个技术锁帧为你的屏幕刷新率数值来解决问题，但是会导致画面稍微延迟，导致头晕。选择开启三重缓冲能缓解头晕，但是偶尔会卡一下。酌情处理吧，一般来说开三重缓冲，快速晃动鼠标时画面跟随晃动没有迟滞感就可以了。

    头晕的原因可能在于相对于手的动作，屏幕的显示有延迟且随帧率波动，fps 射击游戏频繁快速的转身和视角变换导致大脑对手运动的感知和对视觉的感知不一致，进而导致头晕。

    另外把环境晃动、摄像头晃动都关了，这种老式游戏靠战斗时快速变化但是看不清甚至头晕恶心来制造压抑感，然后的胜利给你释放的快感。但是晃画面是让人必晕的，没啥意思，关了反而游戏体验更好。
...
解决办法，如果想凑合下，开启 DLSS4，可以稍微降低对显存的需求，或者在游戏选项中适当降低画质。
...
注意：高功耗显卡的峰值功耗基本比最大功耗多50%+（A 卡的直接翻倍估算），但是毫秒级别波动只有用示波器能抓住，厂家从来不负责功耗超标问题。所以，用 x080 x090 系显卡务必搭配不虚标的大功率电源，有个说法至少要一瓦一块钱判断电源是否靠谱
