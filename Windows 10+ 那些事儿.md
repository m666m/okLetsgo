...
问题是
    ...
    桌面/资源管理器/炒股/游戏/网页浏览器 等软件的默认热键是响应英文的键盘输入，所以仅对文本编辑软件启用中文输入才是最优方案。

    在实际使用中，各个软件只要切换窗口，或者输入个字母就弹出中文候选字对话框，按 shift 键就来回切换中英文，实际输入的过程中，非常非常非常的繁琐。

    在使用 Windows 资源管理器时，无法用按键快速导航到指定开头字母的文件。

    在打 fps 游戏的时候，一按 shift 就切出来中文输入法，再按 asdw 就变成打字到中文输入法的输入栏了。

所以需要再添加一个“英文键盘”，使用它的英文输入法状态，并把这个键盘作为默认状态。两种键盘间切换使用热键“Win+空格”。这样做的好处是，切换键盘的同时中英文输入法也跟着切换了（不需要依赖在中文键盘下微软拼音输入法的 shift 键切换），而且有了多种输入法，不同的窗口还可以选择记住不同的输入法状态，非常方便。打游戏时按 win+空格 切换到英文键盘后，如何按 shift 键都不会冒出来中文输入了。
...
设置->搜索“语言”，“首选语言”
    ...
    现已增加设置对 “非 Unicode 程序的语言” 栏目，把默认的 “英语（美国）” 改为 “中文（中国）”，这样传统的中文软件都能用了。
...
#### 传统的简体中文 Windows（gbk 编码） 程序文字变为乱码

    https://ask.csdn.net/questions/9584924

Win11（22H2+）以前， 默认 ANSI 代码页是CP936（GBK）硬绑定，现在改为了 CP65001（UTF-8）软优先，CP936 注册表键弱化，其实英文 Windows 早就切换到 utf-8 了。

在运行传统的简体中文 Windows（gbk 编码） 程序时文字变为乱码，功能都正常，但是文字得瞎蒙：

按风险由低到高分三级实施：

首选（低风险）：控制面板 → 区域 → 管理 → 更改系统区域设置

    → 勾选「Beta版：使用Unicode UTF-8提供全球语言支持」

        → 不需要勾选启用「非 Unicode 程序的语言」为中文

        支持 Windows 9x时代的程序比如同花顺正常显示，但它安装程序建立的中文目录是 gb2312 的，导致乱码打不开。

    → 重启生效（⚠️注意：可能破坏 .NET Framework 4.7.2 以下版本的 File.WriteAllText）

推荐（稳态）：取消 Beta 选项，设区域为「中文（简体，中国）」

    → 勾选「对基于 Unicode 的程序使用 UTF-8」

    → 不勾选启用「非 Unicode 程序的语言」为中文

    必须重启

为单个软件创建带chcp 936命令的快捷方式临时指定代码页

    cmd /c chcp 936 >nul & start "" "C:\Path\To\Your\App.exe"（将路径替换为实际软件路径）。

兜底（精准）：修改注册表 HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\FontSubstitutes，新增字符串值：
"MS Shell Dlg" = "SimSun"
"MS Shell Dlg 2" = "Microsoft YaHei"
"Tahoma" = "SimSun"

四、工程层：软件级资源级干预（以 VB6 为例）
当系统级配置仍无效时，需深入 PE 资源层。使用 Resource Hacker 修改 EXE 的对话框模板（DIALOGEX）资源：
...
// 修改后（显式指定兼容字体）
FONT 9, "SimSun", 0, 0, 0x1
```

同时，在 VB6 工程属性 → “组件” → 勾选「使用 Unicode API」禁用（确保调用 WideCharToMultiByte(CP_ACP) 而非 CP_UTF8）。
...
### 如何安装 .msix .msixbundle

如果你的 Windows 上有微软软件商店，双击即可自动安装。

.msix 需要在 Power Shell 下用命令的方式安装：

    PS C:\Users\you> add-appxpackage ./文件名

.msixbundle 先用 7zip 解压缩到一个目录，在目录里找 .msix文件，一般叫 AppInstaller_x64.msix，然后在 Power Shell 下安装安装即可。
...
winget
    ...
    微软官方给 Windows 10+ 用的软件包管理工具，就是 apt 命令行操作开源仓库那种形式，最大的优点是支持安装 Microsoft Store 上的应用程序
        ...
        安装包 https://github.com/microsoft/winget-cli/releases
...
### 使用统一写入筛选器（UWF）保护你的磁盘分区

企业版的 Windows 才有这个功能。在开启 UWF 保护的磁盘中，文件并不会直接写入磁盘，而是写入计算机的 RAM 中。我们可以为系统盘开启 UWF，保护系统盘内的文件不被更改；也可以将系统运行时的产生的临时文件写入受 UWF 保护的磁盘分区，进而提升系统运行速度、延长磁盘使用寿命。

    https://zhuanlan.zhihu.com/p/584410603

UWF 用的是命令行操作方式，安装后执行设置都是在命令行使用 `uwfmgr` 来操作。

开启 UWF 保护磁盘分区，需要先给他分配一大块内存缓存写入的内容，重启后这些内容就消失了，实现了对指定磁盘分区的写保护。
...
Bitlocker 不需要输入密码，原理是利用计算机主板上的 TPM 芯片做 boot measure, 只有启动过程未被篡改才可以执行解密过程.

    不需要登录 Microsoft 账户，该账户可以云备份你的紧急恢复密钥

    搞出来问题可以试试 diskgenius 能否恢复出一点数据

    详见章节 [Windows Bitlocker 加密体系](init_a_server thnik)

如果 Bitlock 设置为使用软件加密，硬盘读写速度大概 -30% 的降幅，如果使用 ssd 硬件加密功能，不影响读写速度，但是不够安全(后面详述)。
...
C 盘为操作系统所在盘，一般不建议加密，因为没啥用且使用不便，并且稍微影响系统运行速度。

应该加密你的数据盘，数据盘里可能放有照片或者小电影、或者商业机密文件，这样在电脑送修、硬盘丢弃等场合可以使无关人员无法访问。
...
#### ssd 硬盘不要开启厂商自带的硬件加密功能

这样 Bitlocker 基本给废了。

简单说，ssd 硬盘自带硬件加密功能，微软的 Bitloker 虽然属于软件加密方案，但是会直接调用 ssd 硬盘自带的硬件加密接口。而 ssd 硬盘居然用固件自带的密钥来加密和解密内容，导致用户在 Bitlocker 里认真输入和保存的密码，其实根本没用上。
...
在 WSL 里对 /mnt/c 下的目录连续改名后，目录名虽然还在，但完全无法访问，ls 显示为 d????????? ? ?

根本原因是 NTFS 文件系统的元数据在 WSL 与 Windows 之间出现了不一致，微软至今还未解决此问题。

规避办法：

强制刷新 WSL 的文件系统缓存，在 WSL 终端依次执行：

    # 离开这个目录
    cd /
    # 卸载并重新挂载该 NTFS 盘符（需要 sudo）
    sudo umount /mnt/c
    sudo mount -t drvfs C: /mnt/c -o metadata
    # 再回去看
    cd /mnt/c/Users/mm/ghcode/Agent
    ls -la

如果提示 umount: /mnt/c: target is busy.，可以先用 sudo lsof /mnt/c 查看谁在使用.

或者直接重启 WSL

    wsl --shutdown

然后重新打开 WSL 终端，再进入 Agent 目录检查。很多时候这就能让目录“复活”。

如何避免以后再出现？

在 WSL 里操作 /mnt/c 下的文件时，尽量先确保没有 Windows 程序占用它们。

重要的重命名或移动，可以直接在 Windows 资源管理器里做，或者使用 `cmd.exe /c rename`，因为 NTFS 原生改名更稳定。

WSL 中操作 /mnt/c 文件时，可以加上 --metadata 选项（如果用的是 /etc/wsl.conf 配置），这会让 WSL 把 Linux 权限存在 NTFS 扩展属性里，避免部分不一致。
...
其原因在于显卡在 UEFI 下不兼容 DP 口显示，主板在 UEFI 启动引导，等待屏幕显示超时后自动切换使用 UEFI CSM 模式，这才结束黑屏，出现操作系统启动的画面。

    如果在主板的 BIOS 设置中改为 UEFI CMS 模式后开机启动就很快，不会再等超时后才切换模式，但这样操作系统就不支持 SecureBoot 功能了，参见章节 [安装 Windows 启用 Secure Boot 功能]
...
主板 BIOS 设置为 GPT + UEFI 的情况下，可以连接 HDMI 口安装 Windows 并支持安全启动，参见章节 [安装 Windows 启用 Secure Boot 功能]。

#### 一劳永逸方案：升级显卡固件解决 DP 口开机显示

先把显卡挂到别的能显示的机器上，或先连接 HDMI 口安装 Windows，进入系统后，升级下固件，以后就可以实现连接 DP 口在 UEFI 模式下安装 Windows 10 了

Nvidia 从 GeForce 700 系列就有这个问题

    https://zhuanlan.zhihu.com/p/1910709843406323790

Nvidia 升级显卡固件的程序下载地址：
...
关于主板 BIOS 设置 CSM 模式可以启动 DP 口的解释

    https://nvidia.custhelp.com/app/answers/detail/a_id/3156/kw/doom/related/1

### 主板 Ultra Fast 启动无法再用键盘进入 BIOS 设置

很多支持 Fast Boot 的主板，在主板 BIOS 的“Fast Boot”项设置了“Ultra Fast”之后，在开机过程中不能用按F1进入 BIOS 设置了，解决办法是在操作系统的关机菜单中，在点击重启时按住 shift 键，会出现重启选项，选择引导到 UEFI 固件设置。

对技嘉 B560M AORUS PRO 主板来说，实际试了一下，没感觉到 Ultra Fast 比 Fast Boot 快。估计是因为我的 usb 键盘鼠标网卡都连接到显示器集成的 usb hub 上，设备比较多，开机时 UEFI 加载这堆驱动没法跳过，没什么用的感觉。

UEFI 方式启动的操作系统是跟主板 BIOS 设置密切结合的，UEFI 的“Fast Boot”分几个选项，停用了 usb 相关驱动的加载，导致了初始化部分设备的限制：

    主板 BIOS 设置的“Fast Boot”项如果开启“Fast”选项，可以减少硬件自检时间，但是会存在一些功能限制，比如 Fast 模式下不能使用 USB 设备启动了，因为这个加速功能其实是在 BIOS 自检中禁止加载 USB 设备了，Windows 对此的支持就是在操作系统启动时自动进行加载。

    主板 BIOS 设置的“Fast Boot”项如果开启“Ultra Fast”选项，之后就不能用键盘进入 BIOS 了，估计跟 Fast 模式一样，把大部分不是必须的自检过程给禁用了，所以要想进 BIOS 只能清空 CMOS 或者在操作系统里选择“重启到 UEFI”。
...
确认 Windows 10 快速启动功能是否关闭，这个功能会关闭 USB 口，参见下面章节 [关闭“快速启动”]
...
USB 3.2
    ...
    显示器画面可以输出 3440*1440@60Hz 8bit、3440*1440@100Hz 6bit

USB 2.0

    如果你设置显示器画面达到满血的 3440*1440@100Hz 8bit，显示器会自动把连接埠切换为 USB2.0。如果你不外挂硬盘的话，那么这也不会成为缺点。

PBP 分屏 --- 分屏模式

    分屏显示多个输入设备的桌面，大屏幕显示器都有这个功能。

    usb-tpyec 的线连接笔记本电脑后，可以分隔一半屏幕显示笔记本电脑、手机、平板等设备的桌面内容

    外接三星 Galaxy 手机，配合 sumsung dex 功能，在 Windows10 下装好驱动，可以显示手机桌面，鼠标和键盘也可以操作，即在大屏幕上用你的手机。

    显示器设置 kvm usb up，开启 基础的 usb-hub。
...
KVM 功能
    ...
    软件 KVM 见章节 [多主机共享键盘和鼠标 KVM](gnu_tools.md)

PBP 分屏的一个副作用

    如果用三星 Galaxy 手机 DEX 连接显示器集成的 usb-hub，关机会黑屏，其实是显示器自动切换到显示手机屏幕了，而手机这时是息屏的，拔下来手机之后 Windows 才能继续关机

在 Windows 操作系统的设置里不要开启 HDR 效果，即使用桌面应用不要开启 HDR 效果，只在相关视频软件和游戏内找设置开启 HDR，详见章节 [使用 HDR 显示器]，否则 [显示器开启 HDR 后 Win10 桌面变灰泛白]。

### 使用 HDR 显示器

1、务必安装显示器对应的驱动

显示器厂家的官网一般都会提供自己品牌显示器对应的驱动程序，其实主要就是颜色配置文件，特别是 HDR 显示器务必要安装厂家提供的颜色配置文件。
...
原因见章节 [显示器开启 HDR 后 Win10 桌面变灰泛白]。

如果你的应用不支持单独设置 HDR 模式，而是跟随桌面环境的 HDR 设置，那就只能在打开应用之前，先在桌面开启 HDR
...
目前单独支持 HDR 模式的应用：
    ...
    Nvidia 通过驱动程序实现网页浏览器支持 HDR 模式。方法是打开 Nvidia 控制面板，打开 “RTX 视频增强” 选项：包含 “RTX视频超分辨率技术(RTX Video Super Resolution)”和 “HDR”。这样浏览器在全屏播放网络视频时可以进入 HDR 状态并增强画质：
        ...
        网页浏览器 chrome/edge 等也要开启选项：开启使用显卡硬件加速功能。

但是，Windows 现在不支持在 hdr 应用程序退出后在桌面状态自动关闭 HDR 模式，需要手工调整显示器，把睿动光感、环境光、低蓝光模式、SmartImage 等功能再打开，把显示器亮度降下来歇歇眼。。。
...
这个现象在 FALD/OLED 显示器上是不存在的，仅在 Display HDR400-600 的"假 HDR"显示器上存在
    ...
    微软也没辙，属于技术局限，只能等未来看微软打算怎么解决这个问题了。现在来看，市面上的 HDR 1000+ 的显示器越来越多了，这个问题就不存在了。

    折衷的办法是，显示器厂商单独出 HDR 配置文件，让 Windows 自动识别实际的 HDR 亮度范围，而不是接收显示器现在汇报的这个虚假的 HDR 亮度范围。

其实现在的 HDR 标准其实是纯凑活事，显卡信号输出了剩下的全看显示器，导致 HDR 内容的显示没有任何标准，各厂家的显示器效果千差万别。

这点在真 HDR 显示器上也很明显，不同品牌的 FALD 显示器效果也是完全不同的，颜色亮度各种跑偏，完全是群魔乱舞。

很多游戏内置 HDR 选项，让你自行调节亮度和色调来适应屏幕就是这个原因。

目前显示 HDR 内容效果最真实自然的是 SONY 大屏幕电视，桌面显示器5位数价位的都不行，空间太小了。

#### 看 hdr 电影太黑

电影的画风是按照电影院黑乎乎的环境下观看进行调校的

    sdr 标准亮度 80nit，很多电影拍摄时纸白（大致相当于平均亮度）设置就是 80-100nit，电影院里关灯看，不会感觉太黑。

    hdr 亮度等级：迄今无。很多导演都喜欢画面用阴冷风格，在影院里看不黑就行。在 hdr 效果下，暗部会更黑，需要高对比度高亮度的银幕才能体现细节。这就要求播放设备要高亮导致很容易高温，搜一下新闻---电影院播放阿凡达着火了，就是因为有些负责任的电影院为了保证原片效果用了高亮的电影灯，结果导致设备高温出现火情。所以，现在大多数电影院的播放亮度不足是很常见的，就像很少见到开通风的电影院一样。

电视或电脑播放电影，先天就不是电影院那么黑的环境，在明亮的环境下更不容易看到屏幕上的暗部细节。

而且网络下载的视频都是压缩的，也会导致暗的更暗：

    先排除枪版，至少源片片源是蓝光碟或4k hdtv。

    sdr 片源转制：网络下载电影大多是压缩转制的，会丢失部分信息，如果影片色调本来就偏阴冷，压缩后的暗部会丢失更多细节，即使再用 hdr 转制也是全黑无细节了。

    网络视频节约带宽：网页上写着4k视频，实际播放的码率不到 5Mbps，糊的都不行了，暗部细节早糊没了。
...
显示设置有个颜色范围的选项，默认选项经常不对。

1、RGB输出动态范围与显示设备的匹配

    一般来说，显示器或者电视机会有 “HDMI黑色水平” 这样的设置选项，其实就是两种不同的明度色阶（16-235）和（0-255）：

        其中 低/Limited Range 对应显卡输出动态范围 “有限”

        正常/Full Range 对应显卡输出动态范围 “完全”

    Nvidia 控制面板的输出动态范围选择 “完全”：开了可以增加黑白明暗对比，增加色彩艳丽程度，白色更白黑色更黑，更鲜艳。

如果二者的设置不一致，不对比可能不会觉察，很多人可能被显示器“正常”这个选项误导，一直在错误的显色环境下使用：

    显示器设定 低/Limited Range，显卡设定完全，则屏幕显得曝光过度，暗部亮部全部失去细节，这个很明显。

    显示器设定 正常/Full Range，显卡设定有限，则屏幕发灰。

如果你的显示器/电视机连接PS4、Xbox，都要注意匹配。

2、选择 “低/Limited Range” 还是 “正常/Full Range”

不要被名词误导，两者设定“低/有限”或两者设定“正常/完全”都是正确的，比如电视机信号格式就应该选择 “低/Limited Range”。

如果显示器屏幕素质一般的话开 “正常/Full Range” 反倒不如不开柔和。我办公用的电脑反复试了，开“完全”比不开柔和，全白的时候刺眼，酌情处理：

    VA面板，选完全，使得亮处更亮，暗处更暗，画面层次感更强

    IPS或TN面板，选有限，使得暗部细节能看的清楚，不至于糊成一团

另外，同一个显示器插 HDMI 线或 DP 线也是效果不同，大致因为 HDMI 是电视标准而 DP 是电脑标准，自己酌情设置即可。
...
视频游戏追求逼真的感觉，也是在光影和细致程度上下功夫。
    ...
    光线不足就会降低真实感。如果预烘培和游戏中的光影计算都尽量简化，那么游戏角色的渲染效果就会表现出卡通化，比如手机游戏、3D 网游等对显卡要求很低的游戏，即使画面非常清晰，但画面风格只能做成卡通化。

2020 年代之前的 3D 游戏都是光栅化技术来渲染画面，可以实现光线的反射折射阴影

    依靠游戏开发时在场景中预烘培实现全局光照，美工方面多下功夫就可以给玩家照片级的真实感了，贴图细节调高就可以看到非常清晰的画面，而且场景中预烘培的漫反射光线充足各处都不会太黑。比如 2007年的 《孤岛危机》、2016年的《神秘海域四》、2018年的《荒野大镖客2》、2020年的《美国末日2》等，场景美轮美奂。

    在游戏设置中贴图细节选择越高、LOD 拉的越高，画面就越清晰。

但是，如果 3D 游戏画面显示技术想再进一步，实现 动态效果 也有照片级真实感，比如像过场电影一样的游戏效果，依赖预烘培的光栅化处理技术就有非常大的限制：

    最简单的静态全局光照，光源无法变化导致场景也不能被破坏和移动，可交互性很差。自由运动的游戏人物和角色，很难跟场景中光线的色调协调，比如你会看到游戏角色跑进黑洞但仍然发白的现象。如果使用预烘培全局光照实现动态全局光照，对规模越来越大的开放世界场景 RPG 游戏来说，不止是开发时工作量超大，而且生成的光照信息，可能高达几十TB甚至更多，这已经无法部署到玩家硬盘了。光栅化也无法实现汽车表面这种曲面的反射和镜面对照。

光线追踪技术可以解决这一切。

Nvidia 在2018年的 RTX 2080 开始推广光线追踪后，高端游戏的引擎技术发生了巨大的改变。

这里就引出了题目中的疑问：

    如果你在游戏选项中把画质细节拉满了还是觉得画面糊，亮度拉高到暗部蒙了层白纱感但还是黑，大致原因就是游戏引擎切换为使用光线追踪了，底层原因是都使用了 TAA 这个模糊技术，这是用模糊换速度的妥协技术。

光线追踪的原理是对玩家屏幕上的每个像素点都进行实时计算，精确实现光线与物体表面的互动：

    一个像素点投射n多个光线，打到各个物体上，反弹后如果达到光源如灯、太阳，则屏幕上的这个像素点就发什么颜色的光。其计算混合了物体的材质、粗糙度，光源的颜色等多种因素，实际显示效果比传统的光栅化处理更加真实，未来甚至可以做到像照片那么真实。

光线追踪的先天缺陷一：

    达到 1080p 下可以接受的画面，光线追踪的计算量都超级大

从 2007 年代英特尔就尝试用多核心通用图形处理器（GPGPU）代号 Larrabee 来实现光追游戏 <https://www.gameres.com/853845.html>，甚至云游戏串流运行。但直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才真正成为了现实（光追游戏体验见《赛博朋克2077》）。

如果是计算光线路径的多次反弹即所谓“路径追踪”，间接光下的漫反射制造的光影真实感更强，路径追踪的二次反射是在第一次光线路径的反弹点上递归，再计算反弹出来的光线路径的多次反弹，计算量更是爆炸，帧数一般暴降 30%+。

    至于为何 Nvidia 给显卡非常少的硬件光追单元 RT Core，就不足为外人道也了。比如 5090 显卡有 21760 个 CUDA Core，仅 170 个 RT Core，专为深度学习设计的 Tensor Core 都有 680 个。

光线追踪的先天缺陷二：

    因为每个像素点都要投射光线，噪点是光线追踪技术的先天问题

数字图像技术中涉及光线的运算，一直存在“散粒噪声”的问题。对数码相机来说，可以用更大的光圈和更好的CMOS来缓解。但是在电子游戏里，只能发射更多光线或者用算法降噪。

    发射的光线越多则画面效果越好，但需要庞大的算力，5090 显卡在这面前也只能说是“小小的很可爱”。

    发射的光线数量越少，则计算量就越小，但是画面噪点会变多，整体变暗。参见 《Quake2:RTX》，游戏设置选择关闭滤镜查看原始光线追踪渲染的画面效果。

所以在光线不足条件下降噪是很现实的解决方案，高斯模糊等各种模糊方案就是解决王道。但是，模糊处理在抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。模糊处理的额外好处是降低了系统渲染压力，越糊显示速度越提升。

最终，我们得到了充满各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。而且角色的毛发会有锯齿，跑动中看到的草坪会从清晰变模糊，这不是你的错觉，这是 TAA 的特点。无奈的是，从 UE5 引擎的各种游戏，到 Doom:The Dark Ages 全都切换到使用 TAA 技术了。

    Nvidia 推出的 DLSS 属于对 TAA 的改良，清晰度经过 AI 修正反而能降低模糊程度。

    光线追踪技术在反射、折射上的效果最明显，所以支持光追的游戏场景都喜欢到处放水洼，路面也洒水。而镜子太消耗计算量，那么镜子和玻璃能不用就不用了。

    如果看到有些物体延迟产生阴影，是因为光追计算优化的不到位。

其实 Nvidia DLSS 也是 TAA，但用 AI 进行了清晰度修正，并新增了很多功能，比如从低分辨率生成高分辨率画面，用 AI 生成画面来减少计算量，甚至直接插帧生成新画面，各种歪招全都有。

总体来说，现在的光线追踪游戏，有点类似数码相机替代胶片相机的过程。在刚开始发展的时代，数码相机清晰度不如传统的胶片相机，但是在多次迭代后清晰度达标了，几年功夫就实现了对胶片相机的替代。

Nvidia 在显卡计算能力不足时强推光线追踪，导致了到 2025 年光线追踪游戏的画面效果还不如 10 年前的游戏：

    跟之前的光栅游戏相比，完全光线追踪技术的游戏画面真实感轻微提升，清晰度明显下降，光影反射效果明显提升，物体暗部死黑太多（减少使用预烘焙又不舍得多给实时光线），帧率巨幅下降，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 基本都是如此。UE5 引擎开发的《寂静岭f》画面效果倒退到了 2000 年代，真实感还不如《孤岛危机》。

    比如，优化最好的，基本看不到死黑的 Doom:The Dark Ages，号称完全使用光线追踪（其实是部分烘焙再加强制硬件光追），计算量超级大：为了减少反射的计算量，除了主角，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于部分水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。相比之前的 DOOM2016/DOOM ETERNAL， 各种减少视觉元素，居然只是画面整体的光影真实感更强一些，并无任何惊艳。功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 4K 60帧/秒。<https://doomwiki.org/wiki/Tiago_Sousa>

    赛博朋克2077，糊的情况稍微好点，但暗部死黑太难受了，玩这游戏最好太阳下山了再出门。

    所以 2025 年末发布的游戏《战地6》直接宣布当前阶段不使用光线追踪技术，优先保证玩家体验。

玩家可用的缓解办法，见下面的子章节。

彻底解决办法：

    想看清晰逼真的画面还得再等几代显卡，光线追踪的渲染从 DLSS 发展到神经渲染（AI 技术神经网络机器学习），计算不达标用 AI 给你直接渲染出来，最终实现照片级效果。

光线追踪和路径追踪的概念：

    https://www.bilibili.com/opus/987743328795623446

    计算机图形学算法中，路径追踪是光线追踪的一个实现方式，而现实的发展中受显卡能力影响，光线追踪和路径追踪及光栅渲染分别划代了。

    N 卡 2080 时代开始宣传的光线追踪，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。比如 Doom:Eternal 开启光线追踪后的“邪教徒基地”关卡，场景还是预烘培的，各种不锈钢材质都在反射周边的灯光甚至 slayer 角色模型，也敢于大范围布设玻璃和镜子，光栅和光追组合非常节省显卡的硬件资源，中低档显卡都能畅玩。

    N 卡 4080 时代宣传的路径追踪，逐渐的在抛弃光栅化渲染，更多的使用光追来形成画面，这极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 号称完全使用光线追踪渲染画面，结果跟上一代的 Doom:Eternal 相比只是光影感觉更真实，画质细节完全没有进步，远看不错贴近就是糊。

##### 让你的游戏更清晰

如果感觉你的游戏画面有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高还是糊，尝试以下缓解方法。

1、游戏设置里关闭如下选项：动态模糊、胶片颗粒、电影质感、景深、色差、分辨率缩放、VRS(可变着色率) 等。

屏幕空间反射 (SSR)和后处理滤镜也可能使画面糊，可以尝试把屏幕空间反射 (SSR)拉到最高或直接关闭。

另外游戏帧数不足 60 帧的时候，有的游戏引擎会用动态模糊来增加流畅感，参见 [游戏卡顿不跟手]。

2、锐化

    游戏设置一般都有个 “锐化” 选项，拉高锐化的数值，使凹凸细节更明显，在观感上改善清晰度，大部分这种画质糊的问题都可以缓解。注意锐化数值太大会让亮处噪点明显，树皮的纹理变得不真实等，酌情调整

    如果启用了 DLSS，则还可以调整 DLSS 的锐化选项。另外，DLSS 的档位尽量选择“DLAA”或“质量”，比“性能”档更清晰

    如果有“渲染倍率”选项，拉到100%

比如 Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，总给人画面比较糊的感觉，而把 “锐化” 拉到 3 左右，清晰感就类似过场动画了，跟 Doom：Eternal 的细节水平差不多。当然 doom 系列一贯的风格就是远景清楚近景糊，除了主角，其他物体贴近看你就别想要清晰材质。

另外可尝试 Nvidia 在驱动程序级提供的覆盖设置方式：

    开启 Nvidia 驱动程序自带的滤镜，直接给画面加亮度、加锐化：
...
3、如果游戏引擎原生使用的是 TAA 反锯齿，提高帧率也可以使画面更清晰，这是 TAA 的一个特点。

或者切换到 DLSS4 + DLAA 并拉高锐化数值，DLSS 是 AI 帮你生成画面的技术，比原生 TAA 渲染清楚些。

如果不使用 TAA，可尝试更换反锯齿算法为传统的 8xMSAA，这个可以避免依据时间序列产生的模糊现象。

4、使用 4K 分辨率，依靠高分辨率材质来提高清晰度（如果游戏在开发时没有对材质进行粗糙模糊处理的话）。

把显示器从 1080P 换成 4K，画质也能有明显提升。有些游戏 2k 分辨率拉满设置里的抗锯齿选项，实际画面还不如 4k 分辨率开 DLSS 性能档不开抗锯齿的效果。

其实在 4K 或更高的分辨率下，因为细腻度高于人眼，已经不需要开启反锯齿选项了。在 nvidia 的驱动程序设置中，关闭 反锯齿 相关的各个选项，看游戏图像是否变清楚了。

没有 4k 显示器，可以尝试开启 Nvidia DLDSR(深度学习动态超分辨率) 让你的 1080P 显示器秒变 4K：
    ...
    开启方法
        ...
        Doom:The Dark Ages 等原生支持的游戏，在游戏内可选择4K分辨率即使你的显示器是 1080p，不需要在显卡驱动设置中开启 DLDSR，这样你的桌面环境仍然是标准 1080p。
...
5、类似的，提升视频网站和直播平台的视频清晰度，可以利用 Nvidia RTX 视频增强（Super Resolution）。
...
目前支持该技术的应用软件太少：
    ...
    chrome 浏览器，类似 edge 的设置过程。

    VLC 播放器有个特殊的优化版本支持 Nvidia RTX 视频增强 <https://download.videolan.org/testing/vlc-rtx-upscaler/>

##### 让你的游戏不太暗

光追游戏把预烘培光照的漫反射拿到玩家设备进行实时计算，直接导致漫反射效果降级，光少了就暗

    https://sspai.com/post/83522

在开放场景中常见背光面死黑、白天屋里很黑这种高光强对比度让人眼睛不舒服的情况，根源在于光线追踪迫于计算量优化不能发射太多实时光线，这就非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校。

为了方便（削减成本），很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体背光的暗部出现死黑，只看暗部又会受环境光照影响慢慢变亮，让你的眼睛非常不舒服。

这不是什么黑屋看门口强光的 HDR 过曝效果，这就是游戏对直射光和非直射光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：

    黑神话·悟空，第二章的山洞场景不是过曝亮瞎眼就是黑乎乎什么都看不到，沙漠场景中明暗同时存在的视角闪爆你的眼球，对比度拉的极高但是全局光照考虑的太少，大白天的阴影里愣是黑得伸手不见五指。

    塞伯朋克2077 也是类似的问题，白天的时候物体的背光面死黑，屋里也死黑。在阳光明媚的大街上转动视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑就像犯了低血糖。不要说原本就是黑的，你只看暗部的时候它会慢慢变亮给你正确的光影结果。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站在大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。

    好的例子是 Doom:The dark ages，高光给人冲击时背光面也有足够的亮度能看清细节，面向明暗交错的物体转动视角时的过渡处理的非常自然，眼睛完全不累。在场景设计方面，全局光照（RTGI）调校的好，低光的山洞或水下也会细心的放置光源，死黑很少见，整体画面的风格保持一致。

相比之下，电影级的离线渲染对光线的计算都是不计工本的，每个像素点发出的光线就非常多，而且是多次发射并计算反弹、甚至反弹点再发射超多光线再次进行计算，在 3D 游戏行业的实时渲染是做不到这种程度的。

缓解办法：

1、如果是游戏暗处太黑（含光线追踪），首先在游戏内设置调整“伽马值”，适当放大比如 `1.10`，达到暗处稍亮而亮处不发白即可。伽马值拉太高整个屏幕都会都发白，酌情调整。

如果是 HDR 显示器，游戏内一般会有个专门的 HDR 参数设置页面，可以调整亮度、色调等细节。

NOTE: 传统的调节显示器亮度对比度的方式，已经不适合现代游戏了：

    光线追踪的游戏，对比度亮度都是动态计算出来的，全局光照的曝光度也随着视角和环境变化，这都跟游戏的运行时状态有关，单纯手调显示器的亮度对比度不会起作用。

    最典型的就是游戏开启 HDR 模式，显示器会自动切换到最高亮度模式，全靠游戏或视频的亮度数据来动态变化明暗。

2、增加光线

游戏内的设置，开启 “路径追踪”，可以增强漫反射的效果，提升画面整体的亮度和明暗表现，真实感更强。比如非镜面的反光物(有粗糙度的材质)也会投射光线到其他物体，灯条等自发光材质也能稍微照亮周边物体

    路径追踪会增强间接光照射物体的明暗感觉，真实性更好。判断是否开启了路径追踪，可背向光源看反光面，是否反光到其他物体，那些物体是否改变了亮度和颜色，比如光线直射的桌面，是否照亮了周边的墙面和物体、灯光直射的地面，不被直射的周边墙面也会跟随改变颜色和亮度，而且门框等凹凸物体也会出现阴影轮廓。

    理论上路径追踪也会增加对材质细节的表现，因为每像素的采样精度提升了，画质会更清晰。

    目前的路径追踪技术有个问题，在你的视角运动时光影可能会闪烁

如果是虚幻5引擎开发的游戏，用工具调高 lumen 的漫反射颜色增强（Diffuse Color Boost），尽量增大光照，可以缓解暗部死黑的问题。

    https://ngabbs.com/read.php?tid=41409698&rand=286
...
这个插件目前有鬼影问题解决不了，如果你看到对面走来的人脸是鬼脸，关掉这个插件就可以解决。

##### 其实 TAA 还影响你看电影了

    单张照片用 “浅景深”（背景虚化）技术，可以柔美的虚化背景，突出拍摄主体，拍摄出来的照片非常有感觉

    电影的背景用 CGI（计算机生成图像）或者 LED 背景墙（UE 5），后期人工添加模糊来模拟景深让演员“融入”虚拟背景

有些实景少的低质量电影，全用浅景深给人物特写，让你觉得背景模糊很自然，但是整个电影看下来的感觉就是像看美剧 --- 只有人脸清楚，头发很规律的糊，连肩膀都糊，出门更糊，整个片子的视觉印象就是糊里巴涂。这个问题的根子在 CGI 的模糊技术上：

    模糊用的太多，画面的信息量太少

    模糊时不重视维持焦点，丢失了层次感，前景和背景明显割裂

    运动镜头的动态模糊处理的不好，也会割裂前景和背景

绿幕技术已经方便到，演员的大部分时间都是在绿幕前空喊，空打，画面中所有的背景乃至光影合成都在后期 CGI 实现。我们看到的各种街景、草原、山脉、洞穴等等场景，都是导演在后期时的想法，不满意立刻就能换个样，不需要演员重拍，非常有利于创意。

如果制作时优先考虑观众视觉感受，CGI 背景可以给你大景深的清晰宏大视野，画面细节逼真元素丰富，运动镜头也能保证不是全糊可以看清，最有代表性的就是《阿凡达》、《哪吒》系列。

但很多电影走向了反面，用 CGI 背景偷工减料，最典型的是《蚁人：量子狂潮》：科幻电影可以不要真实感，但是背景全糊连各种炫光都糊，明显割裂出来的前景有两个演员在大喊大叫，画面感直追奥特曼真人动画。作为商业大片，只要宣发给力让人热闹一阵就够本了，但是真坑人。而且这部电影的官宣制作成本居然达到 3.8亿 美元超过阿凡达2，里面有多少内幕无法想象。

##### 其实 TAA 还影响你看视频了

网上下载的有些高压缩片源，画面不是很模糊，但是有颗粒感，闪烁感，这不是你的错觉，根子还在 TAA。

视频压缩技术的特点决定了各种算法基本都是基于时间的：

    只存储连续帧之间发生变化的部分，而不是每一帧都完整存储。这就像翻页动画书，只画动作的部分，背景不变就不用重画。

    实现方法是将颜色和细节相似但实际不同的区域强行“抹平”成同一色块

网络视频的特点，决定了视频不重视高质量还原，而是注重高压缩，以明显有损实现低码率（文件小、带宽低）。

dvd-1080p 时代 x264 等算法，简单粗暴的用模糊降画质，高压缩视频的视觉效果太差，画面模糊黑暗细节丢失。

4k 时代 x265 等压缩算法变了，低码率视频的观感也变成了几种：

    画面抖动感觉被“抽帧”了：

        当画面中有细微的、缓慢的运动（如人脸肌肉的细微变化、水波纹、远处树叶的摇动）时，这些运动信息在低码率下被严重丢弃。每一帧，压缩器都在“猜测”并涂抹这些区域，当预测失败（比如复杂运动）时，解码器没有得到足够的修正信息。它只能用错误的、模糊的或来自之前帧的“脏”数据来拼凑当前帧，导致相邻帧之间被抹平的区域边界（方块边缘）会发生不连贯的、跳跃式的变化。这种边缘部分在连续时间上无法对齐，导致了视觉上的画面在抖。

    色块、难看的颗粒感：

        不走模糊路线了，画面过分锐利，会看到面部有明显青紫色块，低光无光的黑暗场景观感差，不同于电影胶片颗粒感的虚假的、难看的强烈颗粒感。视频码率虽然不低，但是画面颜色单一感强烈。

        在压缩时，在降噪阶段原生颗粒大面积地抹平，其纹理信息已被严重破坏。当画面有轻微变化时（如下一帧光影稍有不同），编码器发现这个“已被抹平的区域”现在和上一帧不一样了，它必须用低码率去描述这个变化。x265 算法会导致块状化、带状化、闪烁的、高对比度的伪影。

简单判断视频质量是否及格，以时长90分钟的枪战片电影为例：

    1080p 视频，码率 2MB/s，文件大小 h264 小于 2.1GB，h265 小于 2GB，会明显影响观感。

    2160p 4k 视频，码率 8MB/s, 文件大小 h265 小于 8GB，会明显影响观感，那种 5,6GB 的 4k 电影不要下载。

    美剧例外：电视剧制作成本小，全片的风格一般就是屏幕元素少，大量使用浅景深画面，连头发都是模糊的，以上最低标准下的观感影响不明显，但是不要讲清晰度。

    误差：电影拍摄设备差异导致实际画面内容占比不同，单纯看容量未必可靠。摄影风格差异导致画面中元素内容多少，细节越丰富对容量要求越大。

#### 游戏卡顿不跟手

通常射击游戏在开发时会针对常见的使用场景设置设计目标，比如帧率 60帧/s，在 60hz 显示器上的帧间距是 1ms，一般原生画面的渲染延迟大概在 10 ms 左右，不会影响手感，视觉上也不卡顿。

如果达不到这个标准，或运行时波动太大，就可能出现卡顿的情况：

    手感卡顿 --- 渲染延迟：指的是画面生成到显示器呈现的时间，渲染延迟过大会影响手感，晃动鼠标会感觉到画面里的人物动作跟不上，即不跟手

    画面卡顿 --- “帧间距”：指的是两帧之间的时间，过大会影响观感，即眼睛看着画面感觉卡顿

帧间距过大，技术上可以用动态模糊骗掉眼睛，从而使画面看起来很流畅，但是画面的延迟却无法用图像处理来解决。

1、每秒生成的帧数不足以覆盖显示器刷新率，“帧间距”过大造成画面卡顿：

    30 帧/s 时在 60hz 显示器上会感觉到画面卡顿

    60 帧/s 时在 120hz 显示器上会感觉到画面卡顿

举个例子，你的显示器刷新率为 300hz，锁 30 帧渲染延迟有 1ms，锁 300 帧渲染延迟有 4ms，这两个渲染延迟不会影响人的感觉，但是“帧间距”会影响到人：

    锁 30帧/s，虽然是 1ms 的渲染延迟，但是“帧间距” 300/30=10ms 太大，会感到画面卡顿

    锁 300帧/s，帧间距 300/300=1ms 眼睛看着更流畅，而且其 4ms 的渲染延迟也不影响手感

这种情况下就应该选择锁 300 帧。

有些低端的电视机的响应时间很长，有的人在这种电视上打游戏感觉不出来滞后感，但如果游戏的每秒帧数从 60 帧降到 20 帧，他就会觉得“卡”，原因就是帧间距的影响。

其实帧数过高也可能影响画面观感，因为显卡送到显示器的数据太快也需要缓冲，反而排队了。

所以电竞选手一般选择高回报率鼠标和高刷显示器，搭配高配显卡保证帧率跟的上刷新率。

例外：把 CS2 和 CSGo 都锁帧到 300帧/s，操作手感上 CS2 就是迟滞，这是游戏引擎的设计问题，CS2 现在成了电竞游戏了，开发商也不敢调整了，否则会影响很多职业选手。

2、 DLSS、抗锯齿等措施

DLSS 等技术针对游戏原生输出的画面，显卡重新计算甚至用帧生成插入 AI 帧，都会增加渲染延迟，比如赛博朋克2077，开启帧生成后延迟会大大增加。

帧生成的最好效果是原始帧率在 60帧/s 以上，在帧率不足时开启其实效果并不是最优的。

但是

    开 DLSS + 帧生成 + Nvidia Reflex 后，Doom:The Dark Ages 的延迟反而很神奇的下降了，而且不需要开启垂直同步（三重缓冲）了。

所以，各项调整应该根据游戏中的实际体验来做选择。

#### 晕 3D 怎么办

1、游戏设置问题

把环境晃动、摄像头晃动等游戏选项都关了：有些游戏战斗时镜头快速变化或视觉模糊来制造压迫感，以让你胜利后有释放压力的快感。但是晃画面非常无聊，关掉晃动反而游戏体验更好。

2、垂直同步

垂直同步会把游戏的帧率锁定为显示器的刷新率，但会导致玩家头晕，还会轻微增加渲染延迟。

    鼠标对准屏幕中心的某个物体，保持对准该物体横向平移跑动，或纵向前后跑动，如果画面中央出现轻微的水波纹或撕裂感，就说明需要开垂直同步

我玩射击游戏时，关垂直同步就不头晕了

    建议除非遇到画面撕裂或抖动的情况下打开垂直同步。或者开启三重缓冲（需要游戏支持）可缓解头晕，但是偶尔会卡一下。

    一般来说开三重缓冲后，快速晃动鼠标时画面即时跟随你的动作没有迟滞感，头也不晕就可以。

帧数不足，但又不想开垂直同步的一个缓解办法是，使用支持可变刷新率 VRR 的显示器如支持 Nvidia G-sync 标准的显示器，在低帧率时候有用，帧数超过显示器刷新率就没什么作用了，遇到画面撕裂还是得开垂直同步。

头晕的原因可能在于相对于手的快速动作，屏幕的显示受帧率影响波动性延迟，而 fps 射击游戏频繁快速的转身和视角变换，导致大脑对手运动的感知和对视觉的感知不一致，进而导致头晕。

#### 8GB 显卡爆显存

某些游戏把光线追踪、反锯齿等高画质功能拉满后，特别是在 4K 分辨率的显示器下，显存占用超过 8GB，会提示显存不足，严重情况下一卡一卡的，这就是爆显存了。

如果没爆显存，说明这个游戏优化的好，渲染时画质稍微降低了或切换到内存，你感觉不出来。

如果想凑合下解决：开启 DLSS，可以稍微降低对显存的需求，或者在游戏选项中适当降低画质。

彻底解决就得换大显存显卡：

4K 分辨率用 16GB+ 显存的显卡，比如 GigaByte GeForce RTX™ 5070 Ti EAGLE OC SFF 16G

    https://www.gigabyte.cn/Graphics-Card/GV-N507TEAGLE-OC-16GD

注意：高功耗显卡的峰值功耗基本比最大功耗多50%+（A 卡的直接翻倍估算），但是这种毫秒级别波动只有用示波器能看到，而厂家却从来不负责功耗超标问题。所以，用200W功耗以上的显卡，必搭配不虚标的大功率电源，有个说法至少要一瓦一块钱判断电源是否靠谱
...
微软自己都废弃了这个“动态磁盘”
    ...
    在 Win10 之后，微软使用类似 Linux LVM 的存储管理方式：“存储空间” 来取代动态磁盘，有“存储池”的概念。将多个物理磁盘组合起来，形成一个虚拟的存储池，并从中创建出一个或多个虚拟磁盘。每个虚拟硬盘可以使用不同的虚拟磁盘布局，如简单、镜像（Raid1）、条带化（Raid0），奇偶校验（Raid5）。这些虚拟硬盘其实都是“软Raid”，不但可以各不相同，而且可以托管系统。比如：3台1T磁盘组成存储池，其中每块硬盘的前100G组成总容量300G的条带化（Raid0）虚拟硬盘，安装系统，用以加快系统启动。剩余空间组成容量1.8T（900*2）的，奇偶校验（Raid5）虚拟硬盘用于保存数据。
...
exFAT 的设计面向 U 盘，既可以存放大于 4GB 的单个文件（单文件最大可达 16EB），又可以减少额外读写，避免伤害 U 盘的寿命。
    ...
    Windows 默认格式化选项设置的比较小，需要在格式化的时候手动选择 256KB
