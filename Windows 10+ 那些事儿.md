...
分析下根源，以前光栅游戏时代，一般是调整 LOD 对应画质细节，设置拉的越高越清晰。4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器分辨率 4K 的优化技术、DLSS 等等歪招全来了，如果你把细节设置拉满画面还是糊，大致原因有以下两种：

    4K 显示器流行以来发展的 TAA 反锯齿: TAA 以前后帧动态模糊的方式解决物体边缘的锯齿现象。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，这是 TAA 的特点。解决方法是画质选项里调高锐化，或把显示器从 1080p 升级到 4K 分辨率甚至画面帧率更高都可以大大缓解这个现象。实在不行，可以尝试更换反锯齿算法为传统的 8xSSAA，或者切换到 DLSS4 拉高锐化数值。

    光线追踪游戏的材质：电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在游戏里，只能发射更多光线或者用算法降噪。前者需要庞大的算力，4090 显卡在这面前也只能说是“小小的也很可爱”。而后者，则是在光线不足条件下的降噪，这样在抹除了噪点的同时，也会抹除画面细节，这是不可避免的现实。目前折衷的办法是加强漫反射，让用户接受材质糊一点，但光影真实感强。另外调高锐化数值使凹凸细节更明显，也会在观感上改善清晰度。
...
光线追踪的很多游戏引擎，都使用这种自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体暗部出现死黑，只看暗部又会慢慢变亮，这不符合现实世界体验。这不是什么黑屋出门看强光的 HDR 过曝效果，这就是对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：

    比如 赛博朋克2077 的白天场景各种物体的暗部动态死黑，在阳光部分直射的大街上来回转头切换下视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑眼睛疼。。。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。

    好的例子是 Doom:The dark ages，游戏引擎的光线追踪全局光照（RTGI）调校的好，高光给人冲击时暗部也有足够的亮度能看清细节，明暗过渡非常自然，面向明暗交错的物体转动视角时眼睛完全不累。在场景设计方面，没光的山洞或水下也会细心的放置光源，死黑很少见。

1、如果是游戏暗处太黑（含光线追踪），首先在游戏内设置调整“伽马值”，适当放大，达到暗处稍亮而亮处不发白即可。伽马值拉太高整个屏幕都会都发白，酌情调整。

如果是 HDR 显示器，游戏内一般会有个专门的 HDR 参数设置页面，可以调整亮度、色调等细节。

传统的调节显示器亮度对比度的方式，已经不适合现代游戏了：

    光线追踪的游戏，对比度亮度都是动态计算出来的，全局光照的曝光度也随着视角和环境变化，这都跟游戏的软件设置有关，单纯调节显示器的亮度对比度没什么作用。

    最典型的就是开启 HDR，显示器会自动切换到最高亮度模式，全靠游戏或视频的亮度数据来动态变化明暗。
...
如果是虚幻5引擎开发的游戏，用工具调高 lumen 的漫反射颜色增强（Diffuse Color Boost），尽量增大光照，可以缓解暗部死黑的问题。比如黑神话悟空，对比度拉的很高但是全局光照考虑的太少，大白天的阴影里愣是黑得伸手不见五指，尤其是第二章的山洞场景不是过曝亮瞎眼就是黑乎乎什么都看不到，沙漠场景明暗同时存在的视角会闪爆你的眼球。这种暗部死黑到无法营造黑暗氛围感的游戏很常见，根源在于光线追踪非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校。

光线追踪和路径追踪

    计算机图形学算法中，路径追踪是光线追踪的一个实现方式，而现实的发展中受显卡能力影响，光线追踪和路径追踪跟光栅渲染分别划代了。

    N 卡 2080 时代宣传的光线追踪，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。比如 Doom:Eternal 开启光线追踪后的 “邪教徒基地关卡”，各种不锈钢材质都在反射周边的灯光甚至 slayer 角色模型，也敢于布设玻璃和镜子，光栅和光追组合非常节省显卡的硬件资源，中低档显卡都能畅玩。

    N 卡 4080 时代宣传的路径追踪，逐渐的在抛弃光栅化渲染，更多的使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 抛弃光栅化渲染完全使用光线追踪，为了降低显卡负担，各种场景中绝大部分材质的粗糙度设置都是不反射，动态光源的使用非常克制，很少放置玻璃和镜子，镜面效果只见于水面和地面的水洼。各种降低视觉效果换来的只是画面整体的光影真实感更强一些，功耗近 400 瓦的 5080 显卡开路径追踪都跑不到 60 帧/秒。估计想看惊艳的光影效果还得再等2代显卡。
...
电影的画风是按照电影院黑乎乎的环境下观看调校的
    ...
    sdr 标准亮度 80nit，很多电影拍摄时纸白（大致相当于平均亮度）设置就是 80-100nit，，影院效果就是关灯看，不会黑。

    hdr 亮度等级：迄今无。很多导演都喜欢用阴冷风，在影院里看不黑就行，但是 hdr 效果是暗部会更黑，需要高亮度高对比度的银幕才能体现细节。这就要求播放设备要高亮导致很容易高温，搜一下新闻---电影院播放阿凡达着火了，就是因为有些负责任的电影院为了保证原片效果用了高亮的灯，结果高温易燃了。所以，现在大多数电影院的播放亮度不足是很常见的，就像很少见到通风常开的电影院一样。。。

电视或电脑播放电影，先天就不是电影院那么黑的环境，明亮的环境下不容易看到屏幕上的暗部细节，而且视频压缩也会导致暗的更暗：

    先排除枪版，至少源片片源是蓝光碟或4k hdtv。

    sdr 片源转制：网络下载电影大多是压缩转制的，会丢失部分信息，如果影片色调本来就偏阴冷，压缩后的暗部会丢失更多细节，即使再用 hdr 转制也是全黑无细节了。

    网络视频节约带宽：页面印着4k视频，实际播放的码率不到 10Mbps，糊的都不行了，暗部细节早糊没了。
