...
在游戏设置里找找显示，把锐化相关选项拉高再看看，如果启用了 DLSS，可能是 DLSS 的锐化选项。另外，DLSS 的档位尽量选择质量或 DLAA，比性能档更清晰。
    ...
    Doom:The Dark Ages 锐化拉到 2~3 清晰感就跟之前的 Doom：Eternal 差不多了。这代游戏为了优化，把大多数的材质限制在粗糙度阈值之下，场景中的动态光影和阴影特效也用的非常克制。所以，把路径追踪打开，可以看到很多有粗糙度的表面比如大理石地面可以反射光线了，这种粗糙材质增加了反射细节变亮了一点也能提高清晰感，火焰、灯光等自发光材质也能稍微照亮周边的材质，也能提高清晰感。
...
动态光照对有光和无光混杂的视角处理不好，亮处很亮而暗部画面非常暗，除非转头只看暗处才能看清，在支持 HDR 效果的游戏上特别明显。这个问题根子上是游戏引擎的调校，调校不好的例子是2077,调校的好的例子是 Doom:The dark ages。

1、如果是游戏暗处太黑（含光线追踪），可以在游戏内设置调整 伽马值，适当放大，达到暗处稍亮而亮处不发白即可。

    最好别调显示器亮度，不然退出游戏到桌面后太亮还得再调，太麻烦了。

如果是 HDR 显示器，游戏内一般会有个专门的 HDR 效果设置页面，可以调整亮度等细节。

2、开启 Nvidia RTX 动态亮丽，让你的游戏暗处不再那么黑
