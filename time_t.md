...
epoch：时间点。 时间计算，按unix系统在70年代定义，操作系统储存的是自1970年1月1日0点（UTC 0时区）开始的秒数，之前的时间用负数。
...
文档中没有专门描述，注意这个 timestamp 类型就是c标准库中的time_t，就是一个整数秒数，一般是从 epoch 开始的秒数，因为是跟系统无关的固定的值，所以在各种语言或数据结构的数据交换中都会支持 timestamp 类型。
