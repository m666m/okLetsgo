...
所以能做变基合并的前提是：
    ...
    本地分支更新，拉取自己的远程分支，需要拉直提交记录，可以做变基合并。这个场景最常见，参见章节 [本地分支从远程仓库更新]。
...
如果被变基合并的分支是你自己使用的本地分支，这样做没有任何问题，只要你的本地提交记录变更不涉及缺陷管理等依赖 hash 值的场景

    # 你的本地分支，变基合并 feature1 的内容
    git rebase feature1 your_branch

如果被变基合并的分支是主干分支，之前有人推送过，否则无法快进合并做拉直，可是这时也不能直接做 rebase 合并，因为变更主干分支的提交记录的 hash 值，推送远程会报错，强推 push -f 会更乱。

    # 妄想直接给主干分支合并 feature1 的内容
    git rebase feature1 master

所以，要避免变更 master 分支现有提交记录的 hash 值，只能变更 feature1 分支的提交记录的 hash 值

    # 先把 master 分支 rebase 合并到自己分支，这样变更的是自己分支提交记录的 hash 值
...
最好的拉直方案是
    ...
    开发分支规定只能使用变基拉直从远程更新，这样甚至不需要使用合并专用分支了

TODO: 开发分支不断更新主干分支，如果是分叉合入的，最终又分叉合入主干分支，提交记录是什么形态？
