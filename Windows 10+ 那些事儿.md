...
光线追踪的游戏，对比度亮度都是动态的，全局光照的曝光度也随着视角和环境变化，这都跟软件的设置有关，单纯调节显示器的亮度没用。而且，如果开启 HDR 显示器会自动最高亮度，全靠游戏或视频每帧画面的亮度信息动态变化明暗。
...
2、游戏内设置，如果支持就把光线追踪的“路径追踪”选项打开。这样可以利用光线的多次反射，即使物体是有粗糙度的材质也会稍微变亮。另外，火焰、灯光等自发光材质也能稍微照亮周边，这些都可以增加一点场景中物体的暗部亮度。
    ...
    现实游戏届的宣传受显卡厂商影响，2018年宣传的实时光追，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。而之后宣传的路径追踪，就是完全抛弃光栅化渲染，完全使用光追的路径追踪来形成画面。
