...
光影对人类立体视觉的影响，以素描为例进行理解。在纸上画几条线段是二维图像，如果在背景上浓淡相宜的扫几笔，就制造出了光影，就有了真实的立体感。要再逼真就得加色彩，再逼真就得提升精细程度，加大色彩和光影的细节变化，到数码相机级别的真实感使用大的CCD采样来实现，人类手绘的方式基本做不到这个程度。
...
但是，画面显示技术想再进一步，实现照片级的真实感，光栅化处理做不到，特别是处理汽车表面这种曲面的反射和镜面对照这种情况。对开放世界场景的 RPG 游戏来说，预烘培生成的光照信息甚至可以高达几十TB，这实际无法部署到玩家硬盘。而光追技术可以解决这一切。
...
这个技术从 2007 年代英特尔就尝试用多核心通用图形处理器（GPGPU）代号 Larrabee 来实现光追游戏 <https://www.gameres.com/853845.html>，甚至云游戏串流运行。但直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才真正成为了现实，光追游戏体验见 赛博朋克2077。如果计算光线的二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降30%+。
...
最终，我们得到了充满了各种妥协的光线追踪游戏画面：
    ...
    Nvidia DLSS 也是 TAA，但新增了很多功能，比如用 AI 生成画面来减少计算量。从低分辨率生成高分辨率画面，到直接插帧生成新画面，各种歪招全都有。
