...
如果执行 `git merge feature1`，因为有共同的基点 c，会做快进，把 f、g 点接续在 c 点的后面，形成拉直效果，但是这样不便于我们观察主干上合入了哪些功能点。

把分支 feature1 强行分叉的方式合入主干，效果如下
...
变基合并通过更新自己分支提交记录的 hash 值的方式，避开制造分叉点获取拉直效果，可能要解决合并冲突
    ...
    # 将 feature2 分支中的修改变基到 feature1 分支上
    # git rebase [basebranch] [topicbranch]
    git rebase feature1 feature2

一般本地分支更新自己的远程分支，用变基合并拉直提交记录，是完全没有问题的。

但是如果本地分支要合并到别的分支，两个分支有远程跟踪分支，则有很多限制

    NOTE: 用 rebase 的方法合并两个分支时，一定要先想清楚哪个分支的提交记录 hash 值不许变，哪个分支的可以变，然后再做。

这是因为 rebase 会变更提交记录的 hash 值，对远程分支做变基会使得其它共享开发的人混乱，参见章节 [遇到提示 push -f 的时候多想想]。

所以能做变基合并的前提是：

    两个本地分支合并，没有远程跟踪分支，提交记录 hash 值变更无影响，可以做变基合并。

    本地分支更新自己的远程分支，需要拉直提交记录，可以做变基合并。这个场景最常见。

    一个本地分支做开发，一个远程分支跟多人同步，本地分支的 hash 值变更无影响，可以做变基合并，方法见章节 [变基合并远程分支要先合并到自身]。这个场景一般是用 hotfix 分支修复主干分支的缺陷，保持拉直状态合入。

    两个分支都有远程分支，但是目的分支做 push -f 变更 hash 值不会造成混乱，也可以做变基合并。一般只适合开发分支结束，合入主干分支后不再依赖开发分支的场景。
...
如果不想被 rebase 修改提交记录的 hash 值，应该选分叉合并
    ...
    目的分支的提交记录的 hash 值变更涉及远程跟踪分支，则使用变基合并会造成混乱：比如功能分支更新合并主干分支，如果用 rebase 合入主干分支的内容，则功能分支的提交记录的 hash 值就全变了，如果功能分支还在持续开发，缺陷管理系统无法对应提交记录，多人共享的环境下大家都要重新拉取功能分支，总会带来一些混乱。所以大多数情况下，功能分支项目组往往选择用 merge 分叉合入主干的内容。

实在需要拉直合并，往下看：

如果目的分支是你自己使用的本地分支，这样做没有任何问题，只要你的本地提交记录变更不涉及缺陷管理等依赖 hash 值的场景

    git rebase your_branch feature1

如果目的分支是主干分支，之前有人推送过，否则无法快进合并做拉直，可是这时也不能直接做 rebase 合并，因为变更主干分支的提交记录的 hash 值，推送远程会报错，强推 push -f 会更乱。

    git rebase master feature1

所以，要避免变更 master 分支现有提交记录的 hash 值，只能变更 feature1 分支的提交记录的 hash 值

    先把目标分支 rebase 合并到自己分支，这样变更的是自己分支提交记录的 hash 值，然后再合并到目标分支，这时变成快进合并了，对目标分支无任何影响。
...
1、先切换到分支 feature1，使用变基 rebase 合并主干分支的内容，使得 feature1 分支在 master 分支的基础上延续拉直。这时 d、e 点被合并进来，f、g 点在后面接续，hash值变更。
...
### 使用合并专用分支进行拉直
    ...
    最好开发分支也统一规定只能使用拉直形态，这样甚至不需要使用合并专用分支了
...
问题：两分支合并，因为开发分支的 commit 点的重整比较繁琐，而且容易搞乱，很多人都因此放弃整理，直接合入。
    ...
    如果不整理，太多的commit点使得分支形态不够简洁，一般开发分支都是各种分叉的形态
        ...
        开发分支 a---b---c ------- hotfix ------ new1 --- new2 (HEAD)
                        \                    /        /
                        d---e feature1 --- /        /
                        \                           /
                        f---g  feature2 --------- /
...
解决也简单，先恢复现场到某个提交点，然后直接拷贝文件往回覆盖，你自己要改动的文件内容自己最清楚。
