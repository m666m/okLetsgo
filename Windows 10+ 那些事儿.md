...
以前的光栅游戏，一般是调整 LOD 对应画质细节。4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器分辨率 4K 的优化技术、DLSS 等等歪招全来了。

    如果感觉你的游戏中画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节拉到最高，抗锯齿选项也拉到最高，还是没有效果，把“锐化”相关选项拉高再看看，可以解决大部分画质糊的问题。注意锐化数值太大会让亮处噪点明显，树皮的纹理变得不真实等，酌情调整。
...
光线追踪的游戏为什么会糊：
    ...
    “散粒噪声” 会一直存在。在相机上，尚且可以用更大的光圈和更好的CMOS来缓解这个问题，但是在游戏里，只能发射更多光线或者用算法降噪。前者需要庞大的算力，4090在这面前也只能说是“小小的也很可爱”。而后者，因为光线不足，而且为了消除噪点，不得不做大力的降噪，抹除了噪点的同时，也抹除了大量画面细节。

        目前折衷的办法就是加强漫反射的效果了，让用户接受材质糊一点，但真实感强。

4K 显示器流行以来发展的 TAA 反锯齿:以模糊的方式解决物体边缘的锯齿问题。所有凹凸表面的边缘包括光线穿过的迷雾甚至不同颜色的相接边缘等都会被软化，整体画面观感就是糊一点。不过，调高锐化，或把显示器从 1080p 升级到 4K 甚至 fps 帧率更高都可以大大缓解这个现象。实在不行可以尝试更换反锯齿算法为传统的 8xSSAA 等来获得更清晰明亮的画面。
...
有些游戏 2k 分辨率拉满设置里的抗锯齿选项，实际画面还不如 4K 显示器开 DLSS 性能档，没有抗锯齿也不影响画面体验帧数还高
