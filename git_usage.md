...
先让主干分支从远程更新，使用 `git pull` 让 git 自动选择快进合并或分叉合并，而不要做强行拉直 `git pull --rebase` 。这样的好处是不会修改主干分支的提交记录。一般来说，主干分支上拉直合入都是 hotfix，分叉的合入是各个功能点。
