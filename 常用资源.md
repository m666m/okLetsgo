...
如果在安卓系统的权限提示中选择禁用，有些比较下作的聊天软件，比如微信QQ等直接不能使用。
...
目前看，可以正常使用手机各项功能，又能够限制app乱读取发送用户数据的办法，只能是在安卓系统上再运行个虚拟机系统，这个虚拟机系统完整照搬当前手机的安卓系统，包括硬件信息，但是软件系统和数据都是原始系统的。把app装到这个虚拟机系统的安卓里，这样它无法越界到宿主机，读取宿主机的应用程序信息。

三星：

    使用三星knox技术的“安全文件夹”，把应用程序添加到“安全文件夹”，就是单独的虚拟系统运行了，理论上是把用户的原生系统单独克隆了一份，原生系统里安装的app不会克隆进来，就是干净的新手机状态。把不安全的app装到这个原生系统里，让他对着一份干净的安卓系统流氓去吧。
