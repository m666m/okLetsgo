...
光线追踪和路径追踪
    ...
    N 卡 4080 时代宣传的路径追踪，逐渐的在抛弃光栅化渲染，使用光追的路径追踪算法来形成画面，极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 抛弃光栅化渲染而是完全使用光线追踪，为了降低显卡负担，各种场景中绝大部分材质的粗糙度设置都是不反射，动态光源的使用非常克制，很少放置玻璃和镜子，镜面效果只见于水面和地面的水洼。各种的降低视觉效果换来的只是画面整体的光影真实感更强一些，功耗近 400 瓦的 5080 显卡开路径追踪都跑不到 60 帧/秒。估计想看惊艳的光影效果还得再过2代。
