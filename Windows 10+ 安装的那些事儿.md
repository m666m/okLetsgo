...
Linux 与 Windows 对于本地 RTC 硬件保存时间的理解方式不同：

    Linux 认为硬件时间为 GMT+0 时间，即世界标准时间UTC，根据当前的时区设置，显示中国本地时间也即东八区时间，为 GMT+8

    而 Windows 系统认为硬件时间就是中国本地时间。

因此，如果用户分别使用过两个系统，则每个操作系统的时间校准服务都会按自己的理解把从网络时间服务器上获取的时间保存到计算机主板上的 RTC 硬件，导致 Windows 系统下时间比正常时间慢 8 个小时。
...
### exFAT 格式的 u 盘无法存放4G以上的文件

这应该是格式化程序的 bug，重新快速格式化一次，格式还是选择 exfat，但要扩大分配单元大小，我选的 128k，格式化完成后，就能正常存放4G以上文件了。

FAT32/NTFS/exFAT 的特点：

FAT32 不能存储大于4G的单个文件，但因为历史悠久，设备兼容性最好

NTFS 采用“日志式”的文件系统，需要记录详细的读写操作，不断的读写导致闪存芯片寿命变短。最大的优点是遇到断电或突然拔出 u 盘的情况，可以回滚数据，损失小。

exFAT 的设计面向 u 盘，既可以存放大于4G的单个文件（单文件最大可达16EB），又可以减少额外读写，避免伤害u盘的寿命。
...
不要突然拔出u盘，要选择弹出 u 盘以便操作系统同步缓存的数据，不然容易丢失数据
...
USB 3.0         理论上最大传输带宽达 5.0 Gbps，即 640 MB/s

USB 3.1/3.2     标准居然来回改，在商家的干扰下混淆了很多概念，GEN1/GEN2 带宽差异，还可以双通道实现翻倍，所以无论 USB 3.1 还是 3.2，Gen1 x2 等效于 Gen2 x1。所以只能看最大传输带宽，到底是 10 Gbps 还是 20 Gbps。

日常使用中，20 Gbps 明显的感觉快，但是 40 Gbps 就感觉不出太大差异。

USB 4           低速 20 Gbps,高速 40 Gbps，同时支持 USB 3.2 和 TBT3 的传输，支持 100 瓦的功率充电

雷电 4           40 Gbps 跟雷电 3速度一致，但是提升了最低连接速度，支持 100 瓦的功率充电，支持 USB4

7200 转硬盘      缓存内读写速度夸张最高300Mb+，一般在 120 MB /s，缓存没有命中的就降速了，读写磁盘内圈或小文件速度更低

三星高速 U 盘     一般在 140 MB /s

tf 卡           一般在 120 MB /s

固态硬盘          普通速度在 1000 MB/S，高速的超过 7000 MB/S
