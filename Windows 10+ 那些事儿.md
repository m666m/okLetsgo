...
路径追踪
    ...
    计算机图形学算法中，路径追踪是光线追踪的一个实现方式

    现实游戏届的宣传受显卡厂商影响，N 卡 2080ti 时代宣传的实时光追，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现，比如 Doom:Eternal 开启光线追踪后“邪教徒基地关卡”各种不锈钢材质都在反射 slayer 的模型。而 4080 时代宣传的路径追踪，就是完全抛弃光栅化渲染，完全使用光追的路径追踪算法来形成画面，Doom:Then Dark Ages 反而不敢到处使用不锈钢材质了，绝大部分材质的粗糙度都是不反射，镜面效果基本只有水面，不敢放玻璃和镜子。
