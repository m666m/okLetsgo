...
#### 让你的桌面颜色更准确 --- Nvidia 控制面板的输出动态范围选择 “完全”

    https://www.zhihu.com/question/279556310/answer/1070407834

开了可以增加黑白明暗对比，增加色彩艳丽程度，白色更白黑色更黑，更鲜艳。但是如果显示器屏幕素质一般的话开着反倒不如不开柔和。我办公用的电脑反复试了，开“完全”觉得不开柔和，全白的时候刺眼。还有如果接电视由于信号格式问题一定要选限制。
...
如果你的显示器支持，选择 10bit，否则 8bit 即可。另外要注意连接线的规格，dp2.0、hdmi1.4 以上。
...
光线追踪流行后，如果感觉你的游戏中画质有点糊，不够清晰锐利，即使在游戏设置中把贴图细节拉到最高也不行。

在游戏设置里找找显示，把锐化相关选项拉高再看看，如果启用了 DLSS，可能是 DLSS 的锐化选项。另外，DLSS 的档位尽量选择质量或 DLAA，比性能档更清晰。

    Doom:The Dark Ages 锐化拉到 2~3 清晰度就跟之前的 Doom：Eternal 差不多了，不知道为啥这游戏出厂默认的锐化值拉的低。

锐化数值太大会让你的图形噪点明显，酌情调整。

3、还有个驱动程序级别的覆盖设置方式（用 steam 截图会得到原图）：

    打开 NVIDIA APP，左侧栏点击“设置”，点击打开 “NVIDIA 信息浮窗”

    运行你的游戏，进入游戏后按热键 alt+z，弹出的信息浮窗选择 “游戏滤镜”，使明朗、锐化和动态亮丽等功能都可以直接勾选启用。

4、开启 Nvidia DLDSR(深度学习动态超分辨率) 让你的1080P显示器秒变4K

    这个功能是显卡驱动级别的功能，不需要游戏支持

    但游戏需要支持 4k 分辨率

DLDSR 能够以更高、更细腻的分辨率渲染游戏，然后把画面智能地缩放至显示器的原生分辨率，在任意屏幕上为玩家提供 4K (3840x2160) 画质的图形效果

    DSR（虚拟超级分辨率）功能是 Nvidia 在 2014 年推出的，现在升级了叫 DLDSR(深度学习动态超分辨率)，号称效果更好 DLDSR 2.25x 相当于 DSR 4x。

开启方法：

    打开 NVIDIA 控制面板，从“管理 3D 设置”>“DSR因数”，然后选择“DL缩放”就可以启用 DLDSR 了，调整渲染时放大的倍数即可。旧版缩放指的是 DSR，1080P 显示器选择 4x 则分辨率变成 4K (3840x2160)。DSR 4.00x就是1颗显示器像素显示的内容是显卡用4颗像素模拟计算后得出的 。

    然后 Windows 桌面进分辨率设置，分辨率选项已经有 3840X2160 的选项了，说明 DSR 已经成功开启。（桌面用如果字体显示扭曲发糊考虑调整字体缩放）
...
效果
    ...
    如果开 DLDSR + DLSS 有噪点降低锐化也不管用，换成 DLDSR + TAA 试试，大型 3A 游戏一般都可选择 TAA。
...
5、同理，还可以利用 Nvidia RTX视频增强（Super Resolution），获得视频网站和直播平台的清晰度提升

    在 Windows 系统设置中，选择“屏幕”-->“显示卡”，点开 “高级图形设置” 确认勾选了 “硬件加速GPU计划”。

    Nvidia 控制面板，打开 “RTX 视频增强” 选项：包含 “RTX视频超分辨率技术(RTX Video Super Resolution)”和 HDR。这样在浏览器播放网络视频时可以进入 HDR 状态并增强画质。
...
1、如果是游戏暗处太黑（含光线追踪），可以在游戏内设置调整 伽马值，适当放大，以暗处亮而亮处不发白为准即可。

    最好别调显示器亮度，不然退出游戏到桌面后太亮还得再调，太麻烦了。

2、驱动程序级别的覆盖设置方式（用 steam 截图会得到原图）：

    这个是显卡驱动级别的功能，不需要游戏支持。

开启 Nvidia RTX 动态亮丽，让你的游戏暗处不再那么黑

    打开 NVIDIA APP，左侧栏点击“图形”，然后点击选择你的游戏。

    右侧栏目，在 “游戏内设置” 中找不到选项就是游戏不支持，需要在下面的 “驱动程序设置”，点击 开启RTX动态亮丽，这样驱动程序的设置就会覆盖你的游戏的设置了，老黄手长
...
2、HDR 技术用于家庭影院很好，但不太适合做电脑显示器
    ...
    因为人一般都是在室内相对偏暗的环境下用电脑的，27 英寸 HDR1000 显示器的亮度大概相当于近距离面对 60w 的白炽灯，会感到很晃眼
...
显示器开启 hdr 模式后，doom 会自动检测到，自动跟随调整颜色。或者在 doom 的设置中选择开启 hdr，会自动设置显示器开启 hdr 模式。

最好关闭垂直同步，否则锁帧。另外如果头晕，记得关闭垂直同步，我玩射击游戏，必须关这个才不晕。
