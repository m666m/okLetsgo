...
所以，制定了国际化统一标准通用编码规则 UTF，目的是分区域的把全人类用的字符都装里面叫 unicode ，目前的互联网和 Linux 系统一般使用的是 UTF-8 编码规则，使用UTF规则进行编码是大势所趋。

全球大多数软件都操作系统设置的本地规则编码解码字符，即读取操作系统的区域语言设置。如果操作系统设置为是 gbk，这些软件默认设置下是无法显示 UTF-8 编码的文件的。

比如中文版 Windows 的默认代码页 ANSI 是 GBK（只要你的程序是基于 WIN-CRT 框架的，而不是 .net 等后期新框架.net 等后期新框架），由于一般文字编辑是跟随当前操作系统代码页的设置，导致你打开个utf-8编码的文本文件都无法正常显示，需要手动选择切换编码方式。包括微软的 Windows、Office 的各种本地化版本，都有这种问题（参见 <https://docs.microsoft.com/zh-cn/cpp/c-runtime-library/code-pages?view=msvc-170> ）。大概在 Windows 10 的 2019 年版本才默认对 .appx 应用使用 UTF-8 编码。

如果你修改了操作系统的代码页设置为默认编码 UTF-8， Windows、Office 可以正常显示 UTF-8 文件了，打开 gbk 编码的文件如果软件判断错误，则又乱码了，想正确显示还是得手动指定编码规则。
...
那统一了 .py 文件的编码方式用 utf-8 就完事了么？ 不！程序中输入输出流的字符串编码方式，对本地化 Windows 来说，默认是 gbk。

这俩还没有统一，带来的问题根源：

很多包或内置函数都不指定字符编码方式，即默认使用当前操作系统的代码页设置，如果你使用的是中文版 Windows，如果不指定转换编码规则，读取到的字符串是 gbk 编码，而你的 .py 文件中的操作是按 utf-8规则，不一致就会报错。

所以，从输入输出流等获取到的字符串，需要统一转码为 utf-8 才能用。
