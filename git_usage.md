...
### Linux 下拷贝代码目录到 Windows 下使用

文件权限变化，导致 git 判断有改变，其实只是操作系统差异，而且不建议在 Windows 下修改文件权限。

让 Git 忽略文件权限变更即可：

    # 此配置只是让 Git 忽略权限差异，不会改变仓库历史记录。
    $ git config core.fileMode false

这样 git 不再跟踪文件的可执行位变化。之后 git status 就会变干净，那些模式差异不再显示为修改。
