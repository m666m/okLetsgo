...
3、不要在 Windows 桌面环境开启 HDR 模式，仅在相关应用中单独设置开启，原因见章节 [显示器开启 HDR 后 Win10 桌面变灰泛白的原因]。
    ...
    大多数游戏支持在自己的选项设置里开启 HDR，这样在开始游戏后会自动切换显示器模式以显示 HDR 内容

如果你的应用不支持单独设置 HDR 模式，而是跟随桌面环境的 HDR 设置，那就只能在打开应用之前在桌面环境手工开启 HDR

    部分 HDR 显示器在自己的设置菜单预设了电影/游戏等 HDR 模式，按显示器上的菜单酌情选择
