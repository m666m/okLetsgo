...
关闭主板的 “快速启动 FAST Boot”

    这个功能在关机时会保留某些硬件的状态，是给 Windows 优化下一次开机加速启动时间用的。

    所以，如果前一次使用计算机时用的是 Windows，则关机后再次开机启动到 Linux，因为硬件的状态不是冷启动的重置状态，可能会影响操作系统稳定性。

如果使用双硬盘，好处是在开机启动的时候即进行选择，隔离性比一个硬盘多个分区安装 Windows 的隔离性要好

    单独安装硬盘分别安装 Windows 和 Linux，然后两块硬盘都安装到 pc 中。

    启动时主板一般会默认找第一块硬盘上的操作系统启动，在开机时按 F12 调出主板的启动菜单，它会列出所有硬盘上找到的可启动的项目，手工指定那个硬盘上的操作系统即可。

    如果在冷机启动时先进入 BIOS 设置屏蔽一个硬盘，可以更好的实现使用隔离性。
