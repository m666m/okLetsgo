...
以前光栅游戏时代，场景都是预烘培的，光线不会太黑。一般玩家只需要调整 LOD 即可对应画质细节，在游戏设置中 LOD 拉的越高画面就越清晰。

Nvidia 在2018年的 RTX 2080 开始推广光线追踪后，高端游戏的引擎技术发生了巨大的变化。如果你在游戏选项中把画质细节拉满了还是觉得画面糊，亮度拉高了暗部还是黑，大致原因就是游戏引擎切换为使用光线追踪了：

    2020 年代之前的游戏技术基本都是光栅化渲染画面的，可以实现光线的反射折射阴影，并依靠游戏开发时在场景中预烘培实现全局光照，这样已经可以给玩家非常真实的感觉了，贴图细节调高就可以看到非常清晰的画面。

    但是，画面显示技术想再进一步，实现照片级的真实感，光栅化处理做不到，而光线追踪可以。特别是处理汽车表面这种曲面的反射和镜面对照这种情况，光线追踪才能做到。

光线追踪的原理是对玩家屏幕上的每个像素点都进行实时计算：

    一个像素点投射n多个光线，打到各个物体上，反射后如果达到光源如灯、太阳，则屏幕上的这个像素点就发什么颜色的光。其计算混合了物体的材质、粗糙度，光源的颜色等多种因素，实际显示效果比传统的光栅化处理更加真实，即像照片那么真实。

光线追踪的先天缺陷一：达到 1080p 下可以接受的画面，光线追踪的计算量都超级大

这个技术从 2000 年代就开始探讨用显卡实现，随着人类电子制造技术的进步，直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才成为了现实，体验见 赛博朋克2077。如果计算光线的二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降30%+。
...
最终，我们得到了充满了各种妥协的光线追踪游戏画面：
    ...
    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。这不是你的错觉，这就是 TAA 的特点。从 UE5 引擎的各种游戏，到 Doom:The Dark Ages 全都切换到使用 TAA 技术了。
...
另外游戏帧数不足 60 帧的时候，有的游戏引擎会用动态模糊来增加流畅感，参见 [游戏不跟手 --- 延迟]。
...
或者尝试不使用 TAA，更换反锯齿算法为传统的 8xSSAA，或者切换到 DLSS4 并拉高锐化数值。
