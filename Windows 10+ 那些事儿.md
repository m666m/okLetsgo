...
4、显示器从 1080P 换成 4K，画质也能有明显提升。
    ...
    2k 分辨率拉满设置里的抗锯齿选项如 dlaa，实际画面还不如 4k DLSS 的性能档。这是因为 4K 的像素足够画面几乎没有锯齿，而且开抗锯齿还特别消耗显卡处理能力。所以 4K 显示器非常适合开 DLSS 性能档，不影响画面体验帧数还高。
...
光线追踪的某些游戏，对光线直射和背光混杂的视角变化处理不好，亮处很亮而暗部画面很黑，除非转头只看暗处才能看清，在开启 HDR 时更明显，这个不是什么黑屋出门看强光的人眼效果，这个是光照曲线有问题，根源在于游戏的设计和调校。不好的例子是2077的白天场景物体的暗部动态死黑，有种白天在街上转头却想打手电筒的感觉。好的例子是 Doom:The dark ages，光线追踪全局光照（RTGI）调校的好明暗过渡自然，高光给人冲击时暗部也有足够的亮度能看清细节，没光的山洞里也会细心的放置光源，不需要打手电。这也是为什么 2077 玩家都喜欢秀夜景的原因，因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。
...
手调显示器亮度对比度的方式不适合现代游戏
    ...
    光线追踪的游戏，对比度亮度都是动态的，全局光照的曝光度也随着视角和环境变化，这都跟软件的设置有关，单纯调节显示器的亮度没用。
...
2、游戏内设置，如果支持就把光线追踪的“路径追踪”选项打开。这样可以利用光线的多次反射，即使物体是有粗糙度的材质也会稍微变亮。另外，火焰、灯光等自发光材质也能稍微照亮周边，这些都可以增加一点场景中物体的暗部亮度。
    ...
    现实游戏届的宣传受显卡厂商影响，N 卡 2080ti 时代宣传的实时光追，其实整个画面还是使用光栅化渲染的，只有反射、环境光这些使用光追来实现。而 4080 时代宣传的路径追踪，就是完全抛弃光栅化渲染，完全使用光追的路径追踪来形成画面。

比如 Doom:Then Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，因为粗糙材质可以大大减少反射的计算，场景中的动态光影和阴影特效也用的非常克制，场景内反射远景的物体很少。但是物体的暗部可以看清，死黑很少见。开启“路径追踪”后，画面整体的亮度增加了，清晰感更强。
