...
2020 年代之前的 3D 游戏都是光栅化技术来渲染画面，可以实现光线的反射折射阴影
    ...
    依靠游戏开发时在场景中预烘培实现全局光照，美工方面多下功夫就可以给玩家照片级的真实感了，贴图细节调高就可以看到非常清晰的画面，而且场景中预烘培的光线充足各处都不会太黑。比如 2007年的 《孤岛危机》、2016年的《神秘海域四》、2018年的《荒野大镖客2》、2020年的《美国末日2》等，场景美轮美奂。

    在游戏设置中贴图细节选择越高、LOD 拉的越高，画面就越清晰。

但是，如果 3D 游戏画面显示技术想再进一步，实现动态效果的照片级真实感，比如像过场动画一样的游戏效果，光栅化处理技术就有很大的限制：

    比如光源无法变化：阳光射入的室内通过预烘培的光照能给你照片级真实感，但是太阳这个光源不能移动，场景也不能被破坏和移动，导致可交互性很差。自由运动的游戏人物和角色，在光线变化时很难跟场景中光线的色调协调，比如你会看到游戏角色跑进黑洞但仍然发白的现象。光栅化也无法实现汽车表面这种曲面的反射和镜面对照。另外，对规模越来越大的开放世界场景的 RPG 游戏来说，预烘培生成的光照信息，可能高达几十TB甚至更多，这已经无法部署到玩家硬盘了。
...
如果是计算光线路径的多次反弹即所谓“路径追踪”，间接光下的漫反射制造的光影真实感更强，路径追踪的二次反射是在第一次路径追踪的反弹点上递归，再计算路径追踪的多次反弹，计算量更是爆炸，帧数一般暴降 +30%。
...
最终，我们得到了充满各种妥协的光线追踪游戏画面：
    ...
    因为优化光线追踪就得模糊处理，所以游戏开发中对物体的材质在处理时就加入粗糙度，这是降清晰度的，即便是高清材质也要模糊处理。
...
光线追踪和路径追踪的概念：
    ...
    N 卡 4080 时代宣传的路径追踪是增加了计算光线路径的多次反弹实现漫反射，逐渐的在抛弃光栅化渲染，更多的使用光追来形成画面，这极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 号称完全使用光线追踪渲染画面，跟上一代的 Doom:Eternal 相比只是光影感觉更真实，画质细节完全没有进步，远看不错贴近就是糊。
...
在开放场景中常见背光面死黑、白天屋里很黑这种高光强对比度让人眼睛不舒服的情况，根源在于光线追踪迫于计算量优化不能放置太多光线，这就非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校。
...
这不是什么黑屋看门口强光的 HDR 过曝效果，这就是游戏对直射光和非直射光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：
    ...
    塞伯朋克2077 也是类似的问题，白天的时候物体的背光面死黑，屋里也死黑。在阳光明媚的大街上转动视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑就像犯了低血糖。不要说原本就是黑的，你只看暗部的时候它会慢慢变亮给你正确的计算结果。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站在大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。
...
游戏内的设置，开启 “路径追踪”，可以增强漫反射的效果，提升画面整体的亮度和明暗表现，真实感更强。“路径追踪”开启了光线路径的多次反弹，非镜面的反光物(有粗糙度的材质)也会投射光线到其他物体，使周围环境稍微变亮或增加阴影。灯条等自发光材质也能稍微照亮周边物体。

    路径追踪会增强间接光照射物体的明暗感觉，真实性更好。判断是否使用了路径追踪，可背向光源看反光面，是否反光到其他物体，那些物体是否改变了亮度和颜色，比如光线直射的桌面，是否照亮了周边的墙面和物体、灯光直射的地面，不被直射的周边墙面也会跟随改变颜色和亮度，而且门框等凹凸物体也会出现阴影轮廓。

    理论上路径追踪也会增加对材质细节的表现，因为像素的采样精度提升了，画质会更清晰。
