...
这应该是格式化程序的 bug，重新快速格式化一次，格式还是选择 exfat，但要扩大分配单元大小，格式化完成后，就能正常存放4G以上文件了。
...
FAT32/NTFS/exFAT 的特点：
    ...
    exFAT 的设计面向 u 盘，既可以存放大于4G的单个文件（单文件最大可达16EB），又可以不额外伤害u盘的寿命。缺点也是不要突然拔出u盘，要选择弹出u盘以便操作系统同步缓存的数据，不然容易丢失数据。

        目前 Linux 对这种文件系统的支持还可以，如果你的 u 盘要经常在 Windows 和 Linux/MacOS 之间拷贝文件，推荐用这种格式。

        很多移动硬盘的默认文件系统是 NTFS，买到手先重新格式化为 exFAT，然后拷贝个大于 4GB 的iso文件试试。

        Windows 10+ 对高速 U 盘会自动关闭写入缓存，这样使用后不需要菜单选择弹出 U 盘的繁琐了。

U盘用什么格式，取决于插入设备的支持情况

    如果是用于 Android 的电视机：那么必须使用 FAT32 格式，且U盘总容量不能超过 32GB，不然会有莫名其妙的问题。

    如果只是 Windows 电脑上使用，32GB 容量以内选择 FAT32，大于32GB容量的 exFAT 或 NTFS 都是好选择。
