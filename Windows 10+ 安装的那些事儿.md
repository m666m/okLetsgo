...
### 安装双系统
    ...
    启动 Linux 后，可以看到 Windows 所在的 NTFS 硬盘分区，可以挂载使用，但是强烈建议只读，因为 Linux 对 NTFS 的支持一直无法完善。

    双启动仍无法保护 /boot 分区，在操作系统引导阶段对木马监控程序的防护差，建议操作系统安装时开启 Secure Boot 功能。

一个硬盘划分多个分区，可以安装多个操作系统：

    建议先安装 Windows 10，再安装 Linux，这样 grub 引导加载器就能直接扫描到已存在的 Windows 10 系统，并将其添加到启动界面中。这样开机时会出现 grub 菜单，用户可以手工选择启动哪个操作系统。

使用双硬盘，隔离性比一个硬盘用多个分区安装多个操作系统要好，且不涉及 gurb 引导菜单

    pc 中每次只安装一个硬盘分别安装 Windows 和 Linux，然后把两块硬盘都安装到 pc 中。

    启动时主板一般会默认找第一块硬盘上的操作系统启动分区直接引导。在开机时按 F12 调出主板的启动菜单，它会列出所有硬盘上找到的可引导的启动分区（如果一个操作系统有多个选项说明可以分别选取UEFI/BIOS方式），手工指定你要引导的操作系统即可。

    可以在冷机启动时先进入 BIOS 设置屏蔽一个硬盘，这样可以更好的隔离两个操作系统。
