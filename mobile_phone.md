...
根源在于你去运营商申请的手机号，有很多是二次销售的手机号码，这个是不告知你的。

用户新号被激活后，可立即通过运营商App搜索“二次号码焕新”，一键解绑前号主遗留的互联网应用绑定，也可以借助微信“一证通查”小程序，查询手机号绑定的互联网账号，及时清理非本人注册的关联信息，降低骚扰风险。

### 浏览器或 app 弹出 “xxx想要查找并连接到本地网络上的任何设备”

怎么来的：

    在iOS 14以前苹果手机中，并没有这个授权选项，App可以随时探测到本地网络里所有设备的名称和MAC地址，由于MAC地址是唯一的，则为关联、追踪用户的行为、画像数据提供可能，例如，用一台新的电脑和手机连接同一 Wi-Fi，这时App对本地网络扫描识别到新设备及其类型，在获取MAC地址后，就可以把手机上的一些购物潜在需求推送到电脑的购物网站或广告中。

    说白了，“查找并连接到本地网络上的设备”的使用为跨平台广告提供了方便，而从个人信息保护和隐私保护体验的角度来讲，这种对设备的监控、跟踪可能会给用户带来担忧。

    edge 和 chrome 浏览器也都提供了向网站提供你的本地网络设备的功能，只是现在被隐私保护法规限制而弹一下提示罢了。。。

拒绝会有什么影响呢？

    现在绝大部分APP都不会用到“查找并连接到本地网络上的设备”权限，所以我们可以直接选择，如果已经打开可以在“设置-找到某APP-关闭“本地网络”

    有部分APP存在使用该权限的实际需要，常见的比如视频APP投屏、智能家居App等需要连接到同一个本地局域网才能够实现相应的功能，当用视频APP向电视投屏时，如果没有授权“查找并连接到本地网络上的设备”权限则无法向局域网的电视进行投屏播映（见下图），当使用某智能家居APP时，不打开该权限，则无法搜索、发现智能家居设备（见下图）。类似的还有需要本地联网进行游戏的App等等。

    edge 浏览器访问网址 edge://flags/#local-network-access-check

    chrome 浏览器输入：chrome://flags#local-network-access-check
...
Android 提供了 “Work Profile(工作空间)” 功能来提供一个隔离的空间，用户可以在其中安装应用程序或把当前手机环境中的应用添加到其中
...
开启 “Work Profile(工作空间)” 之后，系统分为了两个空间，为方便描述，我们把宿主机安装普通应用的空间称为 personal profile（个人空间），虚拟机安装图标上带有公文包徽章的应用的空间称为 work profile（工作空间）。
...
工作空间需要有 Work Profile owner 程序进行管理，比如安装应用和赋予访问宿主机的某些权限等。

    Owner 这个角色设计上是由公司统一部署 owner 程序，由这个程序在工作空间安装各种公司控制的软件。

    对个人自用的手机来说，Owner 程序使用 google 账户即可，但这样会把自己工作空间的控制权交给 google。

    通常我们使用 [Work Profile 隔离 ---- 三星 knox 安全文件夹] 和 [Work Profile 隔离 --- 开源的 Shelter]。
...
工作空间和宿主机的资源交换
    ...
    宿主机里的图片、音频、视频目录里的资源，是默认共享给工作空间的，也即在工作空间中打开的应用都可以使用。

    工作空间的应用保存的资源如图片，是无法直接被宿主机看到的，需要在应用中明确选择“分享”、“下载”、“使用其它应用打开”等方式，保存到工作空间的目录中。

    批量交换资源，需要使用支持工作空间的文件管理器，以 Google Pixel 手机自带的文件极客为例：

        在 Google Pixel 手机的主屏，上滑打开应用列表，会用标签页分类列出“个人空间”和“工作空间”的应用，打开文件极客可以分别浏览对应的图片等资源。

        打开“工作空间”的文件极客，在下载或图片目录，多选你的文件，点击工具栏的“分享”按钮，会列出“个人空间”和“工作空间”的可用应用，点击“个人空间”，在列出的可用功能选择“文件极客（下载）”，就会把你选择的文件，保存到宿主机的“下载”目录中了。
...
Shelter：利用安卓的 "Work Profile" 功能实现应用程序的套娃运行，适合在 Google Pixel 手机上安装。
...
Shelter 是一款免费开源（FOSS）应用程序，它利用安卓系统的 "Work Profile"（工作空间） 功能来提供一个隔离的空间，用户可以在其中安装应用程序或把当前手机环境中的应用添加到其中。

Shelter 的安全性只能与 "Work Profile" 的实施一样，不会超越宿主机。

Shelter 的主要用例包括：

    在工作空间中安装应用程序以进行隔离

    在工作空间中“冻结”应用程序，以防止它们在您不主动使用它们时运行或被唤醒

    在一个手机上安装运行同一应用程序的两个副本

功能/用例

    在独立工作空间中运行应用程序，使其无法访问该工作空间之外的数据
...
工作空间和宿主机的资源共享：
    ...
    在工作空间的应用中保存图片：
        ...
        QQ/微信：点击“下载”，即可保存到工作空间公用的目录中，“图片”目录的子目录中

        知乎：点击“下载”，即可保存到工作空间公用的目录中，“图片”目录的子目录"知乎"中

            选择“分享”-->“更多”，列表中找“切换工作空间”、“下载”、“保存”等，可以选择打开宿主机的“文件极客”，直接保存到 宿主机本地的“Download”目录中。
...
Shelter 不是完整的沙箱实现，它不能保护您免受以下因素的侵害：
    ...
    同样，Shelter不能在一个Android设备上创建多个工作空间，也不能与管理工作空间的任何其他应用程序共存。这是由于Android系统的限制，我对此无能为力。

如何卸载 Shelter?

    1) 前往设置 -> 帐号，在里面移除工作帐号(Work Profile);

    2) 前往设置 -> 安全 -> 高级 -> 设备管理员，把 Shelter 的管理员权限关闭;
...
隐藏 私密空间
    ...
    在设置中可以选择在 Launcher/启动器中隐藏私密空间，这样在应用列表的底部就不能看到私密空间的存在了，可以通过搜索 ”私密空间“ 重新找到和打开它。

其他

    当私密空间锁定时，私密应用不再处于活动状态。

    通过 ADB 命令安装应用时，安装的应用将同时安装到普通空间和私密空间。

Android 隐私空间的主要优势

    高级安全性： 私人空间使用自己的身份验证，可能与设备的身份验证不同（PIN、图案、指纹等），提供 额外的保护层.

    应用程序和数据的完全分离： 隐私空间中安装的应用无法与主环境中的应用交互。你甚至可以使用不同的 Google 帐户，实现真正的应用与主环境中的应用分离。 个人和工作空间.

    隐藏私人空间： 私人空间锁定后，不出现在应用程序抽屉中也不在概览中，只有手动搜索才能访问它。
...
次选 google Pixel 手机，除了 google 全家桶，还真不收集你其它隐私了。。。
