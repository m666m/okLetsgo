...
TODO: 双硬盘的好处是在开机启动的时候即进行选择，隔离性比一个硬盘多个分区安装 Windows 的隔离性要好

    硬盘B启动后对自己开启 Bitlocker 加密，防止硬盘A的操作系统启动后访问硬盘B，避免A上木马监控程序的扫描和传播。
...
硬盘B（分区B）
    ...
    主机：安装 Windows，对自己开启 Bitlocker 加密，只做升级维护等管理用途，不在这里做日常使用。
         安装 PVE，对自己
...
eCryptfs 是文件系统级加密，针对的是文件和目录，不是对整个磁盘的加密。简单点，用 7zip 加密你的文件夹即可实现相同的目的。
...
你会注意到，eCryptFS 默认在你的家目录中创建了两个目录：Private和.Private。~/.Private目录包含有加密的数据，而你可以在~/Private目录中访问到相应的解密后的数据。在你登录时，~/.Private目录会自动解密并映射到~/Private目录，因此你可以访问它。当你登出时，~/Private目录会自动卸载，而~/Private目录中的内容会加密回到~/.Private目录。
...
#### Linux 使用 cryptsetup 加密磁盘
    ...
    https://blog.csdn.net/yifan850399167/article/details/80170215
...
前置知识 dm-crypt
    ...
    一般我们比较常用的是LUKS格式，Luks（Linux Unified Key Setup）是 Linux 硬盘加密的标准。通过提供标准的磁盘格式，它不仅可以促进发行版之间的兼容性，还可以提供对多个用户密码的安全管理。与现有解决方案相比，LUKS 将所有必要的设置信息存储在分区信息首部中，使用户能够无缝传输或迁移其数据。

    dm-crypt 是整合到 Linux 内核中的，而它的命令行前端 cryptsetup，在大多数主流的 Linux 发行版中都会自带，可以与 LVM 无缝结合.

创建一个虚拟的磁盘分区

因为创建加密磁盘的过程会清空整个磁盘分区的内容，为了方便，我这里用 dd 命令创建一个文件来代替真实的磁盘分区。

    $ dd if=/dev/zero of=/tmp/ENC_DISC.img bs=1M count=100

    $ ls -l /tmp/ENC_DISC.img
    -rw-r--r-- 1 lujun9972 lujun9972 104857600 4月  12 18:13 /tmp/ENC_DISC.img
...
虽然说 dm-crypt 支持多种加密格式，但最常用的还是LUKS。
...
这里常用的参数有下面几个:
    ...
    如：cryptsetup --verbose --verify-passphrase -c aes-cbc-plain luksFormat /dev/sdb1

这里我们直接使用默认值

    $ cryptsetup luksFormat /tmp/ENC_DISC.img

    WARNING!
    ========
    这将覆盖 /tmp/ENC_DISC.img 上的数据，该动作不可取消。

    Are you sure? (Type uppercase yes): YES
    输入 /tmp/ENC_DISC.img 的密码：
    确认密码：

执行的过程中，命令会警告你将会清除磁盘上的所有数据，并要求你输入两次密码。

其实 cryptsetup 不仅支持用密码加密，还支持用文件来加密，可以是任意类型的文件，不过解密的时候，要确保文件一个字节都不差，用这个特性可以方便地构建解密U盘。
...
比如，我们这个案例中可以执行:
    ...
    $ sudo cryptsetup luksOpen /tmp/ENC_DISC.img ENC_DISC
    输入 /tmp/ENC_DISC.img 的密码：

注意这里需要用到root权限。

执行该命令后就会将 /tmp/ENC_DISC.img 解密，并映射成 /dev/mapper/ENC_DISC

像普通磁盘分区一样操作 /dev/mapper/ENC_DISC

你可以像普通磁盘一样来操作 /dev/mapper/ENC_DISC

    # 创建文件系统
    $ sudo mkfs.ext4 /dev/mapper/ENC_DISC
    ...
    # 挂载该文件系统到目录
    sudo mount /dev/mapper/ENC_DISC /enc_dir

测试写入

    $ sudo touch /enc_dir/secret

    $ ls -l /enc_dir/secret
    -rw------- 1 root root 0 Apr 12 18:39 /mnt/secret

加密完成，创建好文件系统后，以后的使用只需要重复以下步骤即可

    mount /dev/mapper/ENC_DISC /enc_dir
...
同样的，由于需要初始化device mapper,因此需要root权限：
    ...
    sudo cryptsetup status ENC_DISC

结果为

    /dev/mapper/ENC_DISC is active and is in use.
    type:    LUKS1
    cipher:  aes-xts-plain64
    keysize: 256 bits
    key location: dm-crypt
    device:  /dev/loop0
    loop:    /tmp/ENC_DISC.img
    sector size:  512
    offset:  4096 sectors
    size:    200704 sectors
    mode:    read/write
...
LUKS格式的加密盘默认能够提供8个"key slot",每个"key slot"就是一个解密的钥匙，使用任何一把钥匙都能打开这个LUKS加密盘。相当于是能有多种解密方式。
...
比如
    ...
    $ cryptsetup luksDump /tmp/ENC_DISC.img
    LUKS header information for /tmp/ENC_DISC.img
...
使用keyfile打开加密盘的方式跟普通打开加密盘的方式类似，只是要多用一个 --keyfile 来指定keyfile的路径
    ...
    sudo cryptsetup --key-file /tmp/keyfile open --type luks /tmp/ENC_DISC.img ENC_DISC

删除keyslot

使用下面命令可以删除加密盘的其中一个key slot

    cryptsetup luksKillSlot /tmp/ENC_DISC.img 0
...
所以，使用带有核心引导UEFI的加密Linux发行版，比如 Qube OS，应该可以像大多数信息安全专家一样确保您安全。
...
这几个漏洞属于侧信道攻击(side channel)，居然覆盖了 1998 年到 2018 年的所有英特尔 cpu。。。
    ...
    虚拟机可以关闭这个缓解措施 https://kb.vmware.com/s/article/79832?lang=zh_cn

英特尔提供的解决方式，从bios刷新到操作系统补丁，整体降低cpu速度大概在 10% ~ 20%，特定应用有降速 60% 的。所以是否开启侧信道攻击缓解，需要斟酌决定

    家用电脑必须打补丁，恶意网站内嵌的js脚本跑几个循环就能获取你的密码了
