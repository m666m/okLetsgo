...
以前光栅游戏时代，一般是调整 LOD 对应画质细节，设置拉的越高越清晰。4K 分辨率流行后设计原则变了，渲染分辨率用 1K 然后放大到你显示器分辨率 4K 的优化技术、DLSS 等等歪招全来了，细节设置拉满还是会糊的主要原因有两个：
    ...
    光线追踪的游戏材质糊： “散粒噪声” 会一直存在。在相机上，尚且可以用更大的光圈和更好的CMOS来缓解这个问题，但是在游戏里，只能发射更多光线或者用算法降噪。前者需要庞大的算力，4090在这面前也只能说是“小小的也很可爱”。而后者，因为光线不足，而且为了消除噪点，不得不做大力的降噪，抹除了噪点的同时，也抹除了大量画面细节。目前折衷的办法就是加强漫反射，让用户接受材质糊一点，但真实感强。调高锐化数值也会改善清晰度。
