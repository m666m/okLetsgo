...
这应该是 Windows 的 bug，重新快速格式化一次，格式还是选择 exfat，但要扩大分配单元大小，格式化完成后，就能正常存放4G以上文件了。
...
FAT32/NTFS/exFAT 的特点：
    ...
    exFAT 的设计面向 u 盘，既可以存放大于4G的单个文件（单文件最大可达16EB），又可以不额外伤害u盘的寿命。缺点也是不要突然拔出u盘，要选择弹出u盘以便操作系统同步完成数据，不然容易丢失数据。
