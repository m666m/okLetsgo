...
从 2007 年代英特尔就尝试用多核心通用图形处理器（GPGPU）代号 Larrabee 来实现光追游戏 <https://www.gameres.com/853845.html>，甚至云游戏串流运行。但直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才真正成为了现实（光追游戏体验见 赛博朋克2077）。如果计算光线的二次反射即所谓路径追踪，间接光漫反射下的光影真实感更强，但帧数再爆降 30%+。
...
Nvidia 在显卡计算能力不足时强推光线追踪，导致了到 2025 年光线追踪游戏的画面效果还不如 10 年前的游戏：
    ...
    跟之前的光栅游戏相比，完全光线追踪技术的游戏画面真实感轻微提升，清晰度明显下降，反射效果明显提升，物体暗部死黑太多（减少使用预烘焙又不舍得多给实时光线），帧率巨幅下降，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 基本都是如此。注：UE5 引擎开发的《寂静岭f》画面效果倒退到了 2000 年代。
...
光线追踪和路径追踪的概念：
    ...
    N 卡 4080 时代宣传的路径追踪是增加二次反射和漫反射，逐渐的在抛弃光栅化渲染，更多的使用光追来形成画面，这极大的增加了对显卡硬件的消耗。比如 Doom:The Dark Ages 号称完全使用光线追踪渲染画面，跟上一代的 Doom:Eternal 相比只是光影感觉更真实，画质细节完全没有进步，远看不错贴近就是糊。
...
游戏内的设置，开启 “路径追踪”，可以增强漫反射的效果，提升画面整体的亮度和明暗表现，真实感更强。“路径追踪”开启了光线的二次反弹，非镜面的反光物(有粗糙度的材质)也会投射光线到其他物体，使周围环境稍微变亮或增加阴影。灯条等自发光材质也能稍微照亮周边物体。
