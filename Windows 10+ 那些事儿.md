...
现代的图像数码处理，除了增加色彩和光影细节，还在提升像素点的精细程度。对数码相机级来说，就意味着使用大的CCD采样，跟 1839 年的照片比起来，人类手绘的图画逼真程度已经无法超越现代的照片了。

我们的视频游戏追求逼真的感觉，也是在光影和细致程度上下功夫。
...
但是，3D 游戏画面显示技术想再进一步，实现动态效果的照片级真实感，比如像过场动画一样的游戏效果，光栅化处理却做不到：比如光源无法变化，阳光射入的室内通过预烘培的光照能给你照片级真实感，但是太阳不能移动，场景也不能被破坏和移动，可交互性很差。自由运动的角色，在光线变化时很难跟场景的光线色调协调。光栅化也无法实现汽车表面这种曲面的反射和镜面对照。另外，对规模越来越大的开放世界场景的 RPG 游戏来说，预烘培生成的光照信息，最多可以高达几十TB，这已经无法部署到玩家硬盘了。
...
这里就引出了题目中的疑问：
    ...
    如果你在游戏选项中把画质细节拉满了还是觉得画面糊，亮度拉高到暗部蒙了层白纱感但还是黑，大致原因就是游戏引擎切换为使用光线追踪了。
...
这个技术从 2007 年代英特尔就尝试用多核心通用图形处理器（GPGPU）代号 Larrabee 来实现光追游戏 <https://www.gameres.com/853845.html>，甚至云游戏串流运行。但直到 2018 年 Nvidia RTX 2080 显卡引入了 RT core 进行硬件加速才真正成为了现实（光追游戏体验见 赛博朋克2077）。如果计算光线的二次反射即所谓路径追踪，光影真实感更强，但帧数再爆降 30%+。
...
数字电子技术中涉及光线的运算，一直存在“散粒噪声”的问题。在相机上，可以用更大的光圈和更好的CMOS来缓解。但是在电子游戏里，只能发射更多光线或者用算法降噪。
    ...
    发射的光线数量越少，则计算量就越小，但是画面噪点会变多，整体变暗。参见 《Quake2:RTX》，可选择关闭滤镜查看原始光线追踪渲染的画面效果。

所以在光线不足条件下降噪是很现实的解决方案，高斯模糊等各种模糊方案就是解决王道。但是，模糊处理在抹除了噪点的同时，也会抹除画面细节，这是无法避免的现实。

最终，我们得到了充满了各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。而且角色的毛发会有锯齿，跑动中看到的草坪会从清晰变模糊，这不是你的错觉，这就是 TAA 的特点。可惜的是，从 UE5 引擎的各种游戏，到 Doom:The Dark Ages 全都切换到使用 TAA 技术了。

    因为优化光线追踪就要模糊，所以干脆对物体的材质处理时就加大粗糙度，这是降清晰度的，即便是高清材质也要糊一下才发布。

    光线追踪技术在反射、折射上的效果最明显，所以游戏场景都很喜欢给你到处放置水面，路面上都洒水。而镜子太消耗计算量，所以镜子和玻璃能不用就不用。。。

    在优化不到位的画面部位，你可以看到延迟生成的阴影。

    其实 Nvidia DLSS 也是 TAA，但用 AI 进行了清晰度修正，并新增了很多功能，比如从低分辨率生成高分辨率画面，用 AI 生成画面来减少计算量，甚至直接插帧生成新画面，各种歪招全都有。

总体来说，现在的光线追踪游戏，有点类似数码相机刚开始发展的时代，清晰度是不如传统相机的，但是在多次迭代后清晰度达标了，几年功夫就实现了对传统相机的替代。Nvidia 在显卡计算能力不足时强推光线追踪，导致了至 2025 年光线追踪游戏的画面效果甚至大幅倒退不如 10 年前的游戏：

    跟之前的光栅游戏相比，完全光线追踪技术的游戏画面真实感轻微提升，清晰度明显下降，反射效果明显提升，物体暗部死黑太多（预烘焙减少又不舍得多给实时光线），帧率爆降，从使用 UE5 引擎开发的各种游戏，到 Doom:The Dark Ages 基本都是如此。注：UE5 引擎开发的《寂静岭f》画面效果倒退到了 2000 年代。

    比如 Doom:The Dark Ages，号称完全使用光线追踪，其实是部分烘焙再加强制硬件光追，光影效果很细腻。但是计算量已经超级大了，为了减少反射的计算量，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。各种降低视觉效果，跟 DOOM2016/DOOM  ETERNAL比换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。<https://doomwiki.org/wiki/Tiago_Sousa>

    赛博朋克2077，糊的情况稍微好点，暗部死黑太难受了，这游戏最好晚上出门玩。
...
1、游戏设置里把动态模糊、胶片颗粒、电影质感、景深、色差、分辨率缩放、VRS 等选项关掉。
...
如果感觉你的游戏画面有点糊，不够清晰锐利，即使在游戏设置中把贴图细节等选项拉到最高还是糊：
...
比如 Doom:The Dark Ages 为了优化，把大多数的材质限制在粗糙度阈值之下，总给人画面比较糊的感觉，而把 “锐化” 拉到 3 左右，清晰感就类似过场动画了，跟 Doom：Eternal 的细节水平差不多。当然 doom 系列一贯的风格就是远景清楚近景糊。

驱动程序级别的覆盖设置方式：

在 nvidia 的驱动程序设置中，关闭 反锯齿 相关的各个选项，看游戏图像是否清楚了，建议在 4K 分辨率尝试这个操作，因为 4k 或更高的分辨率下不需要开启反锯齿了。
...
或者尝试不使用 TAA，更换反锯齿算法为传统的 8xMSAA，或者切换到 DLSS4 并拉高锐化数值。
...
或者开启 Nvidia DLDSR(深度学习动态超分辨率) 让你的 1080P 显示器秒变 4K：
