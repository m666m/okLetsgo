...
目前最主要的功能：
    ...
    agent mode:你说，帮我优化下 xxx 代码，他自动替你改了。会自动调用相关工具进行分析、直接修改你的文件，甚至编写并运行测试用例，发布到 github，统统自动完成。

        Copilot 能推断并存储关于代码库的有用信息，并在后续工作中利用这些信息提升输出质量

        老旧框架迁移到新框架，一键搞定你服吗

        扩展商店输入 @mcp 会开启 MCP Server 商店，开启更多的大模型调用工具

        智能体技能 (Agent Skills)，执行工具的自动化流程，你只需要布置任务要求 agent 保存为 skill 即可。或者手工编写。或者在扩展商店搜索 @agentPlugins，一个插件可以包含Skill、MCP Server等，非常适合一键安装复杂的工作流
