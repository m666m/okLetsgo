...
最终，我们得到了充满了各种妥协的光线追踪游戏画面：

    TAA 反锯齿: 以前后帧动态模糊的方式解决物体边缘的锯齿，也适合处理光线追踪中出现的噪点。所有凹凸表面的边缘，包括平面上不同颜色的相接边缘，甚至光线穿过的迷雾等都会被软化，整体画面的观感就是“糊”。而且角色的毛发会有锯齿，跑动中看到的草坪会从清晰变模糊，这不是你的错觉，这是 TAA 的特点。可惜的是，从 UE5 引擎的各种游戏，到 Doom:The Dark Ages 全都切换到使用 TAA 技术了。

    因为优化光线追踪就要模糊，所以干脆对物体的材质在处理时就加大粗糙度，这是降清晰度的，即便是高清材质也要糊一下才发布。

    光线追踪技术在反射、折射上的效果最明显，所以游戏场景都很喜欢给你到处放水洼，路面上都洒水。而镜子太消耗计算量，所以镜子和玻璃能不用就不用了。

    在优化不到位的画面某些部位，你可以看到延迟生成的阴影。
