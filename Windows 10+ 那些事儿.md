...
通常射击游戏在设计时会针对最常见的使用场景调整设计目标，比如帧率 60帧/s。其在 60hz 显示器上的帧间距是 1ms，一般原生画面的渲染延迟大概在 10 ms 左右，不会影响手感，视觉上也不卡顿

    手感卡顿：渲染延迟指的是画面生成到显示器呈现的时间，渲染延迟过大会影响手感，晃动鼠标会感觉到画面里的人物动作跟不上，即不跟手

    画面卡顿：帧间距指的是两帧之间的时间，帧间距过大会影响观感，即眼睛看着画面感觉卡顿

举个例子，你的显示器刷新率为 300hz 情况下锁 30 帧渲染延迟有 1ms，锁 300 帧渲染延迟有 4ms，在实际体验上，锁 30帧/s 虽然 1ms 的渲染延迟不影响手感但是帧间距大会感到画面卡顿，而锁 300 帧的帧间距小眼睛看着更流畅，而且 4ms 的渲染延迟是不影响手感的，就应该选择锁 300 帧。有些低端的电视机的响应时间就很长，但有的人在这种电视上打游戏感觉不出来滞后感（也就是“卡”），但如果游戏的每秒帧数从 60 帧降到 20 帧他就会觉得“卡”。帧间距过大可以用动态模糊骗掉眼睛，从而看起来很流畅；但是延迟的画面却不可以。

1、每秒生成的帧数不足以覆盖显示器刷新率，帧间距过大造成画面卡顿：

    30 帧/s 时在 60hz 显示器上会感觉到画面的卡顿

    60 帧/s 时在 120hz 显示器上会感觉到画面的卡顿

帧数过高也可能影响画面观感，因为显卡送到显示器的数据太快也需要缓冲，反而排队了。

所以电竞选手一般选择高回报率鼠标和支持可变刷新率如 Nvidia G-sync 标准的高刷显示器。

例外：把 CS2 和 CSG0 都锁帧到 300帧/s，操作手感上 CS2 就是迟滞，这是游戏引擎的问题。
...
针对游戏原生输出的画面，显卡重新计算甚至用帧生成插入 AI 帧，都会增加渲染延迟。

但是

    开 DLSS 帧生成 + Nvidia Reflex 后，延迟反而很神奇的下降了，而且不需要锁帧，无需开垂直同步。

所以，对光线追踪+路径追踪的游戏，推荐开启 DLSS:DLAA 档或质量档，然后开启 DLSS 帧生成 + Nvidia Reflex，延迟是可以接受的，画面比游戏原生还更清晰。
