...
最终，我们得到了充满了各种妥协的光线追踪游戏画面：
    ...
    光线追踪技术在反射、折射上的效果最明显，所以游戏场景给你到处放置水面，路面上都铺水。镜子太消耗计算量，所以镜子和玻璃能不用就不用。。。

    在优化不到位的画面部位，你可以看到稍后生成的阴影。
...
总体来说，现在的光线追踪游戏，有点类似数码相机刚开始发展的时代，清晰度是不如传统相机的，但是在多次迭代后清晰度达标了，几年功夫就实现了对传统相机的替代。Nvidia 在显卡计算能力不足时强推光线追踪，导致了至 2025 年光线追踪游戏的画面效果甚至大幅倒退不如 10 年前的游戏：
    ...
    比如 Doom:The Dark Ages，抛弃了光栅化渲染而完全使用光线追踪，为了减少反射的计算量，场景中绝大部分物体的材质都限制在粗糙度阈值之下，远景清楚贴近看都是糊，镜面效果只见于水面和地面的水洼，很少放置玻璃和镜子，反射远景的物体很少，动态光源的使用也非常克制。各种降低视觉效果，换来的居然只是画面整体的光影真实感更强一些，功耗近 400 瓦的 RTX5080 显卡开路径追踪后都跑不到 60 帧/秒。<https://doomwiki.org/wiki/Tiago_Sousa>

    赛博朋克2077，糊的情况稍微好点，暗部死黑太难受了，这游戏注意晚上出门玩。

    所以 2025年发布的游戏 战地6 直接宣布当前阶段不使用光线追踪技术，优先保证玩家体验。

玩家可用的缓解办法，见下面的子章节。

彻底解决办法：

    想看清晰逼真的画面还得再等几代显卡，光线追踪的渲染从 DLSS 发展到神经渲染（AI 技术神经网络机器学习），最终实现照片级效果，这次，全场景都是动态的。
...
很多游戏都使用自动曝光技术，根据视野焦点把暗部慢慢给你调亮，导致被光线直射的物体暗部出现死黑，只看暗部又会慢慢变亮，让你的眼睛非常不舒服。这不是什么黑屋出门看强光的 HDR 过曝效果，这就是游戏对光线直射和背光混杂的视角变化处理的不好，完全属于游戏的设计和调校问题：
    ...
    比如 赛博朋克2077 的白天场景各种物体的背光面死黑，在阳光直射的大街上转动视角就有想打手电筒的感觉，原地蹲下屏幕四边都发黑就像犯了低血糖。。。为什么 2077 玩家都喜欢秀夜景？因为夜景不介意暗处黑，站个大灯底下看彩灯实在太酷炫。像 2077 这样粗暴的光污染却契合了赛博朋克风格，属于取巧了。

    好的例子是 Doom:The dark ages，全局光照（RTGI）调校的好，高光给人冲击时暗部也有足够的亮度能看清细节，明暗过渡非常自然，面向明暗交错的物体转动视角时眼睛完全不累。在场景设计方面，低光的山洞或水下也会细心的放置光源，死黑很少见。
...
这种在开放式场景中背光面死黑的游戏很常见，根源在于光线追踪迫于计算量优化不能放置太多光线，这就非常依赖美工对材质风格的统一处理和具体场景设计时的光影调校，大多数厂商不是实力不足就是不想花太多时间进行调校：
