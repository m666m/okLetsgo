...
简单应用场景下只需要一条命令即可
    ...
    本地开发分支从远程更新，一般要保持拉直状态不能添加分叉，则执行 `git pull --rebase`，强行给本地变基拉直合并远程。如果没有不许分叉的限制，则执行 `git pull`，让 merge 自动选择即可。

    本地主干分支从远程更新，一般做分叉合并，原因见章节 [功能分支合并到主干分支的操作策略]。
...
这样就不能直接做 `git pull` 或 `git pull --rebase`，把拉取和合并分开做。先把远程拉取下来看看内容，然后才能明确选择做何种合并。合并策略的讨论详见章节 [分支合并：merge菱形分叉还是rebase拉直]。
...
TODO:酌情考虑如下策略：
    ...
    功能分支合并到主干分支要分两步
        ...
        功能分支先合并主干分支，应该用分叉合并，如果功能分支的提交记录的 hash 值变更不会引起混乱（远程多分支变更导致其他人的混乱，缺陷管理对应不到提交记录），才使用变基合并做拉直。变基合并见章节 [变基合并远程分支要先合并到自身]。

        然后主干分支合并功能分支，用分叉方式合并到主干分支。

        详见章节 [功能分支合并到主干分支的操作策略]章节
...
·开发分支拉取自主干分支的节点 c，在此基础上延续，采取拉直的合并策略：各开发人员本地的开发分支从远程拉取，规定只能使用 `git pull --rebase` 的方式，这样变更的只是开发人员本地的提交记录的 hash 值，保持远程的提交记录 hash 值不变，以此来保持开发分支是拉直延续的。参见章节 [更新本地分支的操作步骤]。

    feature1分支 a---b---c
                          \
                            ---f---g  分支feature1 自己的提交记录

·主干分支修复bug，单独拉分支 hostfix，改完后使用变基合并的方式合入主干分支，主干分支的节点延续到了 e。参见章节 [变基合并远程分支要先合并到自身]。
...
先让主干分支从远程更新，使用 `git pull` 让 git 自由选择是否分叉合并，这样的好处是不会修改主干分支的提交记录。一般来说，主干分支上拉直的合入都是 hotfix，分叉的合入是各个功能点。

然后开发分支使用 `git merge --no-ff` 强制分叉方式合并主干分支的内容，这样的好处是不修改两个分支的提交记录，而且用菱形分叉便于区分合入的功能分支的提交记录。
...
·开发分支告一段落，使用 `git merge --ff-only` 合入到主干分支，因为开发分支先分叉合入的主干分支，二者有共同的主干分支的内容，所以合入主干分支时是快进合并，不会变更两分支的任何提交记录。
...
本地更新远程，操作步骤根据章节 [更新本地分支的操作步骤]，不直接 git pull，以便可以根据情况来选择合并策略。
...
本地更新远程，操作步骤根据章节 [更新本地分支的操作步骤]，不直接 git pull --rebase，以便可以根据情况来选择合并策略。
