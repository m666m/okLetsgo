...
Intel 的主板芯片中一个独立于CPU和操作系统的微处理器，而且它的电源状态与主机 OS 电源状态无关，但是深度绑定cpu，开机必须有它参与。ME里面有用于远程管理的功能，在出现严重漏洞的时可以在不受用户操控下远程管理企业计算机。比如：公司各部门的电脑，即使关机了，在凌晨也可以响应网管的调用统一更新计算机的固件。
